...
#include "MAPL_ErrLog.h"
   subroutine run(spec, rc)
      ...
      __RETURN_IF(writer_comm == MPI_COMM_NULL)

      benchmark = make_BW_Benchmark(spec, writer_comm, __RC)

      call write_header(writer_comm, spec%file_type, __RC)

      tot_time = 0
      tot_time_sq = 0
      associate (n => spec%n_tries)
        do i = 1, n
           t = time(benchmark, writer_comm, __RC)
           tot_time = tot_time + t
      ...
      call report(spec, avg_time, std_time, writer_comm, __RC)
