...
contains
   ...
   subroutine set_parameters(this,config_file)
      ...
      this%mpi_time = 0.0
      call MPI_COMM_SIZE(MPI_COMM_WORLD,comm_size,_IERROR)
      if (comm_size /= (this%nx*this%ny*6)) then
         call MPI_Abort(mpi_comm_world,error_code,_IERROR)
      endif
  ...
  subroutine create_communicators(this)
     ...
     local_ny = this%ny*6
     call MPI_Comm_Rank(mpi_comm_world,myid,_IERROR)
     nx0 = mod(myid,this%nx) + 1
     ny0 = myid/this%nx + 1
     color = nx0
     call MPI_Comm_Split(MPI_COMM_WORLD,color,myid,this%ycomm,_IERROR)
     color = ny0
     call MPI_Comm_Split(MPI_COMM_WORLD,color,myid,this%xcomm,_IERROR)
     ...
     call MPI_COMM_SPLIT(MPI_COMM_WORLD,color,myid,this%writers_comm,_IERROR)

     if (this%num_writers == local_ny) then
        this%gather_comm = this%xcomm
     else
        j = ny0 - mod(ny0-1,ny_by_writers)
        call MPI_COMM_SPLIT(MPI_COMM_WORLD,j,myid,this%gather_comm, _IERROR)
     end if
  ...
  subroutine create_file(this)
     ...
     if (this%netcdf_writes) then
        ...
        call MPI_INFO_CREATE(info, _IERROR)
        call MPI_INFO_SET(info,"cb_buffer_size","16777216", _IERROR)
        call MPI_INFO_SET(info,"romio_cb_write","enable", _IERROR)
        if (this%extra_info) then
           call MPI_INFO_SET(info,"IBM_largeblock_io","true", _IERROR)
           call MPI_INFO_SET(info,"striping_unit","4194304", _IERROR)
        end if
        if (this%writers_comm /= MPI_COMM_NULL) then
           if (this%split_file) then
              call MPI_COMM_RANK(this%writers_comm,writer_rank, _IERROR)
  ...
  subroutine write_variable(this,var_name,local_var)
     ...
     ndes_x = size(this%in)
       ...
       call mpi_comm_rank(this%ycomm,myrow, _IERROR)
       call mpi_comm_rank(this%gather_comm,myiorank, _IERROR)
       call mpi_comm_size(this%gather_comm,num_io_rows, _IERROR)
       ...
       if (this%write_barrier) call MPI_Barrier(MPI_COMM_WORLD, _IERROR)
...
program checkpoint_tester
   ...
   do i=1,support%n_trials
      ...
      if (support%write_counter > 0) then
         call MPI_COMM_SIZE(support%writers_comm,writer_size, _IERROR)
         call MPI_COMM_RANK(support%writers_comm,writer_rank, _IERROR)
         call MPI_AllReduce(support%data_volume,average_volume,1,MPI_DOUBLE_PRECISION,MPI_SUM,support%writers_comm, _IERROR)
         average_volume = average_volume/real(writer_size,kind=REAL64)
         call MPI_AllReduce(support%time_writing,average_time,1,MPI_DOUBLE_PRECISION,MPI_SUM,support%writers_comm, _IERROR)
