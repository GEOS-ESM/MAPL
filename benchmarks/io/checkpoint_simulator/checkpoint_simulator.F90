...
program checkpoint_tester
   ...
   call MAPL_MemReport(MPI_COMM_WORLD, __FILE__, __LINE__, "After mpi_init ")
   call MPI_Barrier(MPI_COMM_WORLD,status)
   ...
   do i=1,support%n_trials
      ...
      call MAPL_MemReport(MPI_COMM_WORLD, __FILE__, __LINE__, "Before gather ")
      call support%write_file()
      call MPI_Barrier(MPI_COMM_WORLD,status)
      call MAPL_MemReport(MPI_COMM_WORLD, __FILE__, __LINE__, "After gather ")
