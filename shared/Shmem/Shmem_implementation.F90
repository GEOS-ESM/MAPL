...
contains
  ...
  module procedure MAPL_InitializeShmem
     ...
     __ASSERT(MAPL_NodeComm /= -1,'needs informative message')

     allocate(Segs(CHUNK),stat=STATUS)
     __ASSERT(STATUS==0,'needs informative message')
     Segs(:)%shmid = -1
...
!!! Everyone detaches address from shared segment
            ...
            __ASSERT(STATUS /= -1,'needs informative message')
...
!!! The root processor destroys the segment
    ...
    implicit none
      ...
      __ASSERT(size(Ptr)==len,'needs informative message')
    ...
    module procedure MAPL_AllocNodeArray_2DI4
      ...
      call c_f_pointer(Caddr, Ptr, Shp) ! C ptr to Fortran ptr
      __ASSERT(all(shape(Ptr)==Shp),'needs informative message')
    ...
    module procedure MAPL_AllocateShared_2DI4
      integer :: status
      if(MAPL_ShmInitialized) then
         call MAPL_AllocNodeArray(Ptr, Shp, lbd, __RC)
      else
         if (TransRoot) then
...
!!! Everyone exits if it is not there
      ...
      __ASSERT(pos<=size(Segs),'needs informative message')
...
!!!  get the segment id using the common key
      ...
      if (MAPL_AmNodeRoot) then ! root process creates segment
         ...
         __ASSERT(Segs(pos)%shmid /= -1,'needs informative message')
      end if
...
!!! Check that we have valid shared memory
      ...
      __ASSERT(c_associated(Segs(pos)%addr),'needs informative message')
...
!!! Make sure everyone on a node syncs
    ...
    module procedure getNodeComm
      ...
      __ASSERT(NodeComm/=MPI_COMM_NULL,'needs informative message')
      ...
      if (MAPL_AmNodeRoot) then
         __ASSERT(MAPL_NodeRankList(MAPL_MyNodeNum)%rank(1) == rank,'needs informative message')
      end if
...
!     we store the global Min and Max of CoresPerNode
    ...
    module procedure getNodeRootsComm
      ...
      __ASSERT(NodeRootsComm/=MPI_COMM_NULL,'needs informative message')
      ...
      ! Confirm we have the proper communicator
         ...
         __ASSERT(MAPL_NumNodes == NumNodes,'needs informative message')
         call MPI_COMM_RANK(NodeRootsComm, rank, STATUS)
         __ASSERT(STATUS==MPI_SUCCESS,'needs informative message')
         __ASSERT(MAPL_MyNodeNum == rank+1,'needs informative message')
      endif
