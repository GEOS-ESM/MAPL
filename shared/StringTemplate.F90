...
character(len=2), parameter :: valid_tokens(14) = ["y4","y2","m1","m2","mc","Mc","MC","d1","d2","h1","h2","h3","n2","c2"]
...
contains
         ...
         if (c0 == "%") then
            ...
            case default
               ...
               if (valid_token) then
                  ...
                  sbuf = evaluate_token(c1//c2,year,month,day,hour,minute,second)
                  kstp = len_trim(sbuf)
                  m=k+kstp-1
   ...
   function evaluate_token(token,year,month,day,hour,minute,second) result(buffer)
      character(len=2), intent(in) :: token
