...
!           isnew    = .true.
!           FoundOrder(k) = ii(i)
!           __ASSERT(k==MAPL_HashSize(Hash),'needs informative message')
!         else
!           isnew = .false.
!           __ASSERT(FoundOrder(k)==ii(i),'needs informative message')
!         endif
!       enddo
!```
!
...
integer function MAPL_HashIncrement(Hash,i,j,k)
  ...
  if    (present(k)) then
     __ASSERT(present(j),'needs informative message')
...
integer function MAPL_HashDump(Hash,i,j)
  ...
  if(present(j)) then
     __ASSERT(size(i) == size(j),'needs informative message')
