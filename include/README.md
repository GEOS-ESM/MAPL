...
We can take the `__VERIFY` a step further. This is valid Fortran:
```fortran
...
`__RETURN_UNLESS` is the "opposite" of `__RETURN_IF` which returns if a condition is *false*. For example:
