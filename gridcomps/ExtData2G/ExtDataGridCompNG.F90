   ...
   do i=1,self%primary%import_names%size()
      ...
      if (num_rules > 1) then
         ...
         do j=1,num_rules
            ...
            write(sidx,'(I1)')j
            allocate(temp_item)
             call config_yaml%fillin_primary(current_base_name//"+"//sidx,current_base_name,temp_item,time,clock,run_range, &
                  time_range=[time_ranges(j), time_ranges(j+1)],rc=status)
