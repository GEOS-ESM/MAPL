...
!  ---------------------------------------------------
   ...
   do while (bundle_iter /= IoBundles%end())
      ...
      call MAPL_MemUtilsWrite(vm, 'MAPL_ExtData:ESMF_FieldBundleCreate '//trim(io_bundle%file_name), _RC)
   ...
   call MAPL_ExtDataCreateCFIO(IOBundles, vm=vm,_RC)
   call MAPL_TimerOff(MAPLSTATE,"---CreateCFIO")

   call MAPL_TimerOn(MAPLSTATE,"---prefetch")
   call MAPL_ExtDataPrefetch(IOBundles, file_weights=self%file_weights, vm=vm, _RC)
   ...
   call MAPL_ExtDataReadPrefetch(IOBundles, vm=vm, _RC)
   ...
   do while (bundle_iter /= IOBundles%end())
      ...
      call bundle_iter%next()
   enddo
   call MAPL_ExtDataDestroyCFIO(IOBundles, vm=vm, _RC)
...
! ............................................................................
  ...
  subroutine MAPL_ExtDataCreateCFIO(IOBundles, vm, rc)
    ...
    integer, optional,      intent(out  ) :: rc
     ...
     do while (bundle_iter /= IOBundles%end())
        io_bundle => bundle_iter%get()
        If (present(vm)) call MAPL_MemUtilsWrite(vm, 'MAPL_ExtDataCreateCFIO '//trim(io_bundle%file_name), _RC)
  ...
  subroutine MAPL_ExtDataDestroyCFIO(IOBundles, vm, rc)
     ...
     do while (bundle_iter /= IOBundles%end())
        io_bundle => bundle_iter%get()
        If (present(vm)) call MAPL_MemUtilsWrite(vm, 'MAPL_ExtDataDestroyCFIO '//trim(io_bundle%file_name), _RC)
  ...
  subroutine MAPL_ExtDataPrefetch(IOBundles,file_weights,vm,rc)
     ...
     do n = 1, nfiles
        ...
        If (present(vm)) call MAPL_MemUtilsWrite(vm, 'MAPL_ExtDataPrefetch '//trim(io_bundle%file_name), _RC)
  ...
  subroutine MAPL_ExtDataReadPrefetch(IOBundles, vm, rc)
     ...
     do n=1, nfiles
        io_bundle => IOBundles%at(n)
        If (present(vm)) call MAPL_MemUtilsWrite(vm, 'MAPL_ExtDataReadPrefetch '//trim(io_bundle%file_name), _RC)
        if (io_bundle%on_tiles) then
