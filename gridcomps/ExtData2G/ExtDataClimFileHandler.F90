...
contains
   ...
   subroutine get_file_bracket(this, input_time, source_time, bracket, fail_on_missing_file, rc)
      ...
      if (size(source_time)==2) then
         ...
         !_ASSERT(source_years(1) >= valid_years(1),'source time outside valid range')
         !_ASSERT(source_years(1) <=  valid_years(2),'source time outside valid range')
         !_ASSERT(source_years(2) >=  valid_years(1),'source time outside valid range')
         !_ASSERT(source_years(2) <= valid_years(2),'source time outside valid range')
