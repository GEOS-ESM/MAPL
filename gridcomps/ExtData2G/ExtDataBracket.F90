...
!
   ...
   subroutine interpolate_to_time(this,field,time,rc)
      ...
      right_node_set = this%right_node%check_if_initialized(__RC)
      left_node_set = this%left_node%check_if_initialized(__RC)
      ...
      call assign_fptr(field,var1d,__RC)
      if (right_node_set) then
         call assign_fptr(this%right_node%field,var1d_right,__RC)
      end if
      if (left_node_set) then
         call assign_fptr(this%left_node%field,var1d_left,__RC)
      end if
   ...
   subroutine swap_node_fields(this,rc)
      ...
      left_created  = ESMF_FieldIsCreated(this%left_node%field,__RC)
      right_created = ESMF_FieldIsCreated(this%right_node%field,__RC)
      left_created  = ESMF_FieldIsCreated(this%left_node%field,__RC)
      if (left_created .and. right_created) then     
         call assign_fptr(this%left_node%field,left_ptr,__RC) 
         call assign_fptr(this%right_node%field,right_ptr,__RC) 
         left_ptr = right_ptr
      end if
      __RETURN(__SUCCESS)
