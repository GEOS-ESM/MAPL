...
!
      ...
      call this%run_ensemble(rc=status); __VERIFY(status)
      call this%finalize_mpi(rc=status); __VERIFY(status)
   ...
   subroutine run_ensemble(this, unusable, rc)
      ...
      subcommunicator = this%create_member_subcommunicator(this%comm_world, rc=status); __VERIFY(status)
      if (subcommunicator /= MPI_COMM_NULL) then
         call this%initialize_io_clients_servers(subcommunicator, rc = status); __VERIFY(status)
         call this%run_member(rc=status); __VERIFY(status)
   ...
   subroutine run_member(this, rc)
      ...
      select case(split_comm%get_name())
      case('model')
         call this%run_model(comm=split_comm%get_subcommunicator(), rc=status); __VERIFY(status)
      end select
   ...
   subroutine run_model(this, comm, unusable, rc)
      ...
      if (esmfConfigFileExists) then
         ...
         call ESMF_Initialize (configFileName=esmfConfigFile, mpiCommunicator=comm, vm=vm, __RC)
      else
         call ESMF_Initialize (logKindFlag=this%cap_options%esmf_logging_mode, mpiCommunicator=comm, vm=vm, __RC)
      end if

      ! We check to see if ESMF_COMM was built as mpiuni which is not allowed for MAPL
      call ESMF_VmGet(vm, esmfComm = esmfComm, __RC)
      __ASSERT( esmfComm /= 'mpiuni', 'ESMF_COMM=mpiuni is not allowed for MAPL')
   ...
   subroutine initialize_cap_gc(this, unusable, n_run_phases, rc)
     ...
     pinflag = GetPinFlagFromConfig(this%cap_options%cap_rc_file, __RC)
     ...
     if (this%non_dso) then
        ...
        __ASSERT(this%cap_options%root_dso /= 'none',"No set services specified, must pass a dso")
        call MAPL_CapGridCompCreate(this%cap_gc, this%get_cap_rc_file(), &
   ...
   subroutine step_model(this, rc)
     ...
     integer :: status
     call this%cap_gc%step(rc = status); __VERIFY(status)
     __RETURN(__SUCCESS)
   ...
   subroutine rewind_model(this, time, rc)
     ...
     integer, intent(out) :: rc
     integer :: status
     call this%cap_gc%rewind_clock(time,rc = status); __VERIFY(status)
     __RETURN(__SUCCESS)
...
!!$      external :: chdir
      ...
      split_comm = this%splitter%split(rc=status); __VERIFY(status)
   ...
   subroutine initialize_mpi(this, unusable, rc)
      ...
      if (.not. this%mpi_already_initialized) then
         call ESMF_InitializePreMPI(__RC)
      ...
      __ASSERT(provided >= MPI_THREAD_SERIALIZED, 'ESMF requires minimum thread level is MPI_THREAD_SERIALIZED. Please replace MPI lib or use MPI (initialize MPI or launch MPI) in an appropriate way.')
   ...
   function GetPinFlagFromConfig(rcfile, rc) result(pinflag)
     ...
     call ESMF_ConfigLoadFile(config,rcfile, __RC)
     call ESMF_ConfigGetAttribute(config, value=pinflag_str, &
          label='ESMF_PINFLAG:', default='SSI_CONTIG', __RC)
     ...
     case default
        __ASSERT(.false.,'Unsupported PIN flag')
     end select

     call ESMF_ConfigDestroy(config, __RC)
     __RETURN(__SUCCESS)
