...
contains
   ...
   function new_CapOptions_from_flap(unusable, description, authors, dummy, extra, rc) result (cap_options)
      ...
      if (present(extra)) then
         call extra(flap_cli%cli_options, __RC)
      end if

      call flap_cli%cli_options%parse(error=status); __VERIFY(status)
   ...
   function new_CapOptions_from_flap_back_comp(unusable, description, authors, extra, rc) result (flapcap)
      ...
      if (present(extra)) then
         call extra(flapcap%cli_options, __RC)
      end if

      call flapcap%cli_options%parse(error=status); __VERIFY(status)
      __VERIFY(status)
   ...
   subroutine fill_cap_options(flapCLI, cap_options, unusable, rc)
      ...
      call flapCLI%cli_options%get(val=buffer, switch='--root_dso', error=status); __VERIFY(status)
      cap_options%root_dso = trim(buffer)

      call flapCLI%cli_options%get(val=buffer, switch='--egress_file', error=status); __VERIFY(status)
      cap_options%egress_file = trim(buffer)

      call flapCLI%cli_options%get(val=use_sub_comm, switch='--use_sub_comm', error=status); __VERIFY(status)
      cap_options%use_comm_world = .not. use_sub_comm

      if ( .not. cap_options%use_comm_world) then
         call flapCLI%cli_options%get(val=buffer, switch='--comm_model', error=status); __VERIFY(status)
         __ASSERT(trim(buffer) /= '*', "Should provide comm for model")
         call flapCLI%cli_options%get(val=cap_options%comm, switch='--comm_model', error=status); __VERIFY(status)
      else
        ! comm will be set to MPI_COMM_WORLD later on in initialize_mpi
        ! npes will be set to npes_world later on in initialize_mpi
      endif

      call flapCLI%cli_options%get(val=cap_options%npes_model, switch='--npes_model', error=status); __VERIFY(status)
      call flapCLI%cli_options%get(val=compress_nodes, switch='--compress_nodes', error=status); __VERIFY(status)
      cap_options%isolate_nodes = .not. compress_nodes
      call flapCLI%cli_options%get(val=cap_options%fast_oclient, switch='--fast_oclient', error=status); __VERIFY(status)
      call flapCLI%cli_options%get(val=cap_options%with_io_profiler, switch='--with_io_profiler', error=status); __VERIFY(status)
      call flapCLI%cli_options%get(val=cap_options%with_esmf_moab, switch='--with_esmf_moab', error=status); __VERIFY(status)
      call flapCLI%cli_options%get_varying(val=cap_options%npes_input_server, switch='--npes_input_server', error=status); __VERIFY(status)
      call flapCLI%cli_options%get_varying(val=cap_options%npes_output_server, switch='--npes_output_server', error=status); __VERIFY(status)
      call flapCLI%cli_options%get_varying(val=cap_options%nodes_input_server, switch='--nodes_input_server', error=status); __VERIFY(status)
      call flapCLI%cli_options%get_varying(val=nodes_output_server, switch='--nodes_output_server', error=status); __VERIFY(status)
      call flapCLI%cli_options%get(val=one_node_output, switch='--one_node_output', error=status); __VERIFY(status)
      ...
      call flapCLI%cli_options%get(val=buffer, switch='--esmf_logtype', error=status); __VERIFY(status)
      ...
      call flapCLI%cli_options%get(val=buffer, switch='--prefix', error=status); __VERIFY(status)
      cap_options%ensemble_subdir_prefix = trim(buffer)
      call flapCLI%cli_options%get(val=cap_options%n_members, switch='--n_members', error=status); __VERIFY(status)

      call flapCLI%cli_options%get(val=buffer, switch='--cap_rc', error=status); __VERIFY(status)
      cap_options%cap_rc_file = trim(buffer)

      ! Logging options
      call flapCLI%cli_options%get(val=buffer, switch='--logging_config', error=status); __VERIFY(status)
      cap_options%logging_config = trim(buffer)
      ! ouput server type options
      call flapCLI%cli_options%get(val=buffer, switch='--oserver_type', error=status); __VERIFY(status)
      cap_options%oserver_type = trim(buffer)
      call flapCLI%cli_options%get(val=cap_options%npes_backend_pernode, switch='--npes_backend_pernode', error=status); __VERIFY(status)
