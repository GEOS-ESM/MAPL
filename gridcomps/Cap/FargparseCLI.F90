...
contains
   ...
   subroutine add_command_line_options(parser, unusable, rc)
     ...
     call parser%add_argument('--one_node_output', &
      ...
      call parser%add_argument('--enable_global_timeprof', &
           help='Enables global time profiler', &
           action='store_true')

      call parser%add_argument('--enable_global_memprof', &
   ...
   subroutine fill_cap_options(fargparseCLI, cap_options, unusable, rc)
      ...
      ! Profiling options
      option => fargparseCLI%options%at('enable_global_timeprof')
      if (associated(option)) then
         call cast(option, cap_options%enable_global_timeprof, _RC)
      end if

      option => fargparseCLI%options%at('enable_global_memprof')
      if (associated(option)) then
         call cast(option, cap_options%enable_global_memprof, _RC)
      end if
