...
!   ------------------------------
    self%CF = ESMF_ConfigCreate(__RC)
    inquire(file="MAPL_OrbGridComp.rc", exist=found)
    if (found) then
       call ESMF_ConfigLoadFile ( self%CF,'MAPL_OrbGridComp.rc',rc=status)
       __VERIFY(STATUS)

       call ESMF_ConfigGetAttribute(self%CF, self%verbose, Label='verbose:', default=.false. ,  __RC )

!                       ------------------------
!                         Get Mask Definitions
!                       ------------------------

       call ESMF_ConfigGetDim(self%CF, self%no, nCols, LABEL='Nominal_Orbits::',__RC)
       __ASSERT(self%no>0,'needs informative message')
       ...
       call ESMF_ConfigFindLabel(self%CF, 'Nominal_Orbits::',__RC)
       do i = 1, self%no
          call ESMF_ConfigNextLine(self%CF,__RC)
          call ESMF_ConfigGetAttribute(self%CF,self%Instrument(i),__RC)
          call ESMF_ConfigGetAttribute(self%CF,self%Satellite(i),__RC)
          call ESMF_ConfigGetAttribute(self%CF,self%Swath(i),__RC)
          call ESMF_ConfigGetAttribute(self%CF,self%halo(i),__RC)
          if ( self%verbose .AND. MAPL_AM_I_ROOT() ) then
...
!  Get my name and set-up traceback handle
!  ---------------------------------------
   call ESMF_GridCompGet( GC, name=comp_name, VM=VM, __RC )
   Iam = trim(comp_name) // '::Run'

!  Extract relevant runtime information
!  ------------------------------------
   call extract_ ( GC, CLOCK, self, GRID, CF, time, nymd, nhms, timeinterval,  __RC)
...
!   IntervalTime=time-timeinterval
   IntervalTime=time
   call ESMF_TimeGet(IntervalTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
...
!  get orb bundle
   ...
   __VERIFY(STATUS)
   __ASSERT(NORB==self%no,'needs informative message')
...
!   ------------------------------------------
    call ESMF_ClockGet(CLOCK,currTIME=TIME,timeStep=TimeInterval,__RC)
    call ESMF_TimeGet(TIME ,yy=iyr, mm=imm, dd=idd, h=ihr, m=imn, s=isc, __RC)
    call MAPL_PackTime(nymd,iyr,imm,idd)
    call MAPL_PackTime(nhms,ihr,imn,isc)
