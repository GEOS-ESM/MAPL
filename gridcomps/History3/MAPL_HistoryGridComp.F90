...
! EOP
    ...
    character(len=ESMF_MAXSTR)                :: BLANK=EMPTY_STRING
...
! -------------------------------------------------
    ...
    call ESMF_ConfigGetAttribute ( config, value=INTSTATE%expsrc, &
                                   label ='EXPSRC:', default=EMPTY_STRING, rc=status )
    _VERIFY(STATUS)
    call ESMF_ConfigGetAttribute ( config, value=INTSTATE%expid, &
                                   label ='EXPID:', default=EMPTY_STRING, rc=status )
    _VERIFY(STATUS)
    call ESMF_ConfigGetAttribute ( config, value=INTSTATE%expdsc, &
                                   label ='EXPDSC:', default=EMPTY_STRING, rc=status )
    _VERIFY(STATUS)
    ...
    call ESMF_ConfigGetAttribute ( config, value=INTSTATE%global_atts%contact, &
                                   label ='CONTACT:', default=EMPTY_STRING, _RC)
...
! ----------------------------------
    ...
    _VERIFY(STATUS)
    do while (.not.tend)
          call ESMF_ConfigGetAttribute ( config,value=tmpstring,default=EMPTY_STRING,rc=STATUS) !ALT: we don't check return status!!!
          if (tmpstring /= EMPTY_STRING)  then
...
! ------------------------
    ...
    LISTLOOP: do n=1,nlist
       ...
       _VERIFY(STATUS)
       call ESMF_ConfigGetAttribute ( cfg, value=list(n)%template, default=EMPTY_STRING, &
       ...
       _VERIFY(STATUS)
       call ESMF_ConfigGetAttribute ( cfg, list(n)%vunit, default=EMPTY_STRING, &
...
! Get an optional file containing a 1-D track for the output
       call ESMF_ConfigGetAttribute(cfg, value=list(n)%trackFile, default=EMPTY_STRING, &
                                    label=trim(string) // 'track_file:', rc=status)
       if (trim(list(n)%trackfile) /= EMPTY_STRING) list(n)%timeseries_output = .true.
...
!      ----------------------------------------------------
       ...
       if (intstate%version >= 2) then
          call ESMF_ConfigGetAttribute ( cfg, value=field_set_name, label=trim(string)//'field_set:', &
               & default=EMPTY_STRING, rc=status)
          _VERIFY(status)
          if (field_set_name /= EMPTY_STRING) then  ! field names already parsed
...
! ------------------------------------------------------------------------
       ...
       case(1:)
          call ESMF_ConfigGetAttribute ( cfg, tmpString, default=EMPTY_STRING , &
                                         label=trim(string) // 'grid_label:' ,rc=status )
          _VERIFY(status)
          if (len_trim(tmpString) == 0) then
             list(n)%output_grid_label=EMPTY_STRING
          else
             cubeFormat = 0
             i1 = index(tmpstring(:),",")
             if( i1.eq.1 )  tmpstring = adjustl( tmpstring(2:)   )
...
! ---------------------------------------------------
       call ESMF_ConfigGetAttribute ( cfg, value=tilefile, default=EMPTY_STRING, &
                                      label=trim(string) // 'regrid_exch:' ,rc=status )
       _VERIFY(STATUS)

       call ESMF_ConfigGetAttribute ( cfg, value=gridname, default=EMPTY_STRING, &
                                      label=trim(string) // 'regrid_name:' ,rc=status )
       _VERIFY(STATUS)

       NULLIFY(IntState%Regrid(n)%PTR)
       if (tilefile /= EMPTY_STRING .OR. gridname /= EMPTY_STRING) then
       ...
       do m=1,list(n)%field_set%nfields
          ...
          if (list(n)%regex .or. &
             ...
             do while ( k.le.nstatelist )
                if (statelist(k) == EMPTY_STRING) statelist(k) = list(n)%field_set%fields(2,m)
...
!        History, and calculate Xform, then do T2T, followed by T2G
         ...
         if (gridname(1:10) == 'tile_grid_') then
            ...
            _ASSERT(IntState%Regrid(n)%PTR%gridname /= EMPTY_STRING,'needs informative message')
...
! followed by T2T (Xform), and finally G2T on "output" LS("output" grid)
                  ...
                  _ASSERT(IntState%Regrid(n)%PTR%tilefile /= EMPTY_STRING,'needs informative message')
...
!       endif
    ...
    do n=1,nlist
       ...
       if (list(n)%format == 'CFIO') then
          ...
          if (associated(list(n)%levels) .and. list(n)%vvars(1) /= EMPTY_STRING) then
             list(n)%vdata = VerticalData(levels=list(n)%levels,vcoord=list(n)%vvars(1),vscale=list(n)%vscale,vunit=list(n)%vunit,rc=status)
             _VERIFY(status)
          else if (associated(list(n)%levels) .and. list(n)%vvars(1) == EMPTY_STRING) then
          ...
          if (list(n)%timeseries_output) then
             ...
             if (trim(list(n)%output_grid_label)/=EMPTY_STRING) then
...
! --------------------------------
   ...
   if( MAPL_AM_I_ROOT() ) then
      ...
      do n=1,nlist
         ...
         if( list(n)%vvars(1)/=EMPTY_STRING ) then
                                           print *, '   Vert Interp  Var: ',  trim(list(n)%vvars(1))
            if( trim(list(n)%vunit)/=EMPTY_STRING  ) print *, '   Vertical    Unit: ',  trim(list(n)%vunit)
            ...
            do k = 1, size(needSplit) ! loop over "old" fld_set
               ...
               do i=1,size(fieldNames)
                  ...
                  call appendFieldSet(newFieldSet, fldName, &
                       stateName=stateName, &
                       aliasName=fldName, &
                       specialName=EMPTY_STRING, rc=status)
...
!         end if
   ...
   if(any(Writing)) call WRITE_PARALLEL(EMPTY_STRING)
...
! -----------------
   ...
   OPENLOOP: do n=1,nlist
      if( Writing(n) ) then
         ...
         if (trim(INTSTATE%expid) == EMPTY_STRING) then
            fntmpl =          trim(list(n)%filename)
         else
            fntmpl = "%s." // trim(list(n)%filename)
         endif

         if (trim(list(n)%template) /= EMPTY_STRING) then
...
! -------------------------
   ...
   if( trim(expid) == EMPTY_STRING ) then
   ...
   if( list%template == EMPTY_STRING .or. list%duration == 0 ) then
