...
module mapl_ExtDataReader_mod
   ...
   subroutine read_items(this, lgr, rc)
      ...
      integer :: status, i, j, pass, pfio_typekind, num_fields, request_id
      ...
      do pass = 0, 1
         ...
         do i=1,size(field_list)
            ...
            do j=i,size(field_list)
               ...
               if (.not. same_submission_key(this, field_list(i), field_list(j), allow_cache_only_prefetch, pass, rc=status)) cycle
               _VERIFY(status)
               ...
               call ESMF_FieldGet(field_list(j), grid=grid, typekind=esmf_typekind, _RC)
               element_count = MAPL_FieldGetLocalElementCount(field_list(j), _RC)
               ...
               call MAPL_FieldGetCptr(field_list(j), address, _RC)
    ...
    logical function same_submission_key(this, field_a, field_b, allow_cache_only_prefetch, pass, rc) result(matches)
      class(ExtDataReader), intent(in) :: this
      type(ESMF_Field), intent(in) :: field_a
      type(ESMF_Field), intent(in) :: field_b
      logical, intent(in) :: allow_cache_only_prefetch
      integer, intent(in) :: pass
      integer, optional, intent(out) :: rc

      character(len=ESMF_MAXSTR) :: field_name_a, field_name_b
      integer, pointer :: prefetch_only_a, prefetch_only_b
      integer, pointer :: client_id_a, client_id_b, time_index_a, time_index_b
      character(len=:), pointer :: alias_a, alias_b, filename_a, filename_b
      integer :: status

      call ESMF_FieldGet(field_a, name=field_name_a, _RC)
      call ESMF_FieldGet(field_b, name=field_name_b, _RC)

      prefetch_only_a => this%prefetch_only_map%at(trim(field_name_a))
      prefetch_only_b => this%prefetch_only_map%at(trim(field_name_b))
      if ((prefetch_only_a == 1) .and. (.not. allow_cache_only_prefetch)) then
         matches = .false.
         _RETURN(_SUCCESS)
      end if
      if ((prefetch_only_b == 1) .and. (.not. allow_cache_only_prefetch)) then
         matches = .false.
         _RETURN(_SUCCESS)
      end if
      if (prefetch_only_a /= pass .or. prefetch_only_b /= pass) then
         matches = .false.
         _RETURN(_SUCCESS)
      end if

      alias_a => this%alias_map%at(trim(field_name_a))
      alias_b => this%alias_map%at(trim(field_name_b))
      filename_a => this%filename_map%at(trim(field_name_a))
      filename_b => this%filename_map%at(trim(field_name_b))
      client_id_a => this%client_id_map%at(trim(field_name_a))
      client_id_b => this%client_id_map%at(trim(field_name_b))
      time_index_a => this%time_index_map%at(trim(field_name_a))
      time_index_b => this%time_index_map%at(trim(field_name_b))

      matches = trim(alias_a) == trim(alias_b)
      matches = matches .and. trim(filename_a) == trim(filename_b)
      matches = matches .and. client_id_a == client_id_b
      matches = matches .and. time_index_a == time_index_b

      _RETURN(_SUCCESS)
    end function same_submission_key
