...
!  ---------------------------------------
   Iam = 'Initialize_'
   call ESMF_GridCompGet( GC, name=comp_name, config=CF_main, __RC )
   Iam = trim(comp_name) // '::' // trim(Iam)

!  Extract relevant runtime information
!  ------------------------------------
   call extract_ ( GC, self, CF_main, __RC)
   self%CF = CF_main

!  Get the GC pFlogger
!  -------------------
   call MAPL_GetLogger(gc, lgr, __RC)
...
!                         ---------------------------
   ...
   call ESMF_ConfigNextLine(CFtemp,__RC)
         ...
         if (trim(thisLine) == "PrimaryExports%%" .or. trim(thisLine) == "DerivedExports%%" ) then
            select case (trim(thisLine))
               case ("PrimaryExports%%")
                  ...
                  do while(inBLock)
                     call ESMF_ConfigNextLine(CFtemp, __RC)
                     call ESMF_ConfigGetAttribute(CFtemp,thisLine,__RC)
                     if (trim(thisLine) == "%%") then
                        ...
                        !call ESMF_ConfigGetAttribute(CFtemp, primary%item(totalPrimaryEntries)%name,  __RC)
                        ...
                        ! units entry
                        call ESMF_ConfigGetAttribute(CFtemp, primary%item(totalPrimaryEntries)%units, __RC)

                        ! climatology entry
                        call ESMF_ConfigGetAttribute(CFtemp, buffer, __RC)
                        buffer = ESMF_UtilStringLowerCase(buffer, __RC)
                        ...
                        if (trim(buffer) == 'y') then
                           ...
                           k = index(buffer,';')
                           __ASSERT(k > 0,'ERROR: MAPL fractional regridding requires semi-colon in ExtData.rc entry: '//trim(primary%item(totalPrimaryEntries)%name))
                        ...
                        ! offset entry
                        call ESMF_ConfigGetAttribute(CFtemp, c_offset, __RC)
                        ...
                        ! scaling entry
                        call ESMF_ConfigGetAttribute(CFtemp, c_scale, __RC)
                        ...
                        call ESMF_ConfigGetAttribute(CFtemp, primary%item(totalPrimaryEntries)%var,    __RC)
                        ! file template entry
                        call ESMF_ConfigGetAttribute(CFtemp, primary%item(totalPrimaryEntries)%file,   __RC)
               ...
               case ("DerivedExports%%")
                  ...
                  do while(inBlock)
                     ...
                     if (trim(thisLine) == "%%") then
                        ...
                        call ESMF_ConfigGetAttribute(CFtemp,derived%item(totalDerivedEntries)%expression,__RC)
                        call ESMF_ConfigGetAttribute(CFtemp,derived%item(totalDerivedEntries)%refresh_template, __RC)
       ...
       do J = 1, primary%nItems
          ...
          if (primary%item(J)%isVector) then
             ...
             if ( trim(ItemNames(I)) == trim(component1) ) then
                ...
                call ESMF_StateGet(Export,component1,field,__RC)
                ...
                __ASSERT( ITEMTYPES(I) == ESMF_StateItem_Field ,'Vector pair must be fields')
                exit
                ...
                primary%item(j)%ExtDataAlloc = .false.
                if ( primary%item(j)%foundComp1 .and. primary%item(j)%foundComp2 ) PrimaryItemCount = PrimaryItemCount + 1
                call ESMF_StateGet(Export,component2,field,__RC)
                call MAPL_StateAdd(self%ExtDataState,field,__RC)
                ! put protection in, if you are filling vector pair, they must be fields, no bundles
                __ASSERT( ITEMTYPES(I) == ESMF_StateItem_Field ,'Vector pair must be fields')
                exit
             ...
             if (ItemNames(I) == primary%item(J)%name) then
                ...
                if (primary%item(j)%isConst .and. ITEMTYPES(I) == ESMF_StateItem_FieldBundle) then
                   __FAIL( 'Can not have constant bundle in ExtData.rc file')
                end if
                ...
                if (ITEMTYPES(I) == ESMF_StateItem_Field) then
                   ...
                   call ESMF_StateGet(Export,VarName,field,__RC)
                   call MAPL_StateAdd(self%ExtDataState,field,__RC)
                else if (ITEMTYPES(I) == ESMF_StateItem_FieldBundle) then
                   primary%item(J)%vartype = MAPL_BundleItem
                   call ESMF_StateGet(Export,VarName,bundle,__RC)
                   call MAPL_StateAdd(self%ExtDataState,bundle,__RC)
                end if
                exit
          ...
          do J = 1, derived%nItems
             if (ItemNames(I) == derived%item(J)%name) then
                ...
                if (ITEMTYPES(I) == ESMF_StateItem_FieldBundle) then
                   __FAIL('Derived items cannot be field bundle')
                end if
                found = .true.
...
!   if we find them allocate them based on grid of variable we are trying to fill
    do i=1, derived%nItems
       if (DerivedVarNeeded(i)) then
          ...
          expression = ESMF_UtilStringLowerCase(expression, __RC)
          ...
          if (derived%item(i)%masking) then
             call GetMaskName(derived%item(i)%expression,VarNames,LocalVarNeeded,__RC)
          else
             call CheckSyntax(derived%item(i)%expression,VarNames,LocalVarNeeded,__RC)
          end if

          do j=1, primary%nItems
             if (LocalVarNeeded(j)) then
                VarName = trim(primary%item(j)%name)
                call ESMF_StateGet(self%ExtDataState,VarName,itemType=itemType,__RC)
                if (itemType == ESMF_STATEITEM_FIELD) then
                   call ESMF_StateGet(self%ExtDataState,VarName,field,__RC)
                else
                   VarName = trim(derived%item(i)%name)
                   call ESMF_StateGet(self%ExtDataState,VarName,field,__RC)
                   VarName=trim(primary%item(j)%name)
                   fieldnew = MAPL_FieldCreate(field,varname,doCopy=.true.,__RC)
                   primary%item(j)%fileVars%xname=trim(primary%item(j)%var)
                   call MAPL_StateAdd(self%ExtDataState,fieldnew,__RC)
                   PrimaryVarNeeded(j) = .true.
    ...
    do i=1,primary%nItems
       if (PrimaryVarNeeded(i)) then
          ...
          if (self%primary%item(counter)%isVector) then
             __ASSERT( self%primary%item(counter)%foundComp1, 'Did not find Component 1 vector item')
             __ASSERT( self%primary%item(counter)%foundComp2 ,'Did not find both Component 2 vector item')
          end if
       end if
    end do
    __ASSERT(counter==PrimaryItemCount,'Not all needed primary vars found')
    ...
    if (DerivedItemCount > 0) then
       ...
       __ASSERT(counter==DerivedItemCount,'Not all needed derived vars found')
    end if
   ...
   PrimaryLoop: do i = 1, self%primary%nItems
      ...
      if ( .not. item%isConst )  then
         call CreateTimeInterval(item,clock,__RC)
      end if
      ...
      call ESMF_TimeIntervalSet(item%tshift,__RC)
      if (k.ne.0) then
         __ASSERT(trim(item%refresh_template(:k-1))=="0",'Refresh template must start with 0 when offset is present')
         ...
         call ESMF_TimeIntervalSet(item%tshift,h=ihr,m=imn,s=isc,__RC)
         item%refresh_template = "0"
      end if
      call SetRefreshAlarms(clock,primaryItem=item,__RC)

      if (item%vartype == MAPL_BundleItem) then

         call ESMF_StateGet(self%ExtDataState, trim(item%name), bundle,__RC)
         ! let us check that bundle is empty
         call ESMF_FieldBundleGet(bundle, fieldcount = fieldcount , __RC)
         __ASSERT(fieldcount == 0,'Bundle must be empty')
         call MAPL_CFIORead(item%file,time,bundle,noread=.true.,ignorecase=self%ignorecase, only_vars=item%var,__RC)
...
!  --------------------------------------------------------
      ...
      if (item%isConst) then
         ...
         if (item%vartype == MAPL_FieldItem) then
            call ESMF_StateGet(self%ExtDataState,trim(item%name),field,__RC)
            call ESMF_FieldGet(field,dimCount=fieldRank,__RC)
            if (fieldRank == 2) then
                  call MAPL_GetPointer(self%ExtDataState, ptr2d, trim(item%name),__RC)
                  ptr2d = item%const
            else if (fieldRank == 3) then
                  call MAPL_GetPointer(self%ExtDataState, ptr3d, trim(item%name), __RC)
                  ptr3d = item%const
            endif
         else if (item%vartype == MAPL_BundleItem) then
            __FAIL('Cannot assign constant to field bundle')
         else if (item%vartype == MAPL_ExtDataVectorItem) then
            call ESMF_StateGet(self%ExtDataState,trim(item%vcomp1),field,__RC)
            call ESMF_FieldGet(field,dimCount=fieldRank,__RC)
            if (fieldRank == 2) then
                  call MAPL_GetPointer(self%ExtDataState, ptr2d, trim(item%vcomp1),__RC)
                  ptr2d = item%const
            else if (fieldRank == 3) then
                  call MAPL_GetPointer(self%ExtDataState, ptr3d, trim(item%vcomp1), __RC)
                  ptr3d = item%const
            endif
            call ESMF_StateGet(self%ExtDataState,trim(item%vcomp2),field,__RC)
            call ESMF_FieldGet(field,dimCount=fieldRank,__RC)
            if (fieldRank == 2) then
                  call MAPL_GetPointer(self%ExtDataState, ptr2d, trim(item%vcomp2),__RC)
                  ptr2d = item%const
            else if (fieldRank == 3) then
                  call MAPL_GetPointer(self%ExtDataState, ptr3d, trim(item%vcomp2), __RC)
                  ptr3d = item%const
            endif
         end if
         cycle
      ...
      if (PrimaryExportIsConstant_(item)) then
         if (index(item%file,'%') == 0) then
            call MakeMetadata(item%file,item%pfioCollection_id,metadata,__RC)
            call metadata%get_coordinate_info('time',coordSize=tsteps,__RC)
            if (tsteps == 1) then
               item%cyclic = 'single'
      ...
      call GetClimYear(item,__RC)
      ! get levels, other information
      call GetLevs(item,time,self%ExtDataState,self%allowExtrap,__RC)
      call ESMF_VMBarrier(vm)
      ...
      if (item%vartype == MAPL_FieldItem) then
         ...
         item%modelGridFields%v1_finterp1 = MAPL_FieldCreate(field,item%var,doCopy=.true.,__RC)
         item%modelGridFields%v1_finterp2 = MAPL_FieldCreate(field,item%var,doCopy=.true.,__RC)
         if (item%do_fill .or. item%do_vertInterp) then
            call createFileLevBracket(item,cf_main,__RC)
         end if

      else if (item%vartype == MAPL_BundleItem) then

         call ESMF_StateGet(self%ExtDataState, trim(item%name), bundle,__RC)
         call ESMF_FieldBundleGet(bundle,grid=grid,__RC)
         call ESMF_ClockGet(CLOCK, currTIME=time, __RC)
         item%binterp1 = ESMF_FieldBundleCreate( __RC)
         call ESMF_FieldBundleSet(item%binterp1, GRID=GRID, __RC)
         item%binterp2 = ESMF_FieldBundleCreate( __RC)
         call ESMF_FieldBundleSet(item%binterp2, GRID=GRID, __RC)
         call MAPL_CFIORead(item%file,time,item%binterp1,noread=.true.,ignorecase=self%ignorecase,only_vars=item%var,__RC)
         call MAPL_CFIORead(item%file,time,item%binterp2,noread=.true.,ignorecase=self%ignorecase,only_vars=item%var,__RC)

      else if (item%vartype == MAPL_ExtDataVectorItem) then

         ! Only some methods are supported for vector regridding
         __ASSERT(any(item%Trans /= [REGRID_METHOD_BILINEAR,REGRID_METHOD_CONSERVE_HFLUX]), 'Regrid method unsupported for vectors.')

         block
            integer :: gridRotation1, gridRotation2
            call ESMF_StateGet(self%ExtDataState, trim(item%vcomp1), field,__RC)
            call ESMF_AttributeGet(field, NAME='ROTATION', value=gridRotation1, __RC)
            call ESMF_StateGet(self%ExtDataState, trim(item%vcomp2), field,__RC)
            call ESMF_AttributeGet(field, NAME='ROTATION', value=gridRotation2, __RC)
            __ASSERT(GridRotation1 == gridRotation2,'Grid rotations must match when performing vector re-gridding')
         end block
         ...
         item%modelGridFields%v1_finterp1 = MAPL_FieldCreate(field, item%fcomp1,doCopy=.true.,__RC)
         item%modelGridFields%v1_finterp2 = MAPL_FieldCreate(field, item%fcomp1,doCopy=.true.,__RC)
         call ESMF_StateGet(self%ExtDataState, trim(item%vcomp2), field,__RC)
         item%modelGridFields%v2_finterp1 = MAPL_FieldCreate(field, item%fcomp2,doCopy=.true.,__RC)
         item%modelGridFields%v2_finterp2 = MAPL_FieldCreate(field, item%fcomp2,doCopy=.true.,__RC)
         if (item%do_fill .or. item%do_vertInterp) then
            call createFileLevBracket(item,cf_main,__RC)
         end if
      ...
      call ESMF_TimeSet(item%refresh_time, yy=0, __RC)
   end do PrimaryLoop
   ...
   DerivedLoop: do i =1, self%derived%nItems
      ...
      call ESMF_TimeIntervalSet(derivedItem%tshift,__RC)
      if (k.ne.0) then
         __ASSERT(trim(derivedItem%refresh_template(:k-1))=="0",'Refresh template must start with 0 when offset is present')
         ...
         call ESMF_TimeIntervalSet(derivedItem%tshift,h=ihr,m=imn,s=isc,__RC)
         derivedItem%refresh_template = "0"
      end if

      call SetRefreshAlarms(clock,derivedItem=derivedItem,__RC)

      call ESMF_TimeSet(self%derived%item(i)%refresh_time, yy=0, __RC)
   end do DerivedLoop
   ...
   if (any(self%primary%item%do_VertInterp .eqv. .true.)) then
      do i=1,size(self%primary%item)
         ...
         if (self%primary%item(i)%vartype==MAPL_BundleItem) then
            __FAIL('Cannot perform vertical interpolation on field bundle')
         end if
      enddo
      __ASSERT(idx/=-1,'Surface pressure not present for vertical interpolation')
      ...
      __ASSERT(trim(self%primary%item(idx)%units)=="PA",'PS must be in units of PA')
   end if
!  check for PHIS
   idx = -1
   if (any(self%primary%item%do_VertInterp .eqv. .true.)) then
...
!  -------------------------
!ALT   call MAPL_GenericRunChildren ( GC, IMPORT, EXPORT, CLOCK,  __RC)
...
!  ------------------------------------
   ...
   call ESMF_ClockGet(CLOCK, currTIME=time0, __RC)
...
!  ---------------------------------------------------
   ...
   READ_LOOP: do i = 1, self%primary%nItems
      ...
      call CheckUpdate(doUpdate_,time,time0,hasRun,primaryItem=item,__RC)
      ...
      DO_UPDATE: if (doUpdate_) then
         ...
         HAS_RUN: if ( hasRun .eqv. .false.) then
            ...
            if (NotSingle) then
               ...
               __VERIFY(status)
               call IOBundle_Add_Entry(IOBundles,item,self%primaryOrder(i),file_processed1,MAPL_ExtDataLeft,item%tindex1,__RC)
               ...
               __VERIFY(STATUS)
               call IOBundle_Add_Entry(IOBundles,item,self%primaryOrder(i),file_processed2,MAPL_ExtDataRight,item%tindex2,__RC)
               ...
               item%time1 = MAPL_ExtDataGetFStartTime(item,trim(item%file),__RC)
               item%interp_time1 = item%time1
               file_processed1 = item%file
               call IOBundle_Add_Entry(IOBundles,item,self%primaryOrder(i),file_processed1,MAPL_ExtDataLeft,1,__RC)
            end if
         ...
         NOT_SINGLE: if (NotSingle) then
            ...
            DO_SWAP: if (swap) then
               ...
               call swapBracketInformation(item,__RC)
            ...
            UPDATE_R: if (updateR) then
               ...
               __VERIFY(STATUS)
               call IOBundle_Add_Entry(IOBundles,item,self%primaryOrder(i),file_processed,MAPL_ExtDataRight,item%tindex2,__RC)
            ...
            UPDATE_L: if (updateL) then
               ...
               __VERIFY(STATUS)
               call IOBundle_Add_Entry(IOBundles,item,self%primaryOrder(i),file_processed,MAPL_ExtDataLeft,item%tindex1,__RC)
   ...
   INTERP_LOOP: do i = 1, self%primary%nItems
      ...
      if (doUpdate(i)) then
         ...
         if (item%vartype == MAPL_FieldItem) then
               ...
               call ESMF_StateGet(self%ExtDataState, item%name, field, __RC)
               call MAPL_ExtDataInterpField(item,useTime(i),field,__RC)

         else if (item%vartype == MAPL_BundleItem) then

               call ESMF_StateGet(self%ExtDataState, item%name, bundle, __RC)
               call ESMF_FieldBundleGet(bundle, fieldCount = fieldCount, __RC)
               allocate(names(fieldCount),__STAT__)
               call ESMF_FieldBundleGet(bundle, fieldNameList = Names, __RC)
               do j = 1,fieldCount
                  call ESMF_FieldBundleGet(bundle,names(j), field=field, __RC)
                  call MAPL_ExtDataInterpField(item,useTime(i),field,__RC)
               enddo
               deallocate(names)
               ...
               call ESMF_StateGet(self%ExtDataState, item%vcomp1, field, __RC)
               call MAPL_ExtDataInterpField(item,useTime(i),field,vector_comp=1,__RC)
               call ESMF_StateGet(self%ExtDataState, item%vcomp2, field, __RC)
               call MAPL_ExtDataInterpField(item,useTime(i),field,vector_comp=2,__RC)
   ...
   do i=1,self%derived%nItems
      ...
      call CheckUpdate(doUpdate_,time,time0,hasRun,derivedItem=deriveditem,__RC)

      if (doUpdate_) then

         call CalcDerivedField(self%ExtDataState,derivedItem%name,derivedItem%expression, &
              derivedItem%masking,__RC)
...
! ............................................................................
  ...
  subroutine scale_field_(offset, scale_factor, field, rc)
     ...
     integer, optional, intent (inout) :: rc
        ...
        __ASSERT(fieldRank == 2 .or. fieldRank == 3,'Field rank must be 2 or 3')

        if (fieldRank == 2) then
           call ESMF_FieldGet(field, farrayPtr=xy, __RC)

           if (associated(xy)) then
              xy = offset + scale_factor*xy
           end if
        else if (fieldRank == 3) then
           call ESMF_FieldGet(field, farrayPtr=xyz, __RC)
   ...
   ! ............................................................................
     ...
     type (ESMF_Time) function timestamp_(time, template, rc)
        ...
        __ASSERT(len_trim(template) < DATETIME_MAXSTR_,'Timestamp template is greater than Maximum allowed len')

        buff = trim(template)
        buff = ESMF_UtilStringLowerCase(buff, __RC)
        ...
        if (buff == '-'  .or. buff == '--'   .or. buff == '---' .or. &
           ...
           i = scan(buff, 't')
           If (.not.(i > 3)) Then
              __FAIL('ERROR: Time stamp ' // trim(template) // ' uses the fixed format, and must therefore contain a T')
           End If
           ...
           call ESMF_TimeGet(time, yy=yy, mm=mm, dd=dd, h=hs, m=ms, s=ss, __RC)
           ...
           call ESMF_TimeSet(timestamp_, yy=yy, mm=mm, dd=dd, h=hs, m=ms, s=ss, __RC)
        end if
     ...
     subroutine CreateTimeInterval(item,clock,rc)
        ...
        creffTime = ''
        ctInt     = ''
        call ESMF_ClockGet (CLOCK, currTIME=time, startTime=start_time, __RC)
        if (.not.item%hasFileReffTime) then
           ...
           call ESMF_TimeGet(time, yy=iyy, mm=imm, dd=idd,h=ihh, m=imn, s=isc  ,__RC)
           !=======================================================================
           ...
           if (lasttoken.gt.0) then
              ...
              select case(token)
              case("y4")
                 call ESMF_TimeSet(item%reff_time,yy=iyy,mm=1,dd=1,h=0,m=0,s=0,__RC)
                 call ESMF_TimeIntervalSet(item%frequency,startTime=start_time,yy=1,__RC)
              case("m2")
                 call ESMF_TimeSet(item%reff_time,yy=iyy,mm=imm,dd=1,h=0,m=0,s=0,__RC)
                 call ESMF_TimeIntervalSet(item%frequency,startTime=start_time,mm=1,__RC)
              case("d2")
                 call ESMF_TimeSet(item%reff_time,yy=iyy,mm=imm,dd=idd,h=0,m=0,s=0,__RC)
                 call ESMF_TimeIntervalSet(item%frequency,startTime=start_time,d=1,__RC)
              case("h2")
                 call ESMF_TimeSet(item%reff_time,yy=iyy,mm=imm,dd=idd,h=ihh,m=0,s=0,__RC)
                 call ESMF_TimeIntervalSet(item%frequency,startTime=start_time,h=1,__RC)
              case("n2")
                 call ESMF_TimeSet(item%reff_time,yy=iyy,mm=imm,dd=idd,h=ihh,m=imn,s=0,__RC)
                 call ESMF_TimeIntervalSet(item%frequency,startTime=start_time,m=1,__RC)
              end select
           else
              ! couldn't find any tokens so all the data must be on one file
              call ESMF_TimeIntervalSet(item%frequency,__RC)

              ! check if non-token file exists
              inquire(file=trim(item%file),EXIST=found)
              __ASSERT(found,'File ' // trim(item%file) // ' not found')
           end if
        else
           ...
           if (pindex==0) then
              __FAIL( 'ERROR: File template ' // item%file // ' has invalid reference date format')
           end if
     ...
     subroutine GetClimYear(item, rc)
        ...
        if (trim(buffer) == 'n') then
           ...
           if (item%frequency == zero) then
              ...
              lasttoken = index(item%file,'%',back=.true.)
              token = item%file(lasttoken+1:lasttoken+2)
              __ASSERT(token == "m2",'Clim year must be month template "%m2"')
              ! just put a time in so we can evaluate the template to open a file
              nymd = 20000101
              nhms = 0
              call fill_grads_template(file,item%file,nymd=nymd,nhms=nhms,__RC)
           end if
           call MakeMetadata(file,item%pfioCollection_id,metadata,__RC)
           ...
           if (inRange) then
              ...
              __FAIL( 'cyclic keyword was not y, n, or a valid year (0 < year < 3000)')
           end if
        end if
     ...
     subroutine GetLevs(item, time, state, allowExtrap, rc)
        ...
        call ESMF_StateGet(state,trim(temp_name),field,__RC)
        ...
        if (item%frequency == zero) then
           ...
           if ( index(buff,'t')/=0) then
              if (index(buff,'p') == 0) then
                 ftime = timestamp_(time,buff,__RC)
              else
                 ftime = time
              end if
           else
           ...
           call ESMF_TimeGet(fTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=iss,__RC)
        ...
        if (found) then
           ...
           if (allowExtrap .and. (item%cyclic == 'n') ) then
              ...
              if (.not.lfound) then
                 __FAIL( 'From ' // trim(item%file) // ' could not find file with extrapolation')
              end if
           else
              __FAIL('From ' // trim(item%file) // ' could not find time no extrapolation')
           end if
        ...
        if (item%isVector) then
           ...
           __ASSERT(associated(var),"Variable "//TRIM(item%fcomp1)//" not found in file "//TRIM(item%file))
           var => null()
           var=>metadata%get_variable(trim(item%fcomp2))
           __ASSERT(associated(var),"Variable "//TRIM(item%fcomp2)//" not found in file "//TRIM(item%file))
        else
           var=>metadata%get_variable(trim(item%var))
           __ASSERT(associated(var),"Variable "//TRIM(item%var)//" not found in file "//TRIM(item%file))
        end if

        levName = metadata%get_level_name(rc=status)
        __VERIFY(status)
        if (trim(levName) /='') then
           call metadata%get_coordinate_info(levName,coordSize=item%lm,coordUnits=tLevUnits,coords=levFile,__RC)
           ...
           if (item%havePressure) then
              ...
              positive => metadata%get_variable_attribute(levName,'positive',__RC)
     ...
     subroutine UpdateBracketTime(item,cTime,bSide,interpTime,fileTime,file_processed,allowExtrap,rc)
        ...
        if (item%frequency == zero) then
           ...
           call MakeMetadata(file_processed,item%pfioCollection_id,fdata,__RC)
           ...
           if (trim(item%cyclic)=='y') then
              ...
              call ESMF_TimeGet(cTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
              yrOffset = item%climYear - iyr
              call OffsetTimeYear(cTime,yrOffset,fTime,rc)
           else
              yrOffset = 0
              if (item%reff_time > cTime) then
                 __FAIL('Reference time for file ' // trim(item%file) // ' is too late')
              end if
           ...
           readTime = cTime
           call ESMF_TimeGet(fTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
           call MAPL_PackTime(curDate,iyr,imm,idd)
           call MAPL_PackTime(curTime,ihr,imn,isc)
           call fill_grads_template(file_processed,item%file,nymd=curDate,nhms=curTime,__RC)
           ...
           Else if (allowExtrap) then
              ...
              if (lgr%isEnabledFor(DEBUG)) then
                 ...
                 call ESMF_TimeGet(item%reff_time,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
              ...
              ftime = item%reff_time
              call ESMF_TimeGet(item%reff_time,yy=refYear,__RC)
              ...
              call fdata%get_time_info(timeVector=xTseries,__RC)
              ...
              If (LExtrap.or.(LExact.and.RSide)) Then
                 ...
                 call ESMF_TimeGet(cTime,yy=cYearOff,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 iYr = refYear + yrOffset
                 ! Convert year offset to the future value
                 yrOffset = iYr - cYearOff
                 ! Determine the template time
                 call OffsetTimeYear(cTime,yrOffset,newTime,__RC)
                 ftime = item%reff_time
                 n = 0
                 do while (.not.found)
              ...
              ElseIf (RExtrap.or.(RExact.and.RSide)) Then
                 ...
                 call ESMF_TimeGet(cTime,yy=refYear,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 yrOffset = 0
                 fTime = cTime
           ...
           If (buff /= "0" .and. index(buff,"p")==0) Then
              newTime = timestamp_(fTime,item%refresh_template,__RC)
              if (newTime .ne. fTime) Then
                 call ESMF_TimeGet(fTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 call ESMF_TimeGet(newTime,yy=fyr,mm=fmm,dd=fdd,h=fhr,m=fmn,s=fsc,__RC)
              ...
              if (bSide == "R") then
                 ...
                 do while ((status==ESMF_SUCCESS).and.(.not.found))
                    ...
                    if (trim(item%cyclic)=='y') then
                       ...
                       call ESMF_TimeGet(newTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                       if (oldyear/=iyr) then
                          call ESMF_TimeSet(newTime,yy=oldyear,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                          yrOffset = yrOffset - 1
        ...
        if (lgr%isEnabledFor(DEBUG)) then
           ...
           call ESMF_TimeGet(fileTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
           call lgr%debug('            ==> (%a1) Record time   : %i0.4~-%i0.2~-%i0.2 %i0.2~:%i0.2~:%i0.2', bside, iYr, iMm, iDd, iHr, iMn, iSc)
           call ESMF_TimeGet(interpTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
     ...
     subroutine swapBracketInformation(item,rc)
        ...
        if (item%vartype == MAPL_FieldItem) then
           ...
           call ESMF_FieldGet(item%modelGridFields%v1_finterp1, dimCount=fieldRank,__RC)
           if (fieldRank == 2) then
              call ESMF_FieldGet(item%modelGridFields%v1_finterp1, localDE=0, farrayPtr=var2d_prev, __RC)
              call ESMF_FieldGet(item%modelGridFields%v1_finterp2, localDE=0, farrayPtr=var2d_next, __RC)
              var2d_prev=var2d_next
           else if (fieldRank == 3) then
              call ESMF_FieldGet(item%modelGridFields%v1_finterp1, localDE=0, farrayPtr=var3d_prev, __RC)
              call ESMF_FieldGet(item%modelGridFields%v1_finterp2, localDE=0, farrayPtr=var3d_next, __RC)
              var3d_prev=var3d_next
           endif

        else if (item%vartype == MAPL_BundleItem) then

           call ESMF_FieldBundleGet(item%binterp2, fieldCount = fieldCount, __RC)
           allocate(names(fieldCount),__STAT__)
           call ESMF_FieldBundleGet(item%binterp2, fieldNameList = Names, __RC)
           do j = 1,fieldCount
              call ESMF_FieldBundleGet(item%binterp1, names(j), field=field1, __RC)
              call ESMF_FieldBundleGet(item%binterp2, names(j), field=field2, __RC)
              call ESMF_FieldGet(field1, dimCount=fieldRank, __RC)
              if (fieldRank == 2) then
                 call ESMF_FieldGet(field1, localDE=0, farrayPtr=var2d_prev, __RC)
                 call ESMF_FieldGet(field2, localDE=0, farrayPtr=var2d_next, __RC)
                 var2d_prev=var2d_next
              else if (fieldRank == 3) then
                 call ESMF_FieldGet(field1, localDE=0, farrayPtr=var3d_prev, __RC)
                 call ESMF_FieldGet(field2, localDE=0, farrayPtr=var3d_next, __RC)
                 var3d_prev=var3d_next
              endif
           enddo
           ...
           if (fieldRank == 2) then
              ...
              call ESMF_FieldGet(item%modelGridFields%v1_finterp2, localDE=0, farrayPtr=var2d_next, __RC)
              var2d_prev=var2d_next
              call ESMF_FieldGet(item%modelGridFields%v2_finterp1, localDE=0, farrayPtr=var2d_prev, __RC)
              call ESMF_FieldGet(item%modelGridFields%v2_finterp2, localDE=0, farrayPtr=var2d_next, __RC)
              var2d_prev=var2d_next
           else if (fieldRank == 3) then
              call ESMF_FieldGet(item%modelGridFields%v1_finterp1, localDE=0, farrayPtr=var3d_prev, __RC)
              call ESMF_FieldGet(item%modelGridFields%v1_finterp2, localDE=0, farrayPtr=var3d_next, __RC)
              var3d_prev=var3d_next
              call ESMF_FieldGet(item%modelGridFields%v2_finterp1, localDE=0, farrayPtr=var3d_prev, __RC)
              call ESMF_FieldGet(item%modelGridFields%v2_finterp2, localDE=0, farrayPtr=var3d_next, __RC)
              var3d_prev=var3d_next
           endif
     ...
     subroutine makeMetadata(file,collection_id,metadata,rc)
        ...
        metadata => collection%find(file, __RC)
     ...
     subroutine GetTimesOnFile(cfio,tSeries,rc)
        ...
        call getDateTimeVec(cfio%fid,begDate,begTime,tSeriesInt,__RC)
        ...
        do i=1,cfio%tSteps
           ...
           call ESMF_TimeSet(tSeries(i), yy=iyr, mm=imm, dd=idd,  h=ihr,  m=imn, s=isc,__RC)
        enddo
     ...
     subroutine OffsetTimeYear(inTime,yrOffset,outTime,rc)
        ...
        __UNUSED_DUMMY(Iam)
        call ESMF_TimeGet(inTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
        ...
        call ESMF_TimeSet(outTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
     ...
     subroutine GetBracketTimeOnSingleFile(fdata,tSeries,cTime,bSide,UniFileClim,interpTime,fileTime,tindex,allowExtrap,climyear,rc)
        ...
        yrOffset=0
        nsteps = size(tSeries)
        call ESMF_TimeGet(cTime,yy=targYear,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
        ...
        if (lgr%isEnabledFor(DEBUG)) then
           ...
           call ESMF_TimeGet(tSeries(1),yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
           call lgr%debug('                  ==> File start    : %i0.4~-%i0.2~-%i0.2 %i0.2~:%i0.2~:%i0.2', iyr, imm, idd, ihr, imn, isc)
           call ESMF_TimeGet(tSeries(nsteps),yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
        ...
        if (uniFileClim) then
           ...
           call ESMF_TimeSet(climTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)

           tsteps=0
           foundYear = .false.
           do i=1,nsteps

              call ESMF_TimeGet(tseries(i),yy=iyr,__RC)
           ...
           if (bSide == "L") then
              if ( (climTime < tSeriesC(1)) ) then
                 ...
                 tindex = tSteps
                 call ESMF_TimeGet(tSeriesC(tSteps),yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 call ESMF_TimeGet(cTime,yy=curYear,__RC)
                 iyr = curYear - 1
                 call ESMF_TimeSet(interpTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 found = .true.
              else
                 do i=tSteps,1,-1
                    if (climTime >= tSeriesC(i)) then
                       ...
                       if (UniFileClim) then
                          call ESMF_TimeGet(cTime,yy=curYear,__RC)
                          call ESMF_TimeGet(fileTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                          call ESMF_TimeSet(interpTime,yy=curYear,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                       else
              ...
              if ( (climTime >= tSeriesC(tSteps)) ) then
                 fileTime = tSeriesC(1)
                 tindex = 1
                 call ESMF_TimeGet(tSeriesC(1),yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 call ESMF_TimeGet(cTime,yy=curYear,__RC)
                 iyr = curYear + 1
                 call ESMF_TimeSet(interpTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
                 found = .true.
     ...
     subroutine GetBracketTimeOnFile(fdata,tSeries,cTime,bSide,UniFileClim,interpTime,fileTime,tindex,yrOffsetInt,rc)
        ...
        if (lgr%isEnabledFor(DEBUG)) then
           call ESMF_TimeGet(cLimTime,yy=iyr,mm=imm,dd=idd,h=ihr,m=imn,s=isc,__RC)
 ...
 subroutine CalcDerivedField(state,exportName,exportExpr,masking,rc)
     ...
     if (masking) then
        call MAPL_ExtDataEvaluateMask(state,exportName,exportExpr,__RC)
     else
        call ESMF_StateGet(state,exportName,field,__RC)
        call MAPL_StateEval(state,exportExpr,field,__RC)
     end if
     __RETURN(ESMF_SUCCESS)
  ...
  subroutine MAPL_ExtDataInterpField(item,time,field,vector_comp,rc)
     ...
     call ESMF_FieldGet(FIELD, dimCount=fieldRank,name=name,__RC)

     if (lgr%isEnabledFor(DEBUG)) then
        call ESMF_TimeGet(item%interp_time1,yy=yr,mm=mm,dd=dd,h=hr,m=mn,s=sc,__RC)
        call MAPL_PackTime(nhms1,hr,mn,sc)
        call MAPL_PackTime(nymd1,yr,mm,dd)
        if (item%doInterpolate) then
           if (alpha .gt. 0.0) then
              call ESMF_TimeGet(item%interp_time2,yy=yr,mm=mm,dd=dd,h=hr,m=mn,s=sc,__RC)
     ...
     if (fieldRank == 2) then
        ...
        if (item%vartype == MAPL_FieldItem) then
           ...
           call ESMFL_BundleGetPointerToData(item%binterp1,name,var2d_prev,__RC)
           call ESMFL_BundleGetPointerToData(item%binterp2,name,var2d_next,__RC)
        else if (item%vartype == MAPL_ExtDataVectorItem) then
           __ASSERT(present(vector_comp),'Vector comp must be present when performing vector interpolation')
        ...
        call ESMF_FieldGet(field, localDE=0, farrayPtr=var2d, __RC)
        ...
        if (item%vartype == MAPL_FieldItem) then
           ...
           call ESMFL_BundleGetPointerToData(item%binterp1,name,var3d_prev,__RC)
           call ESMFL_BundleGetPointerToData(item%binterp2,name,var3d_next,__RC)
        else if (item%vartype == MAPL_ExtDataVectorItem) then
           __ASSERT(present(vector_comp),'Vector comp must be present when performing vector interpolation')
           if (vector_comp == 1) then
              call ESMF_FieldGet(item%modelGridFields%v1_finterp1, localDE=0, farrayPtr=var3d_prev, __RC)
        ...
        call ESMF_FieldGet(field, localDE=0, farrayPtr=var3d, __RC)
        ! only interpolate if we have to
        if (time == item%interp_time1 .or. item%doInterpolate .eqv. .false.) then
           var3d = var3d_prev
        else if (time == item%interp_time2) then
  ...
  subroutine GetMaskName(FuncStr,Var,Needed,rc)
     ...
     functionname = ESMF_UtilStringLowerCase(functionname, __RC)
     ...
     __ASSERT(found,'Var ' // trim(tmpstring1) // ' not found')
     needed(ivar) = .true.
     ...
     if (twovar) then
        ...
        __ASSERT(found,'Secound Var ' // trim(tmpstring2) // ' not found')
        needed(ivar) = .true.
     end if
     __RETURN(ESMF_SUCCESS)
  ...
  subroutine MAPL_ExtDataEvaluateMask(state,exportName,exportExpr,rc)
    ...
    i1 = index(exportExpr,"(")
    __ASSERT(i1 > 0,'Expected "(" in expression: ' // trim(exportExpr))
    ...
    if (trim(functionname) == "regionmask") then
       ...
       if (rank == 2) then
          call MAPL_GetPointer(state,rvar2d,vartomask,__RC)
          call MAPL_GetPointer(state,var2d,exportName,__RC)
       else if (rank == 3) then
          call MAPL_GetPointer(state,rvar3d,vartomask,__RC)
          call MAPL_GetPointer(state,var3d,exportName,__RC)
       else
...
! ----------
 ...
 END SUBROUTINE MAPL_ExtDataExtractIntegers
  ...
  function MAPL_ExtDataGetFStartTime(item,fname, rc) result(stime)
     ...
     call MakeMetadata(fname,item%pfiocollection_id,metadata,__RC)
     call Metadata%get_time_info(startyear=iyr,startmonth=imm,startday=idd,starthour=ihr,startmin=imn,startsec=isc,rc=status)
     __VERIFY(status)
     call ESMF_TimeSet(sTime, yy=iyr, mm=imm, dd=idd,  h=ihr,  m=imn, s=isc, __RC)
     nullify(metadata)
  ...
  subroutine CheckUpdate(doUpdate,updateTime,currTime,hasRun,primaryItem,derivedItem,rc)
     ...
     if (present(primaryItem)) then
        ...
        if (primaryItem%AlarmIsEnabled) then
           doUpdate = primaryItem%update_alarm%is_ringing(currTime,__RC)
           ...
           if (primaryItem%refresh_template == "0") then
              ...
              if (.not. associated(PrimaryItem%refresh_time)) then
                doUpdate = .false.
                 ...
                 refresh_time = timestamp_(time, PrimaryItem%refresh_template, __RC)
        ...
        if (DerivedItem%AlarmIsEnabled) then
           doUpdate = derivedItem%update_alarm%is_ringing(currTime,__RC)
           updateTime = currTime
        else
           if (derivedItem%refresh_template == "0") then
              ...
              if (.not. associated(derivedItem%refresh_time)) then
                doUpdate = .false.
                 ...
                 refresh_time = timestamp_(time, derivedItem%refresh_template, __RC)
  ...
  function MAPL_ExtDataGridChangeLev(Grid,CF,lm,rc) result(NewGrid)
     ...
     call ESMF_GridGet(grid,name=gName,__RC)
     call ESMF_ConfigGetAttribute(CF, value = NX, Label="NX:", __RC)
     call ESMF_ConfigGetAttribute(CF, value = NY, Label="NY:", __RC)
  ...
  subroutine MAPL_ExtDataFlipVertical(item,filec,rc)
      ...
      if (item%isVector) then
         ...
         if (item%do_Fill .or. item%do_VertInterp) then
            call MAPL_ExtDataGetBracket(item,filec,field=Field1,vcomp=1,getRL=.true.,__RC)
            call MAPL_ExtDataGetBracket(item,filec,field=Field2,vcomp=2,getRL=.true.,__RC)
         else
         ...
         if (item%do_Fill .or. item%do_VertInterp) then
            call MAPL_ExtDataGetBracket(item,filec,field=Field,getRL=.true.,__RC)
         else
  ...
  subroutine createFileLevBracket(item,cf,rc)
     ...
     if (item%vartype==MAPL_FieldItem) then
        call ESMF_FieldGet(item%modelGridFields%v1_finterp1,grid=grid,__RC)
        newGrid = MAPL_ExtDataGridChangeLev(grid,cf,item%lm,__RC)
        call ESMF_FieldGet(item%modelGridFields%v1_finterp1,grid=grid,__RC)
        item%modelGridFields%v1_faux1 = MAPL_FieldCreate(item%modelGridFields%v1_finterp1,newGrid,lm=item%lm,newName=trim(item%var),__RC)
        item%modelGridFields%v1_faux2 = MAPL_FieldCreate(item%modelGridFields%v1_finterp2,newGrid,lm=item%lm,newName=trim(item%var),__RC)
     else if (item%vartype==MAPL_ExtDataVectorItem) then
        call ESMF_FieldGet(item%modelGridFields%v1_finterp1,grid=grid,__RC)
        newGrid = MAPL_ExtDataGridChangeLev(grid,cf,item%lm,__RC)
        call ESMF_FieldGet(item%modelGridFields%v1_finterp1,grid=grid,__RC)
        item%modelGridFields%v1_faux1 = MAPL_FieldCreate(item%modelGridFields%v1_finterp1,newGrid,lm=item%lm,newName=trim(item%fcomp1),__RC)
        item%modelGridFields%v1_faux2 = MAPL_FieldCreate(item%modelGridFields%v1_finterp2,newGrid,lm=item%lm,newName=trim(item%fcomp1),__RC)
        call ESMF_FieldGet(item%modelGridFields%v1_finterp1,grid=grid,__RC)
        item%modelGridFields%v2_faux1 = MAPL_FieldCreate(item%modelGridFields%v2_finterp1,newGrid,lm=item%lm,newName=trim(item%fcomp2),__RC)
        item%modelGridFields%v2_faux2 = MAPL_FieldCreate(item%modelGridFields%v2_finterp2,newGrid,lm=item%lm,newName=trim(item%fcomp2),__RC)
     end if
     __RETURN(__SUCCESS)
