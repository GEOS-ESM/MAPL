...
! --------------------------------------------------
    ...
    call ESMF_ClockGet ( clock,     StopTime=EndTime,   _RC ) ! Added for GCHP adjoint
...
! -------------------------------------------------
    ...
    ! NOTE: prior versions of GCHP used a MAPL version with default=0
...
! ----------------------------------------
       if( list(n)%end_date.ne.MAPL_UndefInt .and. list(n)%end_time.ne.MAPL_UndefInt ) then
           ...
           !   call ESMF_TimeIntervalSet( OneSecond, S = 1, _RC )
           !   ringTime = startTime - oneSecond
           !   list(n)%end_alarm = ESMF_AlarmCreate( clock=clock, RingTime=ringTime, sticky=.false., enabled=.false., _RC)
           !else
           !   call ESMF_TimeIntervalSet( OneSecond, S = 1, _RC )
          ...
          elseif (list(n)%sampler_type == 'station') then
             ...
             if (trim(list(n)%sampler_type) == 'swath' ) then
                ...
                if (trim(list(n)%output_grid_label)/='') then
                   pgrid => IntState%output_grids%at(trim(list(n)%output_grid_label))
                   ! Pass posDown=isPosDown for GCHP
                   call list(n)%mGriddedIO%CreateFileMetaData(list(n)%items,list(n)%bundle,list(n)%timeInfo,ogrid=pgrid,vdata=list(n)%vdata,global_attributes=global_attributes,posDown=isPosDown,_RC)
                else
                   ! Pass posDown=isPosDown for GCHP
                   call list(n)%mGriddedIO%CreateFileMetaData(list(n)%items,list(n)%bundle,list(n)%timeInfo,vdata=list(n)%vdata,global_attributes=global_attributes,posDown=isPosDown,_RC)
                end if
