...
contains
  ...
  function new_StationSampler_readfile (bundle, filename, nskip_line, GENSTATE, rc) result(sampler)
    ...
    if ( MAPL_AM_I_ROOT() ) then
       open(newunit=unit, file=trim(filename), form='formatted', &
            access='sequential', status='old', __IOSTAT)
       ios=0
       nstation=0
       nskip=0
       ...
       call count_substring(line, ',', ncount, __RC)
       con1= (ncount>=2 .AND. ncount<=4).OR.(ncount==0)
       __ASSERT(con1, 'string sequence in Aeronet file not supported')
       if (ncount==0) then
          seq='AFFFA'
       ...
       elseif (ncount==4) then
          ...
          if (con1) then
             ...
             if (con2) then
                ...
                __ASSERT(.false., 'string sequence in Aeronet file not supported')
             end if
          end if
       end if
       ...
       allocate(sampler%station_id(nstation), __STAT)
       allocate(sampler%station_name(nstation), __STAT)
       allocate(sampler%station_fullname(nstation), __STAT)
       allocate(sampler%lons(nstation), __STAT)
       allocate(sampler%lats(nstation), __STAT)
       allocate(sampler%elevs(nstation), __STAT)
    ...
    call MAPL_CommsBcast(vm, DATA=sampler%nstation, N=1, ROOT=MAPL_Root, __RC)
    ...
    ! -- root
    sampler%LSF   = LocStreamFactory(sampler%lons, sampler%lats, __RC)
    sampler%LS_rt = sampler%LSF%create_locstream(__RC)

    ! -- chunk
    sampler%LSF = LocStreamFactory(lons_chunk,lats_chunk,__RC)
    sampler%LS_chunk = sampler%LSF%create_locstream_on_proc(__RC)

    ! -- distributed
    call ESMF_FieldBundleGet(bundle,grid=grid,__RC)
    sampler%LS_ds = sampler%LSF%create_locstream_on_proc(grid=grid,__RC)
  ...
  subroutine add_metadata_route_handle (this,items,bundle,timeInfo,vdata,rc)
    ...
    if (this%vdata%regrid_type == VERTICAL_METHOD_ETA2LEV) then
       call this%vdata%get_interpolating_variable(this%bundle,__RC)
    endif

    call timeInfo%add_time_to_metadata(this%metadata,__RC) ! specify time in fmd
    ...
    pt1=0.0
    pt2=0.0
    call ESMF_FieldRedistStore(src_field,chunk_field,this%RH,__RC)
  ...
  subroutine append_file(this,current_time,rc)
    ...
    !
    rtimes = this%compute_time_for_current(current_time,__RC) ! rtimes: seconds since opening file
    if (mapl_am_i_root()) then
    ...
    field_ds_2d    = ESMF_FieldCreate (this%LS_ds, name='field_2d_ds', typekind=ESMF_TYPEKIND_R4, __RC)
    field_chunk_2d = ESMF_FieldCreate (this%LS_chunk, name='field_2d_chunk', typekind=ESMF_TYPEKIND_R4, __RC)
    new_src_field  = ESMF_FieldCreate (grid, name='new_src_field', typekind=ESMF_TYPEKIND_R4, &
         gridToFieldMap=[2,3],ungriddedLBound=[1],ungriddedUBound=[lm],__RC)
    ...
    call ESMF_FieldGet(field_ds_2d,   localDE=0, farrayptr=p_ds_2d,    __RC)
    call ESMF_FieldGet(field_chunk_2d,localDE=0, farrayPtr=p_chunk_2d, __RC)
    call ESMF_FieldGet(new_src_field, localDE=0, farrayPtr=p_src_3d,   __RC)
    call ESMF_FieldGet(new_dst_field, localDE=0, farrayPtr=p_dst_3d,   __RC)
    call ESMF_FieldGet(field_chunk_3d,localDE=0, farrayPtr=p_chunk_3d, __RC)
    ...
    do while (iter /= this%items%end())
       ...
       if (item%itemType == ItemTypeScalar) then
          ...
          select case (rank)
          case(2)
             call ESMF_FieldGet(src_field,localDE=0,farrayptr=p_src_2d,__RC)
             call ESMF_FieldRegrid (src_field, field_ds_2d, this%regridder%route_handle, __RC)
             call ESMF_FieldRedist (field_ds_2d, field_chunk_2d, this%RH, __RC )
             ...
             if (mapl_am_i_root()) then
                call this%formatter%put_var(trim(item%xname),p_rt_2d,&
                     start=[1,this%obs_written],count=[this%nstation,1],__RC)
             end if
          ...
          case(3)
             ...
             !
             call ESMF_FieldGet(src_field,localDE=0,farrayptr=qin_3d,__RC)
             ...
             call ESMF_FieldRegrid (new_src_field, new_dst_field, this%regridder%route_handle, __RC)
             call MAPL_TimerOff(this%GENSTATE,"3d_regrid")

             call MPI_Barrier(mpic,ierr)
             __VERIFY(ierr)
             call MAPL_TimerOn(this%GENSTATE,"FieldRedist")
             call ESMF_FieldRedist (new_dst_field, field_chunk_3d, this%RH, __RC)
             call MPI_Barrier(mpic,ierr)
             __VERIFY(ierr)
             ...
             if (mapl_am_i_root()) then
                ...
                call this%formatter%put_var(trim(item%xname),p_rt_3d,&
                     start=[1,1,this%obs_written],count=[nz,nx,1],__RC)
          ...
          __FAIL ('ItemType vector not supported')
       endif
    ...
    call ESMF_FieldDestroy(field_ds_2d,    noGarbage=.true., __RC)
    call ESMF_FieldDestroy(field_chunk_2d, noGarbage=.true., __RC)
    call ESMF_FieldDestroy(field_chunk_3d, noGarbage=.true., __RC)
    call ESMF_FieldDestroy(new_dst_field,  noGarbage=.true., __RC)
    call ESMF_FieldDestroy(new_src_field,  noGarbage=.true., __RC)
  ...
  subroutine create_file_handle(this,filename,rc)
    ...
    v = this%time_info%define_time_variable(__RC)
    call this%metadata%modify_variable('time',v,__RC)
    this%obs_written = 0

    if (.not. mapl_am_I_root()) then
       __RETURN(__SUCCESS)
    end if
    call this%formatter%create(trim(filename),__RC)
    call this%formatter%write(this%metadata,__RC)
    call this%formatter%put_var('longitude',this%lons,__RC)
    call this%formatter%put_var('latitude',this%lats,__RC)
    call this%formatter%put_var('station_id',this%station_id,__RC)
    call this%formatter%put_var('station_name',this%station_name,__RC)
  ...
  subroutine close_file_handle(this,rc)
    ...
    if (trim(this%ofile) /= '') then
       if (mapl_am_i_root()) then
          call this%formatter%close(__RC)
       end if
    end if
    __RETURN(__SUCCESS)
  ...
  function compute_time_for_current(this,current_time,rc) result(rtimes)
    ...
    call this%get_file_start_time(file_start_time,tunit,__RC)
    tint = current_time-file_start_time
    select case(trim(tunit))
    case ('days')
       call ESMF_TimeIntervalGet(tint,d_r8=rtimes(1),__RC)
    case ('hours')
       call ESMF_TimeIntervalGet(tint,h_r8=rtimes(1),__RC)
    case ('minutes')
       call ESMF_TimeIntervalGet(tint,m_r8=rtimes(1),__RC)
    case default
       __FAIL('illegal value for tunit: '//trim(tunit))
    end select
    __RETURN(__SUCCESS)
  ...
  subroutine get_file_start_time(this,start_time,time_units,rc)
    ...
    call ESMF_TimeSet(start_time,yy=year,mm=month,dd=day,h=hour,m=min,s=sec,__RC)
    __RETURN(__SUCCESS)
  ...
  subroutine CSV_read_line_with_CH_I_R(line, name, lon, lat, rc)
    ...
    i=index(line, ',')
    j=index(line(i+1:), ',')
    __ASSERT (i>0, 'not CSV format')
    __ASSERT (j>0, 'CSV format: find only 1 comma, should be > 1')
    j=i+j

    read(line(1:i-1), '(a100)', iostat=ios)  name
    __ASSERT (ios==0, 'read error')
