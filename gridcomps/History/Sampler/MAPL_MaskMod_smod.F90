...
module function MaskSampler_from_config(config,string,clock,GENSTATE,rc) result(mask)
  ...
  call ESMF_ConfigGetAttribute(config, value=mask%grid_file_name,label=trim(string)//'obs_files:',    default="",  __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%index_name_x,  label=trim(string)//'index_name_x:', default="x", __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%index_name_y,  label=trim(string)//'index_name_y:', default="y", __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%var_name_x,    label=trim(string)//'var_name_x:',   default="x", __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%var_name_y,    label=trim(string)//'var_name_y:',   default="y", __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%var_name_proj, label=trim(string)//'var_name_proj:',default="",  __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%att_name_proj, label=trim(string)//'att_name_proj:',default="",  __RC)
  call ESMF_ConfigGetAttribute(config, value=mask%thin_factor,   label=trim(string)//'thin_factor:',  default=-1,  __RC)
  call ESMF_ConfigGetAttribute(config, value=output_leading_dim, label=trim(string)//'output_leading_dim:',default='lev',  __RC)
...
module subroutine initialize(this,duration,frequency,items,bundle,timeInfo,vdata,global_attributes,reinitialize,rc)
   ...
   __ASSERT(present(global_attributes), 'PFIO needs global_attributes')
...
!   if (this%vdata%regrid_type == VERTICAL_METHOD_ETA2LEV) call this%vdata%get_interpolating_variable(this%bundle,__RC)
   ...
   this%obs_written = 0
   call this%create_Geosat_grid_find_mask(__RC)
   call this%create_metadata(global_attributes,__RC)
   n1 = MAPL_nsecf( duration )
   n2 = MAPL_nsecf( frequency )
   __ASSERT (n2>0, "list%frequency ==0, fail!")
   this%tmax =  n1/n2
   ...
   if (this%use_pfio) then
      ...
      allocate ( this%var2d(ic_2d), __STAT )
      allocate ( this%var3d(ic_3d), __STAT )

      do j=1, ic_2d
         if (mapl_am_i_root()) then
            allocate ( this%var2d(j)%array_x(this%npt_mask_tot), __STAT )
         else
            allocate ( this%var2d(j)%array_x(0), __STAT )
         end if
      end do
      do j=1, ic_3d
         if (mapl_am_i_root()) then
            if (this%write_lev_first) then
               allocate ( this%var3d(j)%array_zx(this%vdata%lm, this%npt_mask_tot), __STAT )
            else
               allocate ( this%var3d(j)%array_xz(this%npt_mask_tot, this%vdata%lm), __STAT )
            end if
         else
            if (this%write_lev_first) then
               allocate ( this%var3d(j)%array_zx(0,0), __STAT )
            else
               allocate ( this%var3d(j)%array_xz(0,0), __STAT )
            end if
         end if
      end do
   end if
...
module subroutine  create_metadata(this,global_attributes,rc)
    ...
    call this%timeinfo%add_time_to_metadata(this%metadata,__RC)
    ...
    call this%vdata%append_vertical_metadata(this%metadata,this%bundle,__RC) ! specify lev in fmd
    ...
    !
    call ESMF_FieldBundleGet(this%bundle, fieldCount=fieldCount, __RC)
    allocate (fieldNameList(fieldCount), __STAT)
    call ESMF_FieldBundleGet(this%bundle, fieldNameList=fieldNameList, __RC)
    do i=1, fieldCount
       var_name=trim(fieldNameList(i))
       call ESMF_FieldBundleGet(this%bundle,var_name,field=field,__RC)
       call ESMF_FieldGet(field,rank=field_rank,__RC)
       ...
       call this%metadata%add_variable(trim(var_name),v,__RC)
    end do
    deallocate (fieldNameList, __STAT)
    ...
    do while(s_iter /= global_attributes%end())
       ...
       call this%metadata%add_attribute(attr_name,attr_val,__RC)
       call s_iter%next()
    enddo
    ...
    __RETURN(__SUCCESS)
     ...
     module subroutine create_Geosat_grid_find_mask(this, rc)
       ...
       if (mapl_am_i_root()) then
          ...
          call get_ncfile_dimension(fn,nlon=n1,nlat=n2,key_lon=key_x,key_lat=key_y,__RC)
          allocate (x(n1), y(n2), __STAT)
       ...
       call MAPL_CommsBcast(vm, DATA=n1, N=1, ROOT=MAPL_Root, __RC)
       call MAPL_CommsBcast(vm, DATA=n2, N=1, ROOT=MAPL_Root, __RC)
       if ( .NOT. mapl_am_i_root() )  allocate (x(n1), y(n2), __STAT)
       call MAPL_CommsBcast(vm, DATA=lam_sat, N=1, ROOT=MAPL_Root, __RC)
       call MAPL_CommsBcast(vm, DATA=x, N=n1, ROOT=MAPL_Root, __RC)
       call MAPL_CommsBcast(vm, DATA=y, N=n2, ROOT=MAPL_Root, __RC)
       ...
       __ASSERT ( xdim_red * ydim_red > M, 'mask reduced points after thin_factor is less than Nproc!')
       ...
       allocate (lons_chunk(nx2), lats_chunk(nx2), __STAT)
       ...
       call ESMF_VMAllFullReduce(vm, sendData=arr, recvData=nx, &
            count=1, reduceflag=ESMF_REDUCE_SUM, __RC)


       ! gatherV for lons/lats
       if (mapl_am_i_root()) then
          allocate(lons(nx),lats(nx),__STAT)
       else
          allocate(lons(0),lats(0),__STAT)
       endif

       allocate( this%recvcounts(petcount), this%displs(petcount), __STAT )
       allocate( recvcounts_loc(petcount), displs_loc(petcount), __STAT )
       recvcounts_loc(:)=1
       displs_loc(1)=0
       do i=2, petcount
       ...
       deallocate (this%recvcounts, this%displs, __STAT)
       deallocate (recvcounts_loc, displs_loc, __STAT)
       deallocate (x, y, __STAT)
       ...
       ! -- root
       locstream_factory = LocStreamFactory(lons,lats,__RC)
       LS_rt = locstream_factory%create_locstream(__RC)

       ! -- proc
       locstream_factory = LocStreamFactory(lons_chunk,lats_chunk,__RC)
       LS_chunk = locstream_factory%create_locstream_on_proc(__RC)

       ! -- distributed with background grid
       call ESMF_FieldBundleGet(this%bundle,grid=grid,__RC)
       LS_ds = locstream_factory%create_locstream_on_proc(grid=grid,__RC)

       fieldA = ESMF_FieldCreate (LS_chunk, name='A', typekind=ESMF_TYPEKIND_R8, __RC)
       fieldB = ESMF_FieldCreate (LS_ds, name='B', typekind=ESMF_TYPEKIND_R8, __RC)
       call ESMF_FieldGet( fieldA, localDE=0, farrayPtr=ptA)
       call ESMF_FieldGet( fieldB, localDE=0, farrayPtr=ptB)

       ptA(:) = lons_chunk(:)
       call ESMF_FieldRedistStore (fieldA, fieldB, RH, __RC)
       call MPI_Barrier(mpic,ierr)
       __VERIFY(ierr)
       call ESMF_FieldRedist      (fieldA, fieldB, RH, __RC)
       lons_ds = ptB
       ...
       call ESMF_FieldDestroy(fieldA,nogarbage=.true.,__RC)
       call ESMF_FieldDestroy(fieldB,nogarbage=.true.,__RC)
       call ESMF_FieldRedistRelease(RH, noGarbage=.true., __RC)
       allocate ( II(nx), JJ(nx), __STAT )
       call MAPL_GetHorzIJIndex(nx,II,JJ,lonR8=obs_lons,latR8=obs_lats,grid=grid,__RC)
       ...
       fieldI4 = ESMF_FieldCreate (grid, ESMF_TYPEKIND_I4, &
            ...
            totalUwidth=[useableHalo_width,useableHalo_width], __RC)
       call ESMF_FieldGetBounds (fieldI4, &
            ...
            __RC)
       call ESMF_FieldGet (fieldI4, farrayPtr=farrayPtr,  __RC)
       farrayPtr(:,:) = 0
       do i=1, nx
       ...
       call ESMF_FieldHaloStore (fieldI4, routehandle=RH_halo, __RC)
       call ESMF_FieldHalo (fieldI4, routehandle=RH_halo, __RC)
...
!       write(6,*) 'eLB(2), eUB(2)', eLB(2), eUB(2)
       ...
       call ESMF_GridGetCoord (grid, coordDim=1, localDE=0, &
            staggerloc=ESMF_STAGGERLOC_CENTER, farrayPtr=lons_ptr, __RC)
       call ESMF_GridGetCoord (grid, coordDim=2, localDE=0, &
            staggerloc=ESMF_STAGGERLOC_CENTER, farrayPtr=lats_ptr, __RC)
       deallocate (lons, lats, __STAT)
       allocate (lons(this%npt_mask), lats(this%npt_mask), __STAT)
       ...
       iroot=0
       if (mapl_am_i_root()) then
          allocate (this%lons(this%npt_mask_tot), this%lats(this%npt_mask_tot), __STAT)
       else
...
!       __VERIFY(ierr)
       ...
       if (mapl_am_i_root()) then
          ...
          allocate (this%lons_deg(this%npt_mask_tot), this%lats_deg(this%npt_mask_tot), __STAT)
          this%lons_deg = this%lons * MAPL_RADIANS_TO_DEGREES
          this%lats_deg = this%lats * MAPL_RADIANS_TO_DEGREES
       else
          allocate (this%lons_deg(0), this%lats_deg(0), __STAT)
       end if
...
!
!
!       call MAPL_CommsBcast(vm, DATA=, N=1, ROOT=MAPL_Root, __RC)
...
!       call MPI_Barrier(mpic,ierr)
 ...
 module subroutine regrid_append_file(this,current_time,filename,oClients,rc)
    ...
    nx = this%npt_mask
    nz = this%vdata%lm
    allocate(p_dst_2d (nx), __STAT)
    allocate(p_dst_3d (nx * nz), __STAT)
    if (mapl_am_i_root()) then
       allocate ( p_dst_2d_full (this%npt_mask_tot), __STAT )
       allocate ( p_dst_3d_full (this%npt_mask_tot * nz), __STAT )
    else
       allocate ( p_dst_2d_full (0), __STAT )
       allocate ( p_dst_3d_full (0), __STAT )
    end if
    allocate( recvcounts_3d(petcount), displs_3d(petcount), __STAT )
    ...
    !
    allocate( rtimes(1), __STAT )
    rtimes(1) = this%compute_time_for_current(current_time,__RC) ! rtimes: seconds since opening file
    if (mapl_am_i_root()) then
      allocate( rtime(1), __STAT )
      rtime(1) = rtimes(1)
   else
      allocate( rtime(0), __STAT )
   endif
   if (this%use_pfio) then
       ...
       call this%stage2DLatLon(trim(filename),oClients=oClients,__RC)
    else
       if (mapl_am_i_root()) then
          call this%formatter%put_var('time',rtimes(1:1),&
               start=[this%obs_written],count=[1],__RC)
          call this%formatter%put_var('longitude',this%lons_deg,__RC)
          call this%formatter%put_var('latitude',this%lats_deg,__RC)
       end if
    end if
    ...
    do while (iter /= this%items%end())
       ...
       if (item%itemType == ItemTypeScalar) then
          ...
          if (rank==2) then
             ...
             if (this%use_pfio) then
                ...
                if (mapl_am_i_root()) then
                   call this%formatter%put_var(item%xname,p_dst_2d_full,&
                        start=[1,this%obs_written],count=[this%npt_mask_tot,1],__RC)
                end if
             end if
             ...
             __ASSERT (this%vdata%lm == (ub(1)-lb(1)+1), 'vertical level is different from CS grid')
             m=0
             do j=1, nx
                ix = this%index_mask(1,j)
                iy = this%index_mask(2,j)
             ...
             if (this%use_pfio) then
                ...
                if (mapl_am_i_root()) then
                   allocate(arr(nz, this%npt_mask_tot), __STAT)
                   arr=reshape(p_dst_3d_full,[nz,this%npt_mask_tot],order=[1,2])
                   call this%formatter%put_var(item%xname,arr,&
                        start=[1,1,this%obs_written],count=[nz,this%npt_mask_tot,1],__RC)
                   !note:     lev,location,time
                   deallocate(arr, __STAT)
                end if
             end if
             call MAPL_TimerOff(this%GENSTATE,"put3D")

          else
             __FAIL('grid2LS regridder: rank > 3 not implemented')
          end if
       end if
  ...
  module function compute_time_for_current(this,current_time,rc) result(rtime)
    ...
    var => this%metadata%get_variable('time',__RC)
    ...
    class default
       __FAIL("Time unit must be character")
    end select
    allocate (  esmf_time_1d(1), rtime_1d(1), __STAT )
    esmf_time_1d(1)= current_time
    call time_ESMF_to_real ( rtime_1d, esmf_time_1d, datetime_units, __RC )
    rtime =  rtime_1d(1)
    ...
    __RETURN(__SUCCESS)
     ...
     module subroutine modifyTime(this, oClients, rc)
        ...
        if (this%timeInfo%is_initialized) then
           v = this%timeInfo%define_time_variable(__RC)
           ...
           __FAIL("Time was not initialized for the GriddedIO class instance")
        end if
        __RETURN(ESMF_SUCCESS)
...
module subroutine finalize(this,rc)
   ...
   integer :: status
    ...
    if (this%use_pfio) then
       ...
       do j=1, ic_2d
          deallocate ( this%var2d(j)%array_x, __STAT )
       end do
       deallocate ( this%var2d, __STAT )
       do j=1, ic_3d
          if (this%write_lev_first) then
             deallocate ( this%var3d(j)%array_zx, __STAT )
          else
             deallocate ( this%var3d(j)%array_xz, __STAT  )
          end if
       end do
       deallocate ( this%var3d, __STAT )
    end if
