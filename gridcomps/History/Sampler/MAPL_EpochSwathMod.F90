...
contains
  ...
  function new_samplerHQ(clock, key, config, rc) result(hq)
    ...
    call ESMF_ConfigGetAttribute(config, value=time_integer, label=trim(key)//'.Epoch:', default=0, __RC)
    call ESMF_ConfigGetAttribute(config, value=hq%tunit,     label=trim(key)//'.tunit:', default="", __RC)
    __ASSERT(time_integer /= 0, 'Epoch value in config wrong')
    second = hms_2_s (time_integer)
    call ESMF_TimeIntervalSet(frequency_epoch, s=second, __RC)
    hq%frequency_epoch = frequency_epoch
    hq%RingTime  = currTime
    hq%alarm = ESMF_AlarmCreate( clock=clock, RingInterval=Frequency_epoch, &
         RingTime=hq%RingTime, sticky=.false., __RC )
  ...
  function find_config (this, key, rc) result(cf)
    ...
    __ASSERT( j>0 , trim(key)//' is not found in Hsampler CF_loc(:)')
  ...
  subroutine verify_epoch_equals_freq (this, frequency_from_list, swath_grid_label, rc)
    ...
    call ESMF_TimeIntervalGet(this%Frequency_epoch, s=hq_epoch_sec, __RC)
    ...
    call ESMF_ConfigGetAttribute(config_grid, value=time_integer, &
         label=trim(swath_grid_label)//'.Epoch:', default=0, __RC)
    ...
    __ASSERT(con, 'Error in '//trim(swath_grid_label)//' related swath and list in History.rc: Epoch in all swath grids must be equal, and equal to list%freq')
    __RETURN(__SUCCESS)
  ...
  function create_grid(this, key, currTime, grid_type, rc)  result(ogrid)
    ...
    call ESMF_TimeGet(currTime, timeString=time_string, __RC)

    !
    ! -- the `ESMF_ConfigSetAttribute` shows a risk
    !    to overwrite the nextline in config
    !
    call ESMF_ConfigSetAttribute( config_grid, trim(time_string), label=trim(key)//'.Epoch_init:', __RC)

    ogrid = grid_manager%make_grid(config_grid, prefix=trim(key)//'.', __RC )
  ...
  subroutine regrid_accumulate_on_xysubset (this, sp, rc)
    ...
    factory => grid_manager%get_factory(sp%output_grid,__RC)
    call factory%get_xy_subset( timeset, xy_subset, __RC)

    ! __ s2.  interpolate then save data using xy_mask

    call sp%interp_accumulate_fields (xy_subset, __RC)
  ...
  subroutine destroy_rh_regen_ogrid (this, key_grid_label, output_grids, sp, rc)
    ...
    call grid_manager%destroy(pgrid,__RC)
    ...
    do while (iter /= output_grids%end())
       ...
       if (trim(key)==trim(key_str)) then
          ogrid = this%create_grid (key_str, currTime, __RC)
    ...
    !__ s2. destroy RH
    call sp%regrid_handle%destroy(__RC)



    !__ s3. destroy acc_bundle / output_bundle

   call ESMF_FieldBundleGet(sp%acc_bundle,fieldCount=numVars,__RC)
   allocate(names(numVars),__STAT)
   call ESMF_FieldBundleGet(sp%acc_bundle,fieldNameList=names,__RC)
   do i=1,numVars
      call ESMF_FieldBundleGet(sp%acc_bundle,trim(names(i)),field=field,__RC)
      call ESMF_FieldDestroy(field,noGarbage=.true., __RC)
   enddo
   call ESMF_FieldBundleDestroy(sp%acc_bundle,noGarbage=.true.,__RC)
   deallocate(names,__STAT)

   call ESMF_FieldBundleGet(sp%output_bundle,fieldCount=numVars,__RC)
   allocate(names(numVars),__STAT)
   call ESMF_FieldBundleGet(sp%output_bundle,fieldNameList=names,__RC)
   do i=1,numVars
      call ESMF_FieldBundleGet(sp%output_bundle,trim(names(i)),field=field,__RC)
      call ESMF_FieldDestroy(field,noGarbage=.true., __RC)
   enddo
   call ESMF_FieldBundleDestroy(sp%output_bundle,noGarbage=.true.,__RC)
   deallocate(names,__STAT)
  ...
  subroutine fill_time_in_bundle (this, xname, bundle, ogrid, rc)
    ...
    call ESMF_FieldBundleGet (bundle, xname, field=field, __RC)
    call ESMF_FieldGet (field, farrayptr=ptr2d, __RC)

    ! __ obs_time from swath factory
    factory => grid_manager%get_factory(ogrid,__RC)
    call factory%get_obs_time (ogrid, ptr2d, __RC)
    ...
    __RETURN(ESMF_SUCCESS)
     ...
     subroutine Create_bundle_RH(this,items,bundle,tunit,timeInfo,vdata,ogrid,rc)
        ...
        do while (iter /= this%items%end())
           ...
           if (item%itemType == ItemTypeScalar) then
              call this%CreateVariable(item%xname,__RC)
           else if (item%itemType == ItemTypeVector) then
              call this%CreateVariable(item%xname,__RC)
              call this%CreateVariable(item%yname,__RC)
           end if
           call iter%next()
        enddo
        ...
        !
        this%acc_bundle = ESMF_FieldBundleCreate(__RC)
        call ESMF_FieldBundleSet(this%acc_bundle,grid=this%output_grid,__RC)
        iter = this%items%begin()
        do while (iter /= this%items%end())
           item => iter%get()
           call this%addVariable_to_acc_bundle(item%xname,__RC)
           if (item%itemType == ItemTypeVector) then
              call this%addVariable_to_acc_bundle(item%yname,__RC)
           end if
           call iter%next()
        enddo
        ...
        ! add attribute
        !
        call ESMF_AttributeSet(new_field,'UNITS',trim(tunit),__RC)
        call MAPL_FieldBundleAdd( this%acc_bundle, new_field, __RC )
     ...
     subroutine set_param(this,deflation,quantize_algorithm,quantize_level,zstandard_level,chunking,nbits_to_keep,regrid_method,itemOrder,write_collection_id,rc)
        ...
        if (present(chunking)) then
           allocate(this%chunking,source=chunking,__STAT)
        end if
     ...
     subroutine set_default_chunking(this,rc)
        ...
        if (global_dim(1)*6 == global_dim(2)) then
           allocate(this%chunking(5),__STAT)
           ...
           allocate(this%chunking(4),__STAT)
           this%chunking(1) = global_dim(1)
     ...
     subroutine check_chunking(this,lev_size,rc)
        ...
        call MAPL_GridGet(this%output_grid,globalCellCountPerDim=global_dim,__RC)
        if (global_dim(1)*6 == global_dim(2)) then
           write(c2,'(I5)')global_dim(1)
           write(c1,'(I5)')this%chunking(1)
           __ASSERT(this%chunking(1) <= global_dim(1), "Chunk for Xdim "//c1//" must be less than or equal to "//c2)
           write(c1,'(I5)')this%chunking(2)
           __ASSERT(this%chunking(2) <= global_dim(1), "Chunk for Ydim "//c1//" must be less than or equal to "//c2)
           __ASSERT(this%chunking(3) <= 6, "Chunksize for face dimension must be 6 or less")
           if (lev_size > 0) then
              write(c2,'(I5)')lev_size
              write(c1,'(I5)')this%chunking(4)
              __ASSERT(this%chunking(4) <= lev_size, "Chunk for level size "//c1//" must be less than or equal to "//c2)
           end if
           __ASSERT(this%chunking(5) == 1, "Time must have chunk size of 1")
        else
           write(c2,'(I5)')global_dim(1)
           write(c1,'(I5)')this%chunking(1)
           __ASSERT(this%chunking(1) <= global_dim(1), "Chunk for lon "//c1//" must be less than or equal to "//c2)
           write(c2,'(I5)')global_dim(2)
           write(c1,'(I5)')this%chunking(2)
           __ASSERT(this%chunking(2) <= global_dim(2), "Chunk for lat "//c1//" must be less than or equal to "//c2)
           if (lev_size > 0) then
              write(c2,'(I5)')lev_size
              write(c1,'(I5)')this%chunking(3)
              __ASSERT(this%chunking(3) <= lev_size, "Chunk for level size "//c1//" must be less than or equal to "//c2)
           end if
           __ASSERT(this%chunking(4) == 1, "Time must have chunk size of 1")
        endif
        __RETURN(ESMF_SUCCESS)
     ...
     subroutine RegridScalar(this,itemName,rc)
        ...
        if (this%doVertRegrid) then
           ...
           if (fieldRank==3) then
              if (hasDE_in) then
                 ...
                 allocate(ptr3d(0,0,0),__STAT)
              end if
              allocate(ptr3d_inter(size(ptr3d,1),size(ptr3d,2),this%vdata%lm),__STAT)
        ...
        if (fieldRank==2) then
           if (hasDE_in) then
              ...
              allocate(ptr2d(0,0),__STAT)
           end if
           if (hasDE_out) then
              call MAPL_FieldGetPointer(OutField,outptr2d,rc=status)
              __VERIFY(status)
           else
              allocate(outptr2d(0,0),__STAT)
           end if
...
!!           print *, minval(outptr2d)
           ...
           if (hasDE_out) then
              ...
              allocate(outptr3d(0,0,0),__STAT)
           end if
           if (gridIn==gridOut) then
              outPtr3d=Ptr3d
           else
     ...
     subroutine RegridVector(this,xName,yName,rc)
        ...
        if (this%doVertRegrid) then
           ...
           if (fieldRank==3) then
              if (hasDE_in) then
                 ...
                 allocate(xptr3d(0,0,0),__STAT)
              end if
              allocate(xptr3d_inter(size(xptr3d,1),size(xptr3d,2),this%vdata%lm),__STAT)
              if (this%vdata%regrid_type==VERTICAL_METHOD_SELECT) then
           ...
           if (fieldRank==3) then
              if (hasDE_in) then
                 ...
                 allocate(yptr3d(0,0,0),__STAT)
              end if
              allocate(yptr3d_inter(size(yptr3d,1),size(yptr3d,2),this%vdata%lm),__STAT)
              if (this%vdata%regrid_type==VERTICAL_METHOD_SELECT) then
        ...
        if (fieldRank==2) then
           if (hasDE_in) then
              ...
              allocate(xptr2d(0,0),__STAT)
              allocate(yptr2d(0,0),__STAT)
           end if
           ...
           if (hasDE_in) then
              ...
              allocate(xoutptr2d(0,0),__STAT)
              allocate(youtptr2d(0,0),__STAT)
           end if
           ...
           if (hasDE_out) then
              ...
              allocate(xoutptr3d(0,0,0),__STAT)
              allocate(youtptr3d(0,0,0),__STAT)
           end if
  ...
  subroutine alphabatize_variables(this,nfixedVars,rc)
     ...
     __VERIFY(status)
     n = Order%size()
     allocate(temp(nFixedVars+1:n),__STAT)
     do i=1,n
        v1 => order%at(i)
  ...
  subroutine addVariable_to_acc_bundle(this,itemName,rc)
    ...
    call ESMF_FieldBundleGet(this%input_bundle,itemName,field=field,__RC)
    call ESMF_FieldGet(field,rank=fieldRank,rc=status)
    if (this%doVertRegrid .and. (fieldRank ==3) ) then
       newField = MAPL_FieldCreate(field,this%output_grid,lm=this%vData%lm,__RC)
    else
       newField = MAPL_FieldCreate(field,this%output_grid,__RC)
    end if
    call MAPL_FieldBundleAdd(this%acc_bundle,newField,__RC)
  ...
  subroutine interp_accumulate_fields (this,xy_subset,rc)
    ...
    call ESMF_FieldBundleGet(this%output_bundle, grid=grid, __RC)
    call ESMF_GridGet(grid, localDECount=localDECount, dimCount=dimCount, __RC)
    allocate ( LB(dimCount), UB(dimCount), exclusiveCount(dimCount) ,__STAT)
    allocate ( compLB(dimCount), compUB(dimCount), compCount(dimCount) ,__STAT)

    allocate ( j1(0:localDEcount-1) ,__STAT)  ! start
    allocate ( j2(0:localDEcount-1) ,__STAT)  ! end

    __ASSERT ( localDEcount == 1, 'failed, due to localDEcount > 1')
...
!!    write(6,*) 'j2(localDe)', j2(0:localDeCount-1)
    ...
    do while (iter /= this%items%end())
       item => iter%get()
       if (item%itemType == ItemTypeScalar) then
          call this%RegridScalar(item%xname,__RC)
          call ESMF_FieldBundleGet(this%output_bundle,item%xname,field=outField, __RC)
          if (this%vdata%regrid_type==VERTICAL_METHOD_ETA2LEV) then
             call this%vdata%correct_topo(outField,__RC)
          end if
       elseif (item%itemType == ItemTypeVector) then
          call this%RegridVector(item%xname,item%yname,__RC)
          call ESMF_FieldBundleGet(this%output_bundle,item%xname,field=outField, __RC)
          if (this%vdata%regrid_type==VERTICAL_METHOD_ETA2LEV) then
             call this%vdata%correct_topo(outField,__RC)
          end if
          call ESMF_FieldBundleGet(this%output_bundle,item%yname,field=outField2, __RC)
          if (this%vdata%regrid_type==VERTICAL_METHOD_ETA2LEV) then
             call this%vdata%correct_topo(outField2,__RC)
          end if
       end if
       ...
       !    xname
       call ESMF_FieldGet(outField, Array=array1, __RC)
       call ESMF_FieldBundleGet(this%acc_bundle,item%xname,field=new_outField,__RC)
       call ESMF_FieldGet(new_outField, Array=array2, __RC)
       call ESMF_ArrayGet(array1, rank=rank, __RC)
       if (rank==2) then
          call ESMF_ArrayGet(array1, farrayptr=pt2d, __RC)
          call ESMF_ArrayGet(array2, farrayptr=pt2d_, __RC)
          localDe=0
       ...
       elseif (rank==3) then
          call ESMF_ArrayGet(array1, farrayptr=pt3d, __RC)
          call ESMF_ArrayGet(array2, farrayptr=pt3d_, __RC)
          do localDe=0, localDEcount-1
             if (j1(localDe)>0) then
                do j= j1(localDe), j2(localDe)
                   jj= j-jj1+1
          ...
          __FAIL('failed interp_accumulate_fields')
       endif
       ...
       elseif (item%itemType == ItemTypeVector) then
          ...
          ! add yname
          !
          call ESMF_FieldGet(outField2, Array=array1, __RC)
          call ESMF_FieldBundleGet(this%acc_bundle,item%yname,field=new_outField,__RC)
          call ESMF_FieldGet(new_outField, Array=array2, __RC)
          call ESMF_ArrayGet(array1, rank=rank, __RC)
          if (rank==2) then
             call ESMF_ArrayGet(array1, farrayptr=pt2d, __RC)
