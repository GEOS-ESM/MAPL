...
39. persist_closest with a wide valid_range where only a subset of files (2005 monthly) are present on disk. valid_range spans 2004-2006. refine_valid_range narrows to actual on-disk files and intersects with the model run period (20050615) so that no files outside the run window are required.
40. clim extrapolation with a wide valid_range spanning 2005-2009, but only 2007 monthly files exist on disk. refine_valid_range (called for clim via Option B) narrows the valid_range to the actual 2007 file timestamps. Run requests 02/29/2008 (leap day), which year-wraps to 2007 and interpolates between Feb 15 and Mar 15 2007.
