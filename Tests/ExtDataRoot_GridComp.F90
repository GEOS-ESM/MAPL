...
! !ARGUMENTS:
         ...
         call ESMF_GridCompGet( GC, NAME=COMP_NAME, CONFIG=CF, __RC )
         ...
         __VERIFY(status)
         call ESMF_ConfigFindLabel(cf,"tiling_file:",isPresent=synth%on_tiles,__RC)
         ...
         call AddState(GC,CF,"IMPORT",__RC)
         call AddState(GC,CF,"EXPORT",__RC)
         ...
         call MAPL_AddInternalSpec(GC,&
               ...
               units = 'Pa', &
               dims = MAPL_DimsHorzVert, &
               vlocation = MAPL_VLocationEdge, __RC)
         call MAPL_AddExportSpec(GC, &
               short_name='test_bundle', &
               long_name='test', &
               units='X', &
               datatype=MAPL_BundleItem, __RC)
      ...
      SUBROUTINE Initialize_ ( GC, IMPORT, EXPORT, CLOCK, rc )
         ...
         synth%delay = -1.0
         call ESMF_ConfigFindLabel(cf,label='delay:',isPresent=isPresent,__RC)
         if (isPresent) then
            call ESMF_ConfigGetAttribute(cf,label='delay:',value=synth%delay,__RC)
         end if
         fill_bundle=.false.
         call ESMF_ConfigFIndLabel(cf,label='fill_bundle:',isPresent=isPresent,__RC)
         if (isPresent) then
            call ESMF_ConfigGetAttribute(cf,label='fill_bundle:',value=fill_bundle,__RC)
         end if

         call ESMF_ConfigGetDim(cf,nrows,ncolumn,label="FILL_DEF::",rc=status)
         if (status==ESMF_SUCCESS) then
            call ESMF_ConfigFindLabel(cf,label="FILL_DEF::",__RC)
            do i=1,nrows
               call ESMF_ConfigNextLine(cf,__RC)
               call ESMF_ConfigGetAttribute(cf,value=key,__RC)
               call ESMF_ConfigGetAttribute(cf,value=keyVal,__RC)
               call synth%fillDefs%insert(trim(key),trim(keyVal))
            enddo
         end if
         call synth%tFunc%init_time(cf,currTime,__RC)

         call ESMF_ConfigGetAttribute(cf,value=synth%runMode,label="RUN_MODE:",__RC)

         call MAPL_GridCreate(GC, __RC)
         call ESMF_GridCompGet(GC, grid=grid, __RC)
         call set_locstream(__RC)

         call MAPL_GenericInitialize ( GC, IMPORT, EXPORT, clock, __RC)
         call ForceAllocation(Export,__RC)
         if (fill_bundle) then
            call FillBundle(Export,__RC)
         end if

         call MAPL_Get ( MAPL, internal_esmf_state=internal, cf=cf, __RC )
         call FillState(internal,export,currTime,grid,synth,cf,__RC)
      ...
      contains
            ...
            if (synth%on_tiles) then
               call ESMF_ConfigGetAttribute(cf,tile_file,label="tiling_file:",__RC)
               ...
               call MAPL_LocStreamCreate(exch,layout=layout,filename=tile_file, &
                    name = 'my_tiles', mask = [MAPL_LAND], grid=grid,__RC)
               call MAPL_ExchangeGridSet(gc,exch,__RC)
               call MAPL_GenericMakeXchgNatural(MAPL,__RC)
      ...
      SUBROUTINE Run_ ( GC, IMPORT, EXPORT, CLOCK, rc )
         ...
         call MAPL_Get(MAPL, childrens_gridcomps=GCS, &
              childrens_import_states =GIM, childrens_export_states=GEX, __RC)
         call MAPL_Get ( MAPL, internal_esmf_state=internal, cf=cf, __RC )
         call ESMF_ClockGet(Clock,currTime=currTime,__RC)
         ...
         if (.not. synth%on_tiles) then
            call ESMF_GridCompGet(GC,grid=grid,__RC)
            call MAPL_GetPointer(internal,ptrR4,'lons',__RC)
            call ESMF_GridGetCoord (Grid, coordDim=1, localDE=0, &
                              staggerloc=ESMF_STAGGERLOC_CENTER, &
                              farrayPtr=ptrR8, __RC)
            ptrR4=ptrR8
            call MAPL_GetPointer(internal,ptrR4,'lats',__RC)
         ...
         case(RunModeGenerateImports)
            ...
            call FillState(internal,import,currTime,grid,synth,cf,__RC)

         case(runModecompareImports)
            call FillState(internal,export,currTime,grid,synth,cf,__RC)
            call CompareState(import,export,0.001,__RC)

         case(runModeFillImport)
! Nothing to do, we are just letting ExtData run

         case(runModeFillExportFromImport)
            call CopyState(import,export,__RC)
   ...
   subroutine AddState(gc,cf,stateType,rc)
      ...
      if (trim(stateType) == 'IMPORT') then
         ...
         if (status==ESMF_SUCCESS) then
            call ESMF_ConfigFindLabel(cf,label="IMPORT_STATE::",__RC)
            do i=1,nrows
               call ESMF_ConfigNextLine(cf,__RC)
      ...
      if (trim(stateType) == 'EXPORT') then
         ...
         if (status==ESMF_SUCCESS) then
         call ESMF_ConfigFindLabel(cf,label="EXPORT_STATE::",__RC)
            do i=1,nrows
               call ESMF_ConfigNextLine(cf,__RC)
               VarspecDescr = VarspecDescription(CF,ncolumn,rc)
               call VarspecVec%push_back(VarspecDescr)
      ...
      do while (iter /= VarspecVec%end())
         VarspecPtr => iter%get()
         call VarspecPtr%addNewSpec(gc,stateType,__RC)
         call iter%next()
      end do
   ...
   subroutine init_time(this,cf,currTime,rc)
      ...
      call ESMF_ConfigFindLabel(cf,'REF_TIME:',isPresent=isPresent,__RC)
      if (isPresent) then
         call ESMF_ConfigGetAttribute(cf,datetime,label='REF_TIME:',__RC)
         YY =     datetime(1)/10000
         ...
         call ESMF_TimeSet(this%refTime,yy=yy,mm=mm,dd=dd,h=hh,m=mn,s=ss,__RC)
      else
         this%refTime=currTime
      end if
      call ESMF_ConfigGetAttribute(cf,this%timeUnits,label='TIME_UNITS:',default='days',__RC)

      call ESMF_ConfigGetAttribute(cf,this%climYear,label='CLIM_YEAR:',default=-1,__RC)

      this%have_offset = .false.
      this%update_ref_time = -1
      call ESMF_ConfigFindLabel(cf,'UPDATE_OFFSET:',isPresent=isPresent,__RC)
      if (isPresent) then
         call ESMF_ConfigGetAttribute(cf,int_time,label='UPDATE_OFFSET:',__RC)
         HH = int_time/10000
         MN = mod(int_time,10000)/100
         SS = mod(int_time,100)
         call ESMF_TimeIntervalSet(this%update_offset,h=hh,m=mn,s=ss,__RC)
         this%have_offset = .true.
      end if
      call ESMF_ConfigFindLabel(cf,'UPDATE_REF_TIME:',isPresent=isPresent,__RC)
      if (isPresent) then
         call ESMF_ConfigGetAttribute(cf,this%update_ref_time,label='UPDATE_REF_TIME:',__RC)
      end if
      __RETURN(__SUCCESS)
   ...
   function evaluate_time(this,currTime,rc) result(dt)
      ...
      if (this%climYear > 0) then
         call ESMF_TimeGet(currTime,yy=ycurr,__RC)
         yint=this%climYear-ycurr
         call ESMF_TimeIntervalSet(yearInterval,yy=yint,__RC)
         currTime = currTime+yearInterval
      end if
      periodic_time = this%set_time_for_date(currTime,__RC)
      ...
      case ('days')
         call ESMF_TimeIntervalGet(timeInterval,d_r8=dt,__RC)
      case ('hours')
         call ESMF_TimeIntervalGet(timeInterval,h_r8=dt,__RC)
      case ('minutes')
         call ESMF_TimeIntervalGet(timeInterval,m_r8=dt,__RC)
      case ('seconds')
         call ESMF_TimeIntervalGet(timeInterval,s_r8=dt,__RC)
      case default
         __FAIL("Unsupported time units specify for interval")
      end select
   ...
   function set_time_for_date(this,input_time,rc) result(returned_time)
      ...
      if (this%update_ref_time /= -1) then
         call ESMF_TimeGet(input_time,yy=year,mm=month,dd=day,__RC)
         ...
         if (new_time == input_time) then
            ...
            call ESMF_TimeSet(new_time,yy=year,mm=month,dd=day-1,h=hour,m=minute,s=second,__RC)
            returned_time = new_time
         end if
      else
         returned_time = input_time
   ...
   subroutine CopyState(inState,outState,rc)
      ...
      call ESMF_StateGet(inState,itemcount=itemCountIn,__RC)
      allocate(InNameList(itemCountIn),stat=status)
      __VERIFY(status)
      allocate(item_type_in(itemCountIn),stat=status)
      __VERIFY(status)
      call ESMF_StateGet(inState,itemNameList=InNameList,itemTypeList=item_type_in,__RC)

      call ESMF_StateGet(outState,itemcount=ItemCountOut,__RC)
      allocate(outNameList(ItemCountOut),stat=status)
      __VERIFY(status)
      call ESMF_StateGet(outState,itemNameList=outNameList,__RC)

      call ESMF_StateGet(inState,itemNameList=inNameList,__RC)
      do i=1,itemCountIn
         if (item_type_in(i) == ESMF_STATEITEM_FIELD) then
            call ESMF_StateGet(inState,trim(inNameList(i)),impf,__RC)
            call ESMF_StateGet(outState,trim(inNameList(i)),expf,__RC)
            call ESMF_FieldGet(impf,rank=rank,__RC)
            if (rank==1) then
               call MAPL_GetPointer(inState,IMptr1,inNameList(i),__RC)
               call MAPL_GetPointer(outState,Exptr1,inNameList(i),alloc=.true.,__RC)
               EXptr1=IMptr1
            else if (rank==2) then
               call MAPL_GetPointer(inState,IMptr2,inNameList(i),__RC)
               call MAPL_GetPointer(outState,Exptr2,inNameList(i),alloc=.true.,__RC)
               EXptr2=IMptr2
            else if (rank==3) then
               call MAPL_GetPointer(inState,IMptr3,inNameList(i),__RC)
               call MAPL_GetPointer(outState,EXptr3,inNameList(i),alloc=.true.,__RC)
               EXptr3=IMptr3
            end if
         end if
      end do
   ...
   subroutine FillState(inState,outState,time,grid,Synth,cf,rc)
      ...
      if (.not. synth%on_tiles) then
         call MAPL_GridGet(grid,localcellcountperdim=ldims,__RC)
         call MAPL_Grid_Interior(grid,i1,in,j1,jn)
      end if
      call ESMF_StateGet(outState,itemcount=itemCount,__RC)
      allocate(outNameList(itemCount),stat=status)
      __VERIFY(status)
      allocate(item_type(itemCount),stat=status)
      __VERIFY(status)
      call ESMF_StateGet(outState,itemTypeList=item_type,itemNameList=outNameList,__RC)

      if (synth%on_tiles) then
         call MAPL_GetPointer(inState,exPtr1,'time',__RC)
         exPtr1=synth%tFunc%evaluate_time(Time,__RC)
      else
         call MAPL_GetPointer(inState,exPtr2,'time',__RC)
         exPtr2=synth%tFunc%evaluate_time(Time,__RC)
      end if

      if (.not. synth%on_tiles) then
         call MAPL_GetPointer(inState,exPtr2,'i_index',__RC)
         do j = 1,ldims(2)
            do i=1,ldims(1)
               exPtr2(i,j)=i1+i-1
            enddo
         enddo
         call MAPL_GetPointer(inState,exPtr2,'j_index',__RC)
         do i = 1,ldims(1)
            do j=1,ldims(2)
               exPtr2(i,j)=j1+j-1
            enddo
         enddo
         call MAPL_GetPointer(inState,exPtr3,'numlev',__RC)
         do i=1,size(exPtr3,3)
            exPtr3(:,:,i)=i
         enddo
         call MAPL_GetPointer(inState,exPtr3,'PLE',__RC)
         call Fill_PLE(exPtr3, cf, __RC)
      end if

      if (synth%on_tiles) then
         call MAPL_GetPointer(inState,exPtr1,'doy',__RC)
         exPtr1 = compute_doy(time,__RC)
      else
         call MAPL_GetPointer(inState,exPtr2,'doy',__RC)
         exPtr2 = compute_doy(time,__RC)
      end if

      call random_seed(size=seed_size)
      allocate(seeds(seed_size))
      call ESMF_VMGetCurrent(vm,__RC)
      call ESMF_VMGet(vm,localPet=mypet,__RC)
      seeds = mypet
      call random_seed(put=seeds)
      if (synth%on_tiles) then
         call MAPL_GetPointer(inState,exPtr1,'rand',__RC)
         call random_number(exPtr1)
      else
         call MAPL_GetPointer(inState,exPtr2,'rand',__RC)
         call random_number(exPtr2)
      end if

      call ESMF_StateGet(inState,'time',farray(1),__RC)
      call ESMF_StateGet(inState,'lons',farray(2),__RC)
      call ESMF_StateGet(inState,'lats',farray(3),__RC)
      call ESMF_StateGet(inState,'i_index',farray(4),__RC)
      call ESMF_StateGet(inState,'j_index',farray(5),__RC)
      call ESMF_StateGet(inState,'doy',farray(6),__RC)
      call ESMF_StateGet(inState,'rand',farray(7),__RC)
      call ESMF_StateGet(inState,'numlev',farray(8),__RC)
      call ESMF_StateGet(inState,'PLE',farray(9),__RC)
      pstate = ESMF_StateCreate(__RC)
      call ESMF_StateAdd(pstate,farray,__RC)

      do i=1,itemCount
         if (item_type(i) == ESMF_STATEITEM_FIELD) then
            call ESMF_StateGet(outState,trim(outNameList(i)),expf,__RC)
            fexpr => synth%fillDefs%at(trim(outNameList(i)))
            if (associated(fexpr)) then
               call MAPL_StateEval(pstate,fexpr,expf,__RC)
            end if
         end if
      enddo
      ...
      subroutine fill_ple(ple_ptr, cf, rc)
         ...
         call ESMF_ConfigFindLabel(cf,"akbk_file:",isPresent=has_akbk,__RC)
         !call ESMF_ConfigFindLabel(cf,"ps_val:",isPresent=has_psval,__RC)
         call ESMF_ConfigFindLabel(cf,"ps_file:",isPresent=has_psval,__RC)
         if (has_akbk .and. has_psval) then
            call ESMF_ConfigGetAttribute(cf,label="akbk_file:",value=akbk_file,__RC)
            !call ESMF_ConfigGetAttribute(cf,label="ps_val:",value=ps_val,__RC)
            call ESMF_ConfigGetAttribute(cf,label="ps_file:",value=ps_file,__RC)
         else
            __RETURN(__SUCCESS)
         end if
        
         allocate(ps(size(ple_ptr,1),size(ple_ptr,2))) 
         call fs%open(trim(ps_file),pFIO_READ,__RC)
         call fs%get_var("PS",ps, __RC)

         hconfig = ESMF_HConfigCreate(filename=trim(akbk_file), __RC)
         km = size(ple_ptr,3) - 1
         write(km_str,'(i3.3)') km
         akbk = ESMF_HConfigCreateAt(hconfig, keyString='L'//trim(km_str), __RC) 
         ak = ESMF_HConfigAsR4Seq(akbk, keyString='ak', __RC)
         bk = ESMF_HConfigAsR4Seq(akbk, keyString='bk', __RC)
   ...
   subroutine FillBundle(inState,rc)
      ...
      call ESMF_StateGet(InState,itemcount=itemCount,__RC)
      allocate(outNameList(itemCount),stat=status)
      __VERIFY(status)
      allocate(item_type(itemCount),stat=status)
      __VERIFY(status)
      call ESMF_StateGet(InState,itemTypeList=item_type,itemNameList=outNameList,__RC)

      call ESMF_StateGet(InState,"test_bundle",bundle,__RC)
      do i=1,itemCount
         if (item_type(i) == ESMF_STATEITEM_FIELD) then
            call ESMF_StateGet(InState,trim(outNameList(i)),field,__RC)
   ...
   subroutine CompareState(State1,State2,tol,rc)
      ...
      call ESMF_StateGet(State1,itemcount=itemCount,__RC)
         allocate(NameList(itemCount),stat=status)
         __VERIFY(status)
         allocate(foundDiff(itemCount),stat=status,source=.false.)
         __VERIFY(status)
         call ESMF_StateGet(State1,itemNameList=NameList,__RC)
         do i=1,itemCount
            if (trim(nameList(i)) == 'PLE') cycle
            call ESMF_StateGet(State1,trim(nameList(i)),field1,__RC)
            call ESMF_StateGet(State2,trim(nameList(i)),field2,__RC)
            call ESMF_FieldGet(field1,rank=rank1,__RC)
            call ESMF_FieldGet(field2,rank=rank2,__RC)
            all_undef1 = FieldIsConstant(field1,MAPL_UNDEF,__RC)
            all_undef2 = FieldIsConstant(field2,MAPL_UNDEF,__RC)
            if (all_undef1 .or. all_undef2) then
               exit
            end if
            __ASSERT(rank1==rank2,'needs informative message')
            call assign_fptr(field1, ptr1, __RC)
            call assign_fptr(field2, ptr2, __RC)
            __ASSERT(size(ptr1)==size(ptr2),'needs informative message')
            foundDiff(i)=.false.
      ...
      subroutine ForceAllocation(state,rc)
         ...
         __VERIFY(status)
         call ESMF_StateGet(State,itemNameList=NameList,itemTypeList=itemTypeList,__RC)
         if (itemCount == 0) then
            __RETURN(ESMF_SUCCESS)
         end if
         do ii=1,itemCount
            if (itemTypeList(ii)==ESMF_STATEITEM_FIELD) then
               call ESMF_StateGet(State,trim(nameList(ii)),field,__RC)
      ...
      function compute_doy(time,rc) result(doy)
         ...
         call ESMF_TimeGet(time,yy=year,mm=month,dd=day,h=hour,m=minute,s=second,__RC)
         call ESMF_TimeSet(start_0z,yy=year,mm=1,dd=1,h=0,m=0,s=0,__RC)
         call ESMF_TimeSet(current_0z,yy=year,mm=month,dd=day,h=hour,m=minute,s=second,__RC)
         tint = current_0z-start_0z
         call ESMF_TimeIntervalGet(tint,d_r8=doy,__RC)
         __RETURN(__SUCCESS)
      end function
