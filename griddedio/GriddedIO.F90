...
module MAPL_GriddedIOMod
  ...
  contains
     ...
     subroutine CreateFileMetaData(this,items,bundle,timeInfo,vdata,ogrid,global_attributes,rc)
        ...
        call MAPL_FieldBundleDestroy(this%output_bundle, __RC)
        ...
        if (.not.allocated(this%chunking)) then
           ...
           call this%check_chunking(this%vdata%lm,__RC)
        end if
        ...
        if (this%quantizeAlgorithm /= MAPL_NOQUANTIZE) then
           call this%CreateQuantizationInfo(__RC)
        end if
     ...
     subroutine CreateQuantizationInfo(this,rc)
        ...
        factory => get_factory(this%output_grid,__RC)
        ...
        case default
           __FAIL('Unknown quantization algorithm')
        end select
        ...
        call factory%append_variable_metadata(v)
        call this%metadata%add_variable(trim(varName),v,__RC)
     ...
     subroutine bundlepost(this,filename,oClients,rc)
        ...
        if (have_time) then
           ...
           if (tindex==1) then
              call this%stage2DLatLon(filename,oClients=oClients,__RC)
           end if
        else
           tindex = -1
           call this%stage2DLatLon(filename,oClients=oClients,__RC)
     ...
     subroutine RegridScalar(this,itemName,rc)
        ...
        __VERIFY(status)
        long_name = 'unknown'
        call ESMF_AttributeGet(outField, name="LONG_NAME", isPresent=isPresent, __RC)
        if ( isPresent ) then
           call ESMF_AttributeGet(outField, name="LONG_NAME",value=long_name, __RC)
        endif
     ...
     subroutine RegridVector(this,xName,yName,rc)
        ...
        __VERIFY(status)
        long_name_x = 'unknown'
        call ESMF_AttributeGet(xoutField, name="LONG_NAME", isPresent=isPresent, __RC)
        if ( isPresent ) then
           call ESMF_AttributeGet(xoutField, name="LONG_NAME",value=long_name_x, __RC)
        endif

        call ESMF_FieldBundleGet(this%output_bundle,yName,field=youtField,rc=status)
        __VERIFY(status)
        long_name_y = 'unknown'
        call ESMF_AttributeGet(youtField, name="LONG_NAME", isPresent=isPresent, __RC)
        if ( isPresent ) then
           call ESMF_AttributeGet(youtField, name="LONG_NAME",value=long_name_y, __RC)
        endif
  ...
  subroutine stageData(this, field, fileName, tIndex, oClients, rc)
     ...
     call ESMF_VMGet(vm,mpiCommunicator=mpi_comm,__RC)
  ...
  subroutine request_data_from_file(this,filename,timeindex,rc)
     ...
     this%current_file_metadata => collection%find(filename, __RC)
     ...
     do i=1,numVars
        ...
        call i_Clients%collective_prefetch_data( &
             ...
             & ref, start=localStart, global_start=globalStart, global_count=globalCount, __RC)
        deallocate(localStart,globalStart,globalCount)
     enddo
  ...
  subroutine process_data_from_file(this,rc)
     ...
     do while(iter /= this%items%end())
        item => iter%get()
        if (item%itemType == ItemTypeScalar) then
           call this%swap_undef_value(trim(item%xname),__RC)
           call this%regridScalar(trim(item%xname),rc=status)
           __VERIFY(status)
        else if (item%itemType == ItemTypeVector) then
           call this%swap_undef_value(trim(item%xname),__RC)
           call this%swap_undef_value(trim(item%yname),__RC)
  ...
  subroutine swap_undef_value(this,fname,rc)
     ...
     fill_value = this%current_file_metadata%var_get_missing_value(fname,__RC)

     call ESMF_FieldBundleGet(this%input_bundle,fname,field=field,__RC)
     call ESMF_FieldBundleGet(this%input_bundle,grid=gridIn,__RC)
     call ESMF_FieldGet(field,rank=fieldRank,__RC)
     hasDE_in = MAPL_GridHasDE(gridIn,__RC)

     if (fieldRank==2) then
        if (hasDE_in) then
           call MAPL_FieldGetPointer(field,ptr2d,__RC)
        else
           allocate(ptr2d(0,0))
        end if
