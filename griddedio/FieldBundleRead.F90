...
module MAPL_ESMFFieldBundleRead
   ...
   contains
      ...
      subroutine MAPL_create_bundle_from_metdata_id(bundle,metadata_id,file_name,only_vars,rc)
         ...
         metadata => collection%find(trim(file_name), __RC)
         ...
         __ASSERT(num_fields == 0,"Trying to fill non-empty bundle")
         ...
         do while (var_iter /= variables%end())
            ...
            if (create_variable) then
               ...
               __VERIFY(status)
               units = metadata%get_var_attr_string(var_name_ptr,'units',__RC)
               long_name = metadata%get_var_attr_string(var_name_ptr,'long_name',__RC)
      ...
      subroutine MAPL_read_bundle(bundle,file_tmpl,time,only_vars,regrid_method,noread,file_override,file_weights,rc)
         ...
         call ESMF_TimeGet(time, timeString=timestring, __RC)
         ...
         __ASSERT(time_index/=-1,"Time not found on file "//trim(file_name))
