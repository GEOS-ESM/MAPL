...
| 5 | `user_setservices()` | Create DSOSetServices | sharedObj, userRoutine | `DSOSetServices("libconfigurable_gridcomp", "setservices_")` |
| 6 | `parse_child()` | Create ChildSpec | DSOSetServices + `child_hconfig` | ChildSpec for child A |
| 7 | `add_child_by_spec()` | Create child gridcomp | ChildSpec, child name | `child_outer_gc` (ESMF gridcomp) |
| 8 | `add_child_by_spec()` | Register child | `child_outer_gc` | Added to parent's children map |
| 9 | `run_children_setservices()` | Call child's SetServices | child gridcomp | `run_DSOSetServices()` called |
| 10 | `run_DSOSetServices()` | Load DSO and call SetServices | sharedObj, userRoutine | DSO loaded, user's `setservices_` routine called |
...
|-----------|------|---------------------|-------|---------|
| Parent Creation | `GenericGridComp.F90` | `create_grid_comp_primary()` | 87-142 | Create parent gridcomp, store YAML in OuterMetaComponent |
| Parent SetServices Orchestration | `OuterMetaComponent/SetServices.F90` | `SetServices_()` | 29-100 | Orchestrate parsing and child creation |
| Parse Component Spec | `ComponentSpecParser/parse_component_spec.F90` | `parse_component_spec()` | 8-33 | Parse entire parent YAML into ComponentSpec |
| Parse Children Section | `ComponentSpecParser/parse_children.F90` | `parse_children()` | 9-45 | Iterate children mapping in parent YAML |
| **Parse Individual Child** | **`ComponentSpecParser/parse_child.F90`** | **`parse_child()`** | **8-70** | **Read dso, SetServices, `config_file` from parent YAML** |
| Create SetServices Factory | `UserSetServices.F90` | `new_DSOSetServices()` | 135-149 | Create DSOSetServices object |
| Add Child to Parent | `OuterMetaComponent/add_child_by_spec.F90` | `add_child_by_spec()` | 19-55 | Create child gridcomp and register with parent |
