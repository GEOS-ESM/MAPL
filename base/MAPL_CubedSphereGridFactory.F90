...
contains
   ...
   function create_basic_grid(this, unusable, rc) result(grid)
      ...
      if(allocated(this%jms_2d)) then
         __ASSERT(size(this%jms_2d,2) == 6,'incompatible shape')
   ...
   subroutine initialize_from_file_metadata(this, file_metadata, unusable, force_file_coordinates, rc)
      ...
      associate(im => this%im_world)
         ...
         if (hasXdim .and. (.not.haslon)) then
            ...
            __FAIL("can not identify dimenions of cubed-sphere file")
         end if
      end associate
      ...
      if (is_stretched) then
         ...
         class default
            __FAIL('unsupport subclass for stretch params')
         end select
   ...
   subroutine check_and_fill_consistency(this, unusable, rc)
      ...
      if ( (this%target_lon /= MAPL_UNDEFINED_REAL) .and. &
         ...
         __ASSERT(this%target_lat >= -90.0, 'Latitude should be greater than -90.0 degrees')
         __ASSERT(this%target_lat <= 90, 'Latitude should be less than 90.0 degrees')
      ...
      if (allocated(this%jms_2d)) then
        __ASSERT(size(this%jms_2d,2)==6, 'incompatible shape')
        __ASSERT(sum(this%jms_2d) == 6*this%im_world, 'incompatible shape')
      else
   ...
   contains
      ...
      subroutine verify(n, m_world, ms, rc)
         ...
         if (allocated(ms)) then
            __ASSERT(size(ms) > 0, 'must be > 0 PEs in each dimension')

            if (n == MAPL_UNDEFINED_INTEGER) then
               n = size(ms)
            else
               __ASSERT(n == size(ms), 'inconsistent specs')
            end if

            if (m_world == MAPL_UNDEFINED_INTEGER) then
               m_world = sum(ms)
            else
               __ASSERT(m_world == sum(ms), 'inconsistent specs')
            end if

         else

            __ASSERT(n /= MAPL_UNDEFINED_INTEGER,'n not specified')
            __ASSERT(m_world /= MAPL_UNDEFINED_INTEGER,'m_world not specified')
