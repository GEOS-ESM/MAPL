...
contains
   ...
   function string_to_esmf_time(input_string,unusable,rc) result(time)
      ...
      if (have_time) then
         ...
         int_time = string_to_integer_time(time_string,__RC)
      else
         date_string = trim(input_string) 
         int_time = 0
      end if
      int_date = string_to_integer_date(date_string,__RC)
      ...
      call ESMF_TimeSet(time,yy=year,mm=month,dd=day,h=hour,m=min,s=sec,__RC)
      __RETURN(__SUCCESS)
   ...
   function string_to_esmf_timeinterval(time_interval_string,unusable,rc) result(time_interval)
      ...
      __ASSERT(time_interval_string(1:1) == 'P','Not valid time duration')
      ...
      call ESMF_TimeIntervalSet(time_interval,yy=year,mm=month,d=day,h=hour,m=min,s=sec,__RC)
      __RETURN(__SUCCESS)
