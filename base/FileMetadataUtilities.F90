...
module MAPL_FileMetadataUtilsMod
   ...
   function var_get_missing_value(this,var_name,rc) result(missing_value)
      ...
      fname = this%get_file_name(__RC)
      var => this%get_variable(var_name,__RC)
      __ASSERT(associated(var),"no variable named "//var_name//" in "//fname)
      ...
      if (this%var_has_attr(var_name,"_FillValue")) then
         missing_value = this%get_var_attr_real32(var_name,"_FillValue",__RC)
      end if
   ...
   function get_var_attr_real32(this,var_name,attr_name,rc) result(attr_real32)
      ...
      __ASSERT(status == __SUCCESS, 'failed to get attribute named '//attr_name//' in '//var_name//' in '//fname)
   ...
   subroutine get_time_info(this,startTime,startyear,startmonth,startday,starthour,startmin,startsec,units,timeVector,rc)
      ...
      class default
         __FAIL("Time unit must be character in "//fname)
      end select
      ...
      __ASSERT(associated(ptr),"time variable coordinate data not found in "//fname)
      select type (ptr)
      ...
      type is (integer(kind=INT32))
         tr_r64=ptr
      class default
         __FAIL("unsupported time variable type in "//fname)
      end select
      do i=1,tsize
        ...
        case ("seconds")
           ...
           tvec(i)=unmodStartTime+tint
        case default
           __FAIL("unsupported time unit in "//fname)
        end select
      enddo
   ...
   function get_variable_attribute(this,var_name,attr_name,rc) result(units)
      ...
      if (isPresent) then
         ...
         type is (character(*))
            units => vunits
         class default
            __FAIL('units must be string for '//var_name//' in '//fname)
         end select
      else
         units => null()
      end if
   ...
   subroutine get_coordinate_info(this,coordinate_name,coordSize,coordUnits,long_name,standard_name,coords,coordinate_attr,rc)
      ...
      if (present(coordUnits)) then
         ...
         class default
            __FAIL(trim(coordinate_name)//' units must be string in '//fname)
         end select
      end if 
      ...
      if (present(long_name)) then
         if (this%var_has_attr(coordinate_name,"long_name")) then
            ...
            class default
               __FAIL(trim(coordinate_name)//' long_name must be string in '//fname)
            end select
         else
             long_name = 'not found'
         endif
      ...
      if (present(standard_name)) then
         if (this%var_has_attr(coordinate_name,"standard_name")) then
            ...
            class default
               __FAIL(trim(coordinate_name)//' standard_name must be string in '//fname)
            end select
         else
      ...
      if (present(coordinate_attr)) then
         if (this%var_has_attr(coordinate_name,"coordinate")) then
            ...
            class default
               __FAIL(trim(coordinate_name)//' name must be string in '//fname)
            end select
         else
      ...
      if (present(coords)) then
         ptr => var%get_coordinate_data()
         __ASSERT(associated(ptr),"coord variable coordinate data not found in "//fname)
         select type (ptr)
         type is (real(kind=REAL64))
            coords=ptr
         type is (real(kind=REAL32))
         ...
         type is (integer(kind=INT32))
            coords=ptr
         class default
            __FAIL("unsupported coordinate variable type in "//fname)
         end select
      end if
      __RETURN(__SUCCESS)
