...
!------
    ...
    __ASSERT(.not.request%active, 'request is already active')
...
!--------------------------
    ...
    __ASSERT(gridRank > 1, 'rank 1 is not supported')
...
!--------------------------------------------------------
    ...
    call MAPL_DistGridGet (distgrid, minIndex=AL, maxIndex=AU, RC=STATUS); __VERIFY(STATUS)
...
!---------------------------------------------
    ...
    if(requestType==MAPL_IsGather) then
       if(request%amRoot) then
          if(present(DstArray)) then
             ...
             __ASSERT(all(shape(DstArray)==(/ request%IM_WORLD, request%JM_WORLD/)), 'inconsistent shape')
          else
    ...
    elseif(requestType==MAPL_IsScatter) then
       if(present(DstArray)) then
          request%DstArray => DstArray
          __ASSERT(all(shape(DstArray)==(/ request%IM0     , request%JM0     /)), 'inconsistent shape')
       else
          allocate(request%DstArray(request%IM0 , request%JM0 ),stat=STATUS)
          __VERIFY(STATUS)
       end if
    else
       __FAIL( 'unsupported action')
    end if
...
!------
    ...
    __ASSERT(.not.associated(GlobArray), 'GlobalArray already associated')
...
!------
    ...
    if(HaveGlobal) then
       __ASSERT(size(GlobArray,3)==NC, 'inconsisntent rank')
...
!--BCAST -----------------
  ...
  subroutine MAPL_CommsBcastVM_STRING_0( vm, data, N, ROOT,RC)
    ...
    __ASSERT(slen <= N, 'exceeded string length')
  ...
  subroutine MAPL_BcastShared_1DR8(VM, Data, N, Root, RootOnly, rc)
    ...
    if(.not.MAPL_ShmInitialized) then
       ...
       call MAPL_CommsBcast(vm, DATA=Data, N=N, ROOT=Root, __RC)
    else
       call MAPL_SyncSharedMemory(__RC)
       call MAPL_BroadcastToNodes(Data, N=N, ROOT=Root, __RC)
       call MAPL_SyncSharedMemory(__RC)
    endif
