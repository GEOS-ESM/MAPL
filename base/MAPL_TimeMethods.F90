...
contains
  ...
  function define_time_variable(this,rc) result(v)
    ...
    __ASSERT(this%frequency/=TimeData_uninit_int,"Frequency component was not set before use.")
    ...
    case('minutes')
       if (this%integer_time) then
          __ASSERT(mod(this%frequency,60)==0,"Requested output frequency not representable as an integer minute")
       end if
       time_increment = packed_hms
    case('hours')
       if (this%integer_time) then
          __ASSERT(mod(this%frequency,3600)==0,"Requested output frequency not representable as an integer hour")
       end if
       time_increment = packed_hms
    case('days')
       if (this%integer_time) then
          __ASSERT(mod(this%frequency,86400)==0,"Requested output frequency not representable as an integer day")
       end if
       time_increment = this%frequency/86400
    case default
       __FAIL( 'Not supported yet')
    end select
  ...
  subroutine add_time_to_metadata(this,metadata,rc)
    ...
    if (this%is_initialized) then
       ...
       v = this%define_time_variable(__RC)
       call metadata%add_variable('time',v,__RC)
    end if
    __RETURN(__SUCCESS)
  ...
  function parse_time_string(timeUnits,rc) result(time)
    ...
    if (firstdash .LE. 0 .OR. lastdash .LE. 0) then
       __FAIL('time string is not a valid format')
    endif
    ypos(2) = firstdash - 1
    mpos(1) = firstdash + 1
    ypos(1) = ypos(2) - 3
