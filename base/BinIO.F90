...
!---------------------------------------------------------------------------
  SUBROUTINE FREE_FILE(UNIT, RC)
    ...
    if(UNIT < 0) then
      ...
      __ASSERT(-UNIT<=LAST_UNIT, 'illegal io unit')
      __ASSERT(MTAKEN(-UNIT), 'illegal io unit')
...
! Local vars
    ...
    if(present(NAME)) then
       DOIT = ITEMNAMES==NAME
       __ASSERT(count(DOIT)/=0, 'cont(doit) must be > 0')
    else
       DOIT = .true.
    endif
...
!ALT          else
!ALT             __FAIL('failed mapl_statevarread')
...
!---------------------------
  ...
  subroutine MAPL_BundleRead(UNIT,BUNDLE, ARRDES, BOOTSTRAPABLE, RC)
    ...
    __VERIFY(STATUS)
    __ASSERT(N==fieldCount, 'inconsistent fieldCount')
...
! Local variables
    ...
    if(present(arrdes)) then
       __ASSERT(present(mask), 'mask must be present if arrdes is present')
    ...
    if (mypeRd <= nrdrs-1) write(*,'(5i)') mypeRd, IM_WORLD, first, last, Rsize
       ...
       if(arrdes%readers_comm /= MPI_COMM_NULL) then
          ...
          __VERIFY(STATUS)
          __ASSERT(cnt == numread, 'inconsistent numread')
#ifdef DEBUG_MPIIO
          write(*,'(3i,1f)') IM_WORLD, loffset, numread, VAR(1)
#endif

          __ASSERT( (lbound(mask,1) <= first), 'location not in bounds')
          __ASSERT( (ubound(mask,1) >= last ), 'location not in bounds')
          msk = mask(first:last)
    ...
    elseif(unit < 0) then
       ...
       __ASSERT(-UNIT<=LAST_UNIT, 'illegal unit')
       munit => MEM_units(-unit)
       munit%prevrec = munit%prevrec + 1
       __ASSERT(associated(munit%Records(munit%prevrec)%R4_1), 'unassociated pointer')
       __ASSERT(size(A)==size(munit%Records(munit%prevrec)%R4_1), 'inconsistent array size')
...
!          __VERIFY(STATUS)
    ...
    elseif(unit < 0) then
      ...
      munit => MEM_units(-unit)
      munit%prevrec = munit%prevrec + 1
      __ASSERT(associated(munit%Records(munit%prevrec)%R4_2), 'pointer not associated')
      __ASSERT(size(A)==size(munit%Records(munit%prevrec)%R4_2), 'inconsistent array size')
...
! Local variables
    ...
    elseif(unit < 0) then
       ...
       munit => MEM_units(-unit)
       munit%prevrec = munit%prevrec + 1
       __ASSERT(associated(munit%Records(munit%prevrec)%R8_1), 'pointer not associated')
       __ASSERT(size(A)==size(munit%Records(munit%prevrec)%R8_1), 'inconsistent array size')
...
!          __VERIFY(STATUS)
    ...
    elseif(unit < 0) then
      ...
      munit => MEM_units(-unit)
      munit%prevrec = munit%prevrec + 1
      __ASSERT(associated(munit%Records(munit%prevrec)%R8_2), 'pointer not associated')
      __ASSERT(size(A)==size(munit%Records(munit%prevrec)%R8_2), 'array size mismatch')
...
! Local variables
  ...
  itime_beg = MPI_Wtime()
    ...
    if(present(arrdes)) then
       ...
       if(present(mask)) then
          __ASSERT(JM_WORLD==size(A,2), 'inconsistent array shape')
...
!---------------------------
    ...
    ! --------------------------------------------------------------------------
        ...
        NFLD = size(NAMES)
        __ASSERT(NFLD>0, 'NFLD must be > 0')
