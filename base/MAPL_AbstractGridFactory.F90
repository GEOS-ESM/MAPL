...
contains
   ...
   function make_grid(this, unusable, force_new_grid, rc) result(grid)
      ...
      if (new_grid) then
         grid = this%make_new_grid(__RC)
      else
         if (allocated(this%grid)) then
            grid = this%grid
         else
            this%grid = this%make_new_grid(__RC)
            grid = this%grid
         end if
      end if
   ...
   subroutine spherical_to_cartesian_2d_real32(this,u,v,xyz,basis,unusable,rc)
      ...
      im = size(u,1)
      jm = size(u,2)
      __ASSERT(im == size(v,1), 'u-v shape mismatch for IM')
      __ASSERT(jm == size(v,2), 'u-v shape mismatch for JM')
      __ASSERT(3 == size(xyz,1), 'u-v shape mismatch (LM=1)')
      __ASSERT(1 == size(xyz,2), 'u-xyz shape mismatch (LM=1)')
      __ASSERT(im == size(xyz,3), 'u-xyz shape mismatch for IM')
      __ASSERT(jm == size(xyz,4), 'u-xyz shape mismatch for JM')
   ...
   subroutine spherical_to_cartesian_3d_real32(this,u,v,xyz,basis,unusable,rc)
      ...
      __ASSERT(im == size(v,1), 'u-v shape mismatch for IM')
      __ASSERT(jm == size(v,2), 'u-v shape mismatch for JM')
      __ASSERT(km == size(v,3), 'u-v shape mismatch for LM')
      __ASSERT(3 == size(xyz,1), 'u-xyz shape mismatch for')
      __ASSERT(km == size(xyz,2), 'u-xyz shape mismatch for LM')
      __ASSERT(im == size(xyz,3), 'u-xyz shape mismatch for IM')
      __ASSERT(jm == size(xyz,4), 'u-xyz shape mismatch for JM')
      basis_vectors => this%get_basis(basis,rc=status)
      __VERIFY(status)
   ...
   subroutine cartesian_to_spherical_2d_real32(this,xyz,u,v,basis,unusable,rc)
      ...
      __ASSERT(im == size(v,1), 'u-v shape mismatch for IM')
      __ASSERT(jm == size(v,2), 'u-v shape mismatch for JM')
      __ASSERT(3 == size(xyz,1), 'u-v shape mismatch')
      __ASSERT(1 == size(xyz,2), 'u-v shape mismatch')
      __ASSERT(im == size(xyz,3), 'u-v shape mismatch for IM')
      __ASSERT(jm == size(xyz,4), 'u-v shape mismatch for JM')
   ...
   subroutine cartesian_to_spherical_3d_real32(this,xyz,u,v,basis,unusable,rc)
      ...
      __ASSERT(jm == size(v,2), 'u-v shape mismatch for JM')
      __ASSERT(3 == size(xyz,1), 'u-v shape mismatch (LM=1)')
      __ASSERT(km == size(xyz,2), 'u-xyz shape mismatch (LM=1)')
      __ASSERT(im == size(xyz,3), 'u-xyz shape mismatch for IM')
      __ASSERT(jm == size(xyz,4), 'u-xyz shape mismatch for JM')
      basis_vectors => this%get_basis(basis,rc=status)
      __VERIFY(status)
   ...
   function get_basis(this,basis,unusable,rc) result(basis_vectors)
      ...
      __ASSERT(allocated(this%grid), 'grid not allocated')
      select case (basis)
      case ('north-south')
   ...
   subroutine destroy(this, rc)
      ...
      call ESMF_GridDestroy(this%grid, noGarbage=.true., __RC)      
      __RETURN(__SUCCESS)
