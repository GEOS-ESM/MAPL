...
contains
   ...
   function create_basic_grid(this, unusable, rc) result(grid)
      ...
      call ESMF_AttributeSet(grid, 'GridType', 'Swath', __RC)
...
!!      call ESMF_VMGet(vm, mpiCommunicator=mpic, localPet=mypet, petCount=petCount, __RC)
      ...
      call MAPL_AllocateShared(arr_lon,[Xdim,Ydim],transroot=.true.,__RC)
      call MAPL_AllocateShared(arr_lat,[Xdim,Ydim],transroot=.true.,__RC)
...
!      call MPI_Barrier(mpic, status)
      ...
      call MAPL_BcastShared (VM, data=arr_lon, N=count, Root=MAPL_ROOT, RootOnly=.false., __RC)
      call MAPL_BcastShared (VM, data=arr_lat, N=count, Root=MAPL_ROOT, RootOnly=.false., __RC)
...
!      call MPI_Barrier(mpic, status)
      ...
      if(MAPL_ShmInitialized) then
         call MAPL_DeAllocNodeArray(arr_lon,__RC)
         call MAPL_DeAllocNodeArray(arr_lat,__RC)
      else
         deallocate(arr_lon)
         deallocate(arr_lat)
      end if
...
!      end if
   ...
   subroutine initialize_from_config_with_prefix(this, config, prefix, unusable, rc)
      ...
      call ESMF_ConfigGetAttribute(config, this%input_template, label=prefix//'GRID_FILE:', default='unknown.txt', __RC)
      call ESMF_ConfigGetAttribute(config, this%epoch, label=prefix//'Epoch:', default=300, __RC)
      call ESMF_ConfigGetAttribute(config, tmp,      label=prefix//'Epoch_init:', default='2006', __RC)
      __ASSERT (this%lm /= MAPL_UNDEFINED_INTEGER, 'LM: is undefined in swath grid')

      call lgr%debug(' %a  %a', 'CurrTime =', trim(tmp))


      if ( index(tmp, 'T') /= 0 .OR. index(tmp, '-') /= 0 ) then
         call ESMF_TimeSet(currTime, timeString=tmp, __RC)
      else
         read(tmp,'(i4,5i2)') yy,mm,dd,h,m,s
         call ESMF_Timeset(currTime, yy=yy, mm=mm, dd=dd, h=h, m=m, s=s, __RC)
      endif
      second = hms_2_s(this%Epoch)
      call ESMF_TimeIntervalSet(this%epoch_frequency, s=second, __RC)


      call ESMF_ConfigGetAttribute(config, value=STR1, default="", &
           label= prefix// 'obs_reftime:', __RC)
      __ASSERT (trim(STR1)/='', 'obs_reftime missing, critical for data with 5 min interval!')
      call ESMF_TimeSet(this%obsfile_start_time, timestring=STR1, __RC)
      ...
      call ESMF_ConfigGetAttribute(config, value=STR1, default="", &
           label= prefix// 'obs_frequency:', __RC)
      __ASSERT(STR1/='', 'fatal error: obs_frequency not provided in RC file')
      ...
      !
      call ESMF_VMGet(vm, mpiCommunicator=mpic, __RC)
      ...
      if (irank==0) then
         call ESMF_TimeIntervalSet(Toff, h=0, m=0, s=0, __RC)
         call Find_M_files_for_currTime (currTime, &
              ...
              T_offset_in_file_content = Toff,  __RC)
         this%M_file = M_file
         ...
         call time_esmf_2_nc_int (currTime, this%tunit, j0, __RC)
         ...
         if (jt1==jt2) then
            __FAIL('Epoch Time is too small, empty swath grid is generated, increase Epoch')
         endif

         jt1 = jt1 + 1               ! (x1,x2]  design
         this%epoch_index(3)= jt1
         this%epoch_index(4)= jt2
         __ASSERT( jt1 < jt2, 'Swath grid fail : epoch_index(3) > epoch_index(4)')
...
106      format (1x,a,2x,10i8)
   ...
   contains
      ...
      subroutine get_ims_from_file(values, file_name, n, rc)
         ...
         __ASSERT(FileExists, "File <"//trim(file_name)//"> not found")
         if (MAPL_AM_I_Root(VM)) then
            open(newunit=UNIT, file=trim(file_name), form="formatted", iostat=status )
            __VERIFY(STATUS)
            read(UNIT,*) total
            __ASSERT(total == n, trim(file_name) // " n is different from total")
            do i = 1,total
                read(UNIT,*) values(i)
            enddo
            close(UNIT)
   ...
   subroutine initialize_from_esmf_distGrid(this, dist_grid, lon_array, lat_array, unusable, rc)
      ...
      __FAIL ('stop: not implemented: subroutine initialize_from_esmf_distGrid')
...
! from tclune: This needs thought. I suspect we want something that indicates this is a swath grid.
   ...
   subroutine init_halo(this, unusable, rc)
      ...
      class (KeywordEnforcer), optional, intent(in) :: unusable
      integer, optional, intent(out) :: rc
      __FAIL('Stop: subroutine init_halo is not needed for SwathGridFactory')
   ...
   subroutine halo(this, array, unusable, halo_width, rc)
      ...
      integer, optional, intent(out) :: rc
      __FAIL( 'Stop: subroutine halo is not needed for SwathGridFactory')
   end subroutine halo
   ...
   subroutine get_xy_subset(this, interval, xy_subset, rc)
      ...
      if (irank==0) then
         ...
         ! this%t_alongtrack
         !
         call time_esmf_2_nc_int (T1, this%tunit, i1, __RC)
         call time_esmf_2_nc_int (T2, this%tunit, i2, __RC)
    ...
    subroutine get_obs_time(this, grid, obs_time,  rc)
      ...
      call MAPL_grid_interior(grid, i_1, i_n, j_1, j_n)
      call MAPL_AllocateShared(arr_time,[Xdim,Ydim],transroot=.true.,__RC)
      call MAPL_SyncSharedMemory(__RC)
...
!         write(6,'(11x,100E12.5)')  arr_time(::5,189)
      ...
      call MAPL_BcastShared (VM, data=arr_time, N=count, Root=MAPL_ROOT, RootOnly=.false., __RC)
...
!      call MPI_Barrier(mpic, status)
      ...
      if(MAPL_ShmInitialized) then
         call MAPL_DeAllocNodeArray(arr_time,__RC)
      else
         deallocate(arr_time)
      end if
