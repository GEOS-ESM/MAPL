...
contains
  ...
  function get_filename_from_template (time, file_template, rc) result(filename)
    ...
    __FAIL ('DO not use get_filename_from_template')
    call ESMF_time_to_two_integer(time, itime, __RC)
    print*, 'two integer time, itime(:)', itime(1:2)
    nymd = itime(1)
    nhms = itime(2)
    call fill_grads_template ( filename, file_template, &
         experiment_id='', nymd=nymd, nhms=nhms, __RC )
  ...
  subroutine time_real_to_ESMF (times_R8_1d, times_esmf_1d, datetime_units, rc)
    ...
    do i=1, len
       ...
       call convert_NetCDF_DateTime_to_ESMF(int_time, datetime_units, interval, &
            time0, time=time1, time_unit=tunit, __RC)
  ...
  subroutine time_ESMF_to_real (times_R8_1d, times_esmf_1d, datetime_units, rc)
    ...
    do i=1, len
       ...
       case ('days')
          call ESMF_TimeIntervalGet(t_interval,d_r8=times_R8_1d(i),__RC)
       case ('hours')
          call ESMF_TimeIntervalGet(t_interval,h_r8=times_R8_1d(i),__RC)
       case ('minutes')
          call ESMF_TimeIntervalGet(t_interval,m_r8=times_R8_1d(i),__RC)
       case ('seconds')
          call ESMF_TimeIntervalGet(t_interval,s_r8=times_R8_1d(i),__RC)
       case default
  ...
  subroutine create_timeunit (time, datetime_units, input_unit, rc)
    ...
    call ESMF_timeget (time, timestring=string, __RC)
  ...
  subroutine reset_times_to_current_day(current_time, times_1d, rc)
    ...
    call ESMF_TimeGet(current_time,yy=year,mm=month,dd=day,__RC)
    do i=1,size(times_1d)
       call ESMF_TimeGet(times_1d(i),yy=yp,mm=mp,dd=dp,h=h,m=m,s=s,ms=ms,us=us,ns=ns,__RC)
       call ESMF_TimeSet(times_1d(i),yy=year,mm=month,dd=day,h=h,m=m,s=s,ms=ms,us=us,ns=ns,__RC)
    enddo
    __RETURN(__SUCCESS)
...
!    print*, '1st n1, n2', n1, n2
    ...
    __ASSERT ( M < size(filenames) , 'code crash, number of files exceeds upper bound')
    __ASSERT (M/=0, 'M is zero, no files found for currTime')
  ...
  subroutine read_M_files_4_swath ( filenames, Xdim, Ydim, &
    ...
    !__ s1. get Xdim Ydim
    M = size(filenames)
    __ASSERT(M/=0, 'M is zero, no files found')
    ...
    do i = 1, M
       ...
       CALL get_ncfile_dimension(filename, nlon=nlon, nlat=nlat, &
            key_lon=index_name_lon, key_lat=index_name_lat, __RC)
       nlons(i)=nlon
       nlats(i)=nlat
       jx=jx+nlat
    ...
    if ( present(Tfilter) .AND. Tfilter ) then
       if ( .not. (present(time) .AND. present(lon) .AND. present(lat)) ) then
          __FAIL('when Tfilter present, time/lon/lat must also present')
       end if
       ...
       do i = 1, M
          ...
          call get_var_from_name_w_group (var_name_time, time_loc_R8, filename, __RC)
!!          write(6,*) 'af ith, filename', i, trim(filename)
       ...
       do i = 1, M
          ...
          call get_var_from_name_w_group (var_name_lon, lon_loc, filename, __RC)
          allocate (lat_loc(nlon, nlat))
          call get_var_from_name_w_group (var_name_lat, lat_loc, filename, __RC)
          !
          do j=1, nlat
             !
             ! -- filter, e.g., eliminate -9999
       ...
       do i = 1, M
          ...
          if (present(var_name_time).AND.present(time)) then
             call get_var_from_name_w_group (var_name_time, time(1:nlon,jx+1:jx+nlat), filename, __RC)
          end if
          if (present(var_name_lon).AND.present(lon)) then
             call get_var_from_name_w_group (var_name_lon, lon(1:nlon,jx+1:jx+nlat), filename, __RC)
          end if
          if (present(var_name_lat).AND.present(lat)) then
             call get_var_from_name_w_group (var_name_lat, lat(1:nlon,jx+1:jx+nlat), filename, __RC)
          end if
  ...
  function get_filename_from_template_use_index (obsfile_start_time, obsfile_interval, &
    ...
    __ASSERT ( j==0 .OR. allow_wild_char, "* is not allowed in template")
    call fill_grads_template ( filename, file_template, &
         experiment_id='', nymd=nymd, nhms=nhms, __RC )
    if (j==0) then
       ! exact file name
  ...
  subroutine get_var_from_name_w_group (var_name, var2d, filename, rc)
    ...
    ! ncid2:  grp2
    !
    call check_nc_status(nf90_open(filename, NF90_NOWRITE, ncid), __RC)
    ncid_final = ncid
    if ( found_group ) then
       call check_nc_status(nf90_inq_ncid(ncid, grp1, ncid1), __RC)
       ncid_final = ncid1
       if (j>0) then
          call check_nc_status(nf90_inq_ncid(ncid1, grp2, ncid2), __RC)
          ncid_final = ncid2
       endif
    else
!!       print*, 'no grp name'
    ...
    call check_nc_status(nf90_inq_varid(ncid_final, short_name, varid), __RC)
!!    write(6,*) 'ncid, short_name, varid', ncid, trim(short_name), varid
    call check_nc_status(nf90_get_var(ncid_final, varid, var2d), __RC)
  ...
  subroutine sort_three_arrays_by_time(U,V,T,rc)
    ...
    __ASSERT (size(U)==size(V), 'U,V different dimension')
    __ASSERT (size(U)==size(T), 'U,T different dimension')
    len = size (T)
  ...
  subroutine sort_index (X, IA, rc)
    ...
    __ASSERT (size(X)==size(IA), 'X and IA (its index) differ in dimension')
    len = size (X)
    allocate (IX(len))
    do i=1, len
       IX(i)=X(i)
  ...
  function union_platform(a, b, rc)
    ...
    union_platform = copy_platform_nckeys(a, __RC)
...
121   format (2x, a,10(2x,i8))
  ...
  subroutine fglob(search_name, filename, rc)     ! give the last name
    ...
    if (lenmax < slen) then
       ...
       __FAIL ('PATHLEN is greater than filename_max_char_len')
       STOP 'lenmax < slen'
    end if
