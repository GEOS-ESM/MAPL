...
contains
  ...
  subroutine MAPL_ease_convert (EASELabel, lat, lon, r, s, rc)   ! note odd/reversed order of (lat,lon) and (r,s)
    ...
    if (     index(EASELabel,'M36') /=0 ) then
       ...
       __FAIL("MAPL_ease_convert(): unknown grid projection and resolution: "//trim(EASELabel)//"  STOPPING.")
    endif
    ...
    if(     index(EASELabel,'EASEv2') /=0) then
       ...
       __FAIL("MAPL_ease_convert(): unknown grid version: "//trim(EASELabel)//"  STOPPING.")
    endif
  ...
  subroutine MAPL_ease_inverse (EASELabel, r, s, lat, lon, rc)   ! note odd/reversed order of (r,s) and (lat,lon)
    ...
    if (     index(EASELabel,'M36') /=0 ) then
       ...
       __FAIL("MAPL_ease_inverse(): unknown grid projection and resolution: "//trim(EASELabel)//"  STOPPING.")
    endif

    if(     index(EASELabel,'EASEv2') /=0) then
       call easeV2_inverse(grid,r,s,lat,lon, __RC)
    else if(index(EASELabel,'EASEv1') /=0) then
       call easeV1_inverse(grid,r,s,lat,lon, __RC)
    else
       __FAIL("MAPL_ease_inverse(): unknown grid version: "//trim(EASELabel)//"  STOPPING.")
    endif
  ...
  subroutine MAPL_ease_extent (EASELabel, cols, rows, cell_area, ll_lon, ll_lat, ur_lon, ur_lat, rc)
    ...
    if (     index(EASELabel,'M36') /=0 ) then
       ...
       __FAIL("MAPL_ease_extent(): unknown grid projection and resolution: "//trim(EASELabel)//"  STOPPING.")
    endif

    if(     index(EASELabel,'EASEv2') /=0) then

       call easeV2_get_params(grid, map_scale_m, cols, rows, r0, s0, __RC)

       if(present(cell_area)) cell_area = map_scale_m**2

    else if(index(EASELabel,'EASEv1') /=0) then

       call easeV1_get_params(grid, CELL_km, cols, rows, r0, s0, Rg, __RC)

       if(present(cell_area)) cell_area = CELL_km**2 * 1000. * 1000.

    else
       __FAIL("MAPL_ease_extent(): unknown grid version: "//trim(EASELabel)//"  STOPPING.")
    endif
    ...
    if (present(ll_lat))  call MAPL_ease_inverse(EASElabel, 0., rows-0.5, ll_lat, tmplon, __RC)
    if (present(ur_lat))  call MAPL_ease_inverse(EASElabel, 0.,     -0.5, ur_lat, tmplon, __RC)
  ...
  subroutine easeV1_get_params( grid, CELL_km, cols, rows, r0, s0, Rg, rc )
    ...
    if ((grid(1:1).eq.'N').or.(grid(1:1).eq.'S')) then
       ...
       __FAIL('easeV1_get_params(): polar projections not implemented yet')
       ...
       if      (grid .eq. 'M36') then ! SMAP 36 km grid
          ...
          __FAIL( 'easeV1_get_params(): unknown resolution: ' // grid)
       endif

    else
       __FAIL('easeV1_get_params(): unknown projection: '// grid)
    endif
  ...
  subroutine easeV2_convert (grid, lat, lon, col_ind, row_ind, rc)
    ...
    if      (grid(1:1).eq.'M') then
       ...
       __FAIL('EASEv2_convert(): Polar projections not implemented yet')
  ...
  subroutine easeV2_inverse (grid, r, s, lat, lon, rc)
    ...
    if      (grid(1:1).eq.'M') then
       ...
       __FAIL('EASEv2_inverse(): Polar projections not implemented yet')
  ...
  subroutine easeV2_get_params( grid, map_scale_m, cols, rows, r0, s0, rc)
    ...
    if (grid(1:1).eq.'M') then
       ...
       if      (grid .eq. 'M36') then      ! SMAP 36 km grid
          ...
          __FAIL('easeV2_get_params(): unknown resolution: '//grid)

       endif

    else if ((grid(1:1).eq.'N').or.(grid(1:1).eq.'S')) then

       __FAIL('easeV2_get_params(): Polar projections not implemented yet')

    else

       __FAIL('easeV2_get_params(): unknown projection: '// grid)
  ...
  function MAPL_get_ease_gridname_by_cols(cols, rc) result(name)
    ...
    !   must be maintained manually if more EASE grids are added.
      ...
      case (34668)
        name = 'EASEv1_M01'
      case default
        __FAIL('EASEGridFactory does not support this solution')
      end select
      __RETURN(__SUCCESS)
  end function
