...
contains
   ...
   logical function is_valid_prototype(this, prototype_name, unusable, rc)
      ...
      if (.not. initialized) then
         call initialize_prototypes(this, __RC)
         initialized = .true.
      end if
   ...
   function make_grid_from_config(this, config, unusable, prefix, rc) result(grid)
      ...
      __ASSERT(status==0,'label ['//label//'] not found')
   ...
   subroutine destroy_grid(this, grid, unusable, rc)
      ...
      call ESMF_AttributeGet(grid, factory_id_attribute, id, __RC)
      factory => this%factories%at(id)
      call factory%destroy(__RC)
   ...
   subroutine delete(this, grid, unusable, rc)
      ...
      if (.not. this%keep_grids) then
         ...
         __ASSERT(status==0,'failed to destroy grid')
      end if
   ...
   function make_factory_from_file(this, file_name, unused, force_file_coordinates, rc) result(factory)
      ...
      if (file_metadata%has_attribute('grid_type')) then
         ...
         class default
            __FAIL("grid_type attribute must be stringwrap") 
         end select
