...
contains
   ...
   logical function supports(spec, unusable, rc)
      ...
      call MAPL_GridGet(spec%grid_in, localCellCountPerDim=counts_in, __RC)
      call MAPL_GridGet(spec%grid_out, localCellCountPerDim=counts_out, __RC)
   ...
   subroutine initialize_subclass(this, unusable, rc)
     ...
     associate (grid_in => spec%grid_in, grid_out => spec%grid_out)
       ...
       associate (IM_in => this%IM_in, JM_in => this%JM_in, IM_out => this%IM_out, JM_out => this%JM_out)
         ...
         call MAPL_GridGet(grid_in, localCellCountPerDim=counts, __RC)
         IM_in = counts(1)
         JM_in = counts(2)
         
         call MAPL_GridGet(grid_out, localCellCountPerDim=counts, __RC)
         IM_out = counts(1)
         JM_out = counts(2)

         __ASSERT(mod(IM_in, IM_out) == 0, 'grids not nested')
         __ASSERT(mod(JM_in, JM_out) == 0, 'grids not nested')
         __ASSERT((IM_in / IM_out) == (JM_in / JM_out), 'inconsistent aspect ratio')
   ...
   subroutine regrid_vector_2d_real32(this, u_in, v_in, u_out, v_out, rotate, rc)
      ...
      __UNUSED_DUMMY(rotate)

      __ASSERT(size(u_in,1) == this%IM_in, 'mismatch in IM for input')
      __ASSERT(size(v_in,1) == this%IM_in, 'mismatch in IM for input')
      __ASSERT(size(u_in,2) == this%JM_in, 'mismatch in JM for input')
      __ASSERT(size(v_in,2) == this%JM_in, 'mismatch in JM for input')
      __ASSERT(size(u_out,1) == this%IM_out, 'mismatch in IM for output')
      __ASSERT(size(v_out,1) == this%IM_out, 'mismatch in IM for output')
      __ASSERT(size(u_out,2) == this%JM_out, 'mismatch in JM for output')
      __ASSERT(size(v_out,2) == this%JM_out, 'mismatch in JM for output')
   ...
   subroutine regrid_vector_3d_real32(this, u_in, v_in, u_out, v_out, rotate, rc)
      ...
      __ASSERT(size(u_in,3) == size(u_out,3), 'mismatch in number of levels')
      __ASSERT(size(v_in,3) == size(v_out,3), 'mismatch in number of levels')
      __ASSERT(size(u_in,3) == size(v_in,3), 'mismatch in number of levels')
