...
! !ARGUMENTS:
      ...
      call ESMF_ConfigGetAttribute (CF, &
         ...
         default=DEFAULT_ORBIT_ECCENTRICITY, __RC)

      call ESMF_ConfigGetAttribute (CF, &
         OB, label="OBLIQUITY:", &
         default=DEFAULT_ORBIT_OBLIQUITY, __RC)

      call ESMF_ConfigGetAttribute (CF, &
         PER, label="PERIHELION:", &
         default=DEFAULT_ORBIT_PERIHELION, __RC)

      call ESMF_ConfigGetAttribute (CF, &
         EQNX, label="EQUINOX:", &
         default=DEFAULT_ORBIT_EQUINOX, __RC)
      ...
      call ESMF_ConfigGetAttribute (CF, &
         ...
         default=DEFAULT_ORB2B_YEARLEN, __RC)

      ! Reference date and time for orbital parameters
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_REF_YYYYMMDD, label="ORB2B_REF_YYYYMMDD:", &
         default=DEFAULT_ORB2B_REF_YYYYMMDD, __RC)
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_REF_HHMMSS, label="ORB2B_REF_HHMMSS:", &
         default=DEFAULT_ORB2B_REF_HHMMSS, __RC)

      ! Orbital eccentricity at reference date
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_ECC_REF, label="ORB2B_ECC_REF:", &
         default=DEFAULT_ORB2B_ECC_REF, __RC)

      ! Rate of change of orbital eccentricity per Julian century
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_ECC_RATE, label="ORB2B_ECC_RATE:", &
         default=DEFAULT_ORB2B_ECC_RATE, __RC)

      ! Earth's obliquity (axial tilt) at reference date [degrees]
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_OBQ_REF, label="ORB2B_OBQ_REF:", &
         default=DEFAULT_ORB2B_OBQ_REF, __RC)

      ! Rate of change of obliquity [degrees per Julian century]
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_OBQ_RATE, label="ORB2B_OBQ_RATE:", &
         default=DEFAULT_ORB2B_OBQ_RATE, __RC)

      ! Longitude of perihelion at reference date [degrees]
      !   (from March equinox to perihelion in direction of earth's motion)
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_LAMBDAP_REF, label="ORB2B_LAMBDAP_REF:", &
         default=DEFAULT_ORB2B_LAMBDAP_REF, __RC)

      ! Rate of change of LAMBDAP [degrees per Julian century]
      !   (Combines both equatorial and ecliptic precession)
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_LAMBDAP_RATE, label="ORB2B_LAMBDAP_RATE:", &
         default=DEFAULT_ORB2B_LAMBDAP_RATE, __RC)

      ! March Equinox date and time
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_EQUINOX_YYYYMMDD, label="ORB2B_EQUINOX_YYYYMMDD:", &
         default=DEFAULT_ORB2B_EQUINOX_YYYYMMDD, __RC)
      call ESMF_ConfigGetAttribute (CF, &
         ORB2B_EQUINOX_HHMMSS, label="ORB2B_EQUINOX_HHMMSS:", &
         default=DEFAULT_ORB2B_EQUINOX_HHMMSS, __RC)
...
! !ARGUMENTS:
       ...
       __ASSERT(MAPL_SunOrbitCreated(ORBIT,RC=STATUS),'MAPL_SunOrbit not yet created!')
...
!   Begin
      ...
      __FAIL(" pmn: this routine is not up to date, is it even used anywhere?")
...
!
   ...
   if(present(HK)) then
      if(size(HK)==8) then ! Chou Bands
         ...
         __ASSERT(abs(1.0-sum(HK))<1.e-4,'Chou Solar band weightings do not sum to unity!')
      else
         __FAIL('HK: Solar band weightings only available for Chou')
      endif
   end if
...
!
   subroutine MAPL_SunGetSolarConstantFromNetcdfFile(CLOCK,fileName,SC,HK,MESOPHOT,JCALC4,rc)
      ...
      ! ------------
      if (PRESENT(HK)) then
         __ASSERT(ABS(1.0-SUM(HK)) < 1.E-4,'needs informative message')
      end if
   ...
   subroutine MAPL_SunGetSolarConstantFromNRLFile(CLOCK,filename_in,SC,MG,SB,PersistSolar,rc)
      ...
      CREATE_TABLE: if (.not. TableCreated) then
         ...
         if (amIRoot) then
            ...
            __ASSERT(status==0,'Could not find NRL data file '// trim(filename ))
         ...
         ! -----------------------------
         call MAPL_CommsBcast(vm, DATA=numlines, N=1, ROOT=0, __RC)

         ! Allocate our arrays on all processes
         ! ------------------------------------

         allocate(yearTable(numlines), source=0,   __STAT)
         allocate(doyTable(numlines),  source=0,   __STAT)
         allocate(tsi(numlines),       source=0.0, __STAT)
         allocate(mgindex(numlines),   source=0.0, __STAT)
         allocate(sbindex(numlines),   source=0.0, __STAT)
         ...
         if (amIRoot) then
            ...
            __ASSERT(size(yearTable) == numlines,"Inconsistency in NRL number of lines")
         ...
         call MAPL_CommsBcast(vm, DATA=yearTable, N=numlines, ROOT=0, __RC)
         call MAPL_CommsBcast(vm, DATA=doyTable,  N=numlines, ROOT=0, __RC)
         call MAPL_CommsBcast(vm, DATA=tsi,       N=numlines, ROOT=0, __RC)
         call MAPL_CommsBcast(vm, DATA=mgindex,   N=numlines, ROOT=0, __RC)
         call MAPL_CommsBcast(vm, DATA=sbindex,   N=numlines, ROOT=0, __RC)
      ...
      ! -----------------------------------------------
      call ESMF_TimeIntervalSet(oneDayInterval, D=1, __RC)
      ...
      ! Get the DOYs
      ! ------------
      call ESMF_TimeGet( prevNoon, YY = prevNoonYear, dayOfYear = prevDOY, __RC)
      call ESMF_TimeGet( nextNoon, YY = nextNoonYear, dayOfYear = nextDOY, __RC)
      ...
      ! --------------------------------------------------
      call ESMF_TimeIntervalGet(intToNextNoon, d_r8=days_r8, __RC)
      FAC = real(days_r8)
...
!     Begin
      ...
      __ASSERT(MAPL_SunOrbitCreated(ORBIT),'MAPL_SunOrbit not yet created!')
...
!EOPI
      ...
      __ASSERT(OBQ >= 0, 'Obliquity less than 0 detected!')
      __ASSERT(OBQ < MAPL_PI, 'Obliquity greater than pi detected!')
...
!     Locals
      ...
      if (present(TIME)) then
         ...
         if (present(CLOCK)) then
            call ESMF_ClockGet (      CLOCK, CURRTIME=T, __RC)
         else
            call ESMF_ClockGet (ORBIT%CLOCK, CURRTIME=T, __RC)
         end if
      end if
      ...
      if (do_EOT) then
         ...
         if (ORBIT%ANAL2B) then
            ...
            ! include time variation in orbit from reference time
            call ESMF_TimeIntervalGet (T - ORBIT%ORB2B_TIME_REF, d_r8=days, __RC)
            ECC     = ORBIT%ORB2B_ECC_REF     + days * ORBIT%ORB2B_ECC_RATE
            OBQ     = ORBIT%ORB2B_OBQ_REF     + days * ORBIT%ORB2B_OBQ_RATE
            LAMBDAP = ORBIT%ORB2B_LAMBDAP_REF + days * ORBIT%ORB2B_LAMBDAP_RATE
            ! derived quantities
            ...
            EAFAC = sqrt(OMECC/OPECC)
            ! time interval since perhelion in days
            call ESMF_TimeIntervalGet (T - ORBIT%ORB2B_TIME_PERI, d_r8=days, __RC)
            ! mean anomaly
            MA = ORBIT%ORB2B_OMG0 * days
            ! eccentric anomaly
            call invert_Keplers_Newton (MA,ECC,EA,dE,nits)
