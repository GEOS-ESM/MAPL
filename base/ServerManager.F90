...
contains
   ...
   subroutine initialize(this, comm, unusable, application_size, nodes_input_server, nodes_output_server,&
      ...
      if (oserver_type_ == "multilayer" .or. oserver_type_ == 'multigroup') then
         if (nodes_out(1) == 0) then
            __ASSERT(npes_out_backend >=2, "captain-soldier needs at least two backend")
         else
            __ASSERT(nodes_out(1)*npes_out_backend >=2, "captain-soldier needs at least two backend")
         endif
      endif
      if (oserver_type_ == "multicomm") then
         __ASSERT(npes_out_backend >=1, "needs at least one backend for multicomm server")
      endif
     ...
     this%split_comm = this%splitter%split(rc=status); __VERIFY(status)
        ...
        if ( trim(s_name) =='o_server'//trim(i_to_string(i)) ) then
           ...
           if (oserver_type_ == 'multicomm' ) then
              ...
              if (nodes_out(i) > 0 .and. this%o_server%node_num /= nodes_out(i)) then
                 __FAIL("Inconsistent output server number. " // "The requested "//i_to_string(nodes_out(i)) //" nodes for output server is different from available "//i_to_string(this%o_server%node_num)// " nodes")
              endif
           else
     ...
     if ( index(s_name, 'o_server') /=0 ) then
        call this%o_server%start(__RC)
     endif

     if ( index(s_name, 'i_server') /=0 ) then
        call this%i_server%start(__RC)
     endif
   ...
   subroutine finalize(this,rc)
      ...
      if (associated(this%i_server)) then
         call this%i_server%report_profile(__RC)
         deallocate(this%i_server)
      endif

      if (associated(this%o_server)) then
         call this%o_server%report_profile(__RC)
