...
contains
   ...
   subroutine fill_grads_template(output_string,template,unusable,experiment_id,nymd,nhms,time,preserve,rc)
      ...
      if (present(time)) then
         __ASSERT(.not.present(nymd),'can not specify an ESMF_Time and an integer time')
         __ASSERT(.not.present(nhms),'can not specify an ESMF_Time and an integer time')
         call ESMF_TimeGet(time,yy=year,mm=month,dd=day,h=hour,m=minute,s=second,rc=status)
         __VERIFY(status)
      end if
      if (present(nymd)) then
         __ASSERT(.not.present(time),'can not specify an ESMF_Time and an integer time')
         year = nymd/10000
         ...
         if (c0 == "%") then
            ...
            case("s")
               if (present(experiment_id)) then
                  ...
                  __FAIL("Using %s token with no experiment id")
               end if
            case("%")
               istp=2
            ...
            case default
               ...
               if (valid_token) then
                  ...
                  if (.not.preserve_) then
                     __ASSERT(present(nymd) .or. present(nhms) .or. present(time),'Using token with no time')
                  end if
                  ...
                  __FAIL("Invalid token in file template: "//c1//c2)
               end if
            end select
         else
            istp=1
         ...
         if (.not.skip_token(day,preserve)) then
            if (c2 == "3") then
               ...
               call ESMF_TimeSet(time, yy=year, mm=month, dd=day, &
                    calendar=gregorianCalendar, __RC)
               call ESMF_TimeGet(time, dayOfYear=doy, __RC)
               call ESMF_CalendarDestroy(gregorianCalendar)
               write(buffer,'(i3.3)')doy
            else
               __FAIL('Day of Year must be %D3')
            end if
         else
            buffer="%"//token
         end if
