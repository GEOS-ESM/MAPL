...
contains
   ...
   function new_MultiCommServer(server_comm, port_name, nwriter_per_node, rc) result(s)
      ...
      s_comm = s%splitter%split(rc=status); __VERIFY(status)
   ...
   contains
      ...
      subroutine start_back(rc)
         ...
         do while (.true.)
            ...
            __VERIFY(ierr)
            if (cmd == -1) exit
            call this%create_remote_win(__RC)
            call this%receive_output_data(__RC)
            call this%put_dataToFile(__RC)

            call this%clean_up(__RC)
        ...
        ! time to write file
         ...
         do while (msg_iter /= threadPtr%request_backlog%end())
            ...
            type is (CollectiveStageDataMessage)
               ...
               class default
                  __FAIL( "I expect localmemReference")
               end select
            ...
            class default
               __FAIL( "I expect CollectiveStageDataMessage")
            end select
            call msg_iter%next()
         enddo
