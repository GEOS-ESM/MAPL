...
contains
   ...
   function new_ServerThread(sckt, server, rc) result(s)
      ...
      call s%set_connection(sckt, __RC)
   ...
   subroutine init(this, sckt, server, rc)
      ...
      call this%set_connection(sckt, __RC)
   ...
   subroutine run(this, rc)
      ...
      if (associated(message)) then
         call message%dispatch(this, __RC)
         deallocate(message)
      end if
      __RETURN(__SUCCESS)
   end subroutine run
   ...
   subroutine run_done(this, rc)
      ...
      if ( .not. this%have_done) then
        call dMessage%dispatch(this, __RC)
        __RETURN(__SUCCESS)
      endif
   ...
   recursive subroutine handle_Done(this, message, rc)
      ...
      type is (PrefetchDataMessage)
         __FAIL( "please use done_prefetch")
         __RETURN(__SUCCESS)
      type is (CollectivePrefetchDataMessage)
         __FAIL( "please use done_collective_prefetch")
         __RETURN(__SUCCESS)
      type is (StageDataMessage)
         __FAIL( "please use done_stage")
         __RETURN(__SUCCESS)
      type is (CollectiveStageDataMessage)
         __FAIL( "please use done_collective_stage")
         __RETURN(__SUCCESS)
      class default
         __FAIL( "Wrong message type")
      end select
            ...
            if (request_iter == this%containing_server%prefetch_offset%end() .and. &
              ...
              ! get address where data should put
               ...
               address   = c_loc(i_ptr(offset+1))
                ! (2) read data
               call this%get_DataFromFile(q, address, __RC)
   ...
   subroutine handle_AddExtCollection(this, message, rc)
      ...
      connection=>this%get_connection()
      call connection%send(IdMessage(n),__RC)
   ...
   subroutine handle_PrefetchData(this, message, rc)
      ...
      connection=>this%get_connection()
      call connection%send(handshake_msg,__RC)
   ...
   subroutine get_DataFromFile(this,message,address, rc)
      ...
      formatter => collection%find(message%file_name, __RC)
      ...
      class default
        __FAIL( "wrong PrefetchDataMessage type")
      end select
...
!      if (product(count) /= product(file_data_reference%shape)) stop "memory size not match"
      ...
      case (0)
          ...
          case (pFIO_INT32)
              ...
              call formatter%get_var(message%var_name, values_int32_0d, __RC)
          case (pFIO_REAL32)
              call c_f_pointer(address, values_real32_0d)
              call formatter%get_var(message%var_name, values_real32_0d, __RC)
          case (pFIO_INT64)
              call c_f_pointer(address, values_int64_0d)
              call formatter%get_var(message%var_name, values_int64_0d, __RC)
          case (pFIO_REAL64)
              call c_f_pointer(address, values_real64_0d)
              call formatter%get_var(message%var_name, values_real64_0d, __RC)
          case default
              __FAIL( "Not supported type")
          end select
      case (1:)
          select case (message%type_kind)
          case (pFIO_INT32)
              call c_f_pointer(address, values_int32_1d, [product(count)])
              call formatter%get_var(message%var_name, values_int32_1d, start=start, count=count, __RC)
          case (pFIO_REAL32)
              call c_f_pointer(address, values_real32_1d, [product(count)])
              call formatter%get_var(message%var_name, values_real32_1d, start=start, count=count, __RC)
          case (pFIO_INT64)
              call c_f_pointer(address, values_int64_1d, [product(count)])
              call formatter%get_var(message%var_name, values_int64_1d, start=start, count=count, __RC)
          case (pFIO_REAL64)
              call c_f_pointer(address, values_real64_1d, [product(count)])
              call formatter%get_var(message%var_name, values_real64_1d, start=start, count=count, __RC)
          case default
              __FAIL( "Not supported type")
          end select
      end select
   ...
   subroutine handle_CollectiveStageData(this, message, rc)
      ...
      call this%insert_RequestHandle(message%request_id, handle, __RC)
   ...
   subroutine put_DataToFile(this, message, address, rc)
      ...
      formatter =>hist_collection%find(message%file_name, __RC)
      ...
      class default
        __FAIL( "wrong StageDataMessage type")
      end select
!      if (product(count) /= product(file_data_reference%shape)) stop "memory size not match"
      select case (size(count)) ! rank
      case (0)
          ...
          case (pFIO_INT32)
              call c_f_pointer(address, values_int32_0d)
              call formatter%put_var(message%var_name, values_int32_0d, __RC)
          case (pFIO_INT64)
              call c_f_pointer(address, values_int64_0d)
              call formatter%put_var(message%var_name, values_int64_0d, __RC)
          case (pFIO_REAL32)
              call c_f_pointer(address, values_real32_0d)
              call formatter%put_var(message%var_name, values_real32_0d, __RC)
          case (pFIO_REAL64)
              call c_f_pointer(address, values_real64_0d)
              call formatter%put_var(message%var_name, values_real64_0d, __RC)
          case default
              __FAIL( "not supported type")
          end select
      case (1:)
          ...
          case (pFIO_INT32)
              ...
              call formatter%put_var(message%var_name, values_int32_1d, start=start, count=count, __RC)
          case (pFIO_INT64)
              call c_f_pointer(address, values_int64_1d, [product(int(count, INT64))])
              call formatter%put_var(message%var_name, values_int64_1d, start=start, count=count, __RC)
          case (pFIO_REAL32)
              call c_f_pointer(address, values_real32_1d,[product(int(count, INT64))])
              call formatter%put_var(message%var_name, values_real32_1d, start=start, count=count, __RC)
          case (pFIO_REAL64)
              call c_f_pointer(address, values_real64_1d,[product(int(count, INT64))])
              call formatter%put_var(message%var_name, values_real64_1d, start=start, count=count, __RC)
          case default
              __FAIL( "not supported type")
          end select
       end select
  ...
  recursive subroutine handle_Done_collective_stage(this, message, rc)
      ...
      __ASSERT( associated(this%containing_server), "need server")

      call this%containing_server%create_remote_win(__RC)
      call this%containing_server%receive_output_data(__RC)
      call this%containing_server%put_dataToFile(__RC)
   ...
   recursive subroutine handle_Done_prefetch(this, message, rc)
      ...
      do while ( iter /= this%request_backlog%end())
         ...
         type is (PrefetchDataMessage)
             ...
             call this%get_DataFromFile(q,mem_data_reference%base_address, __RC)
   ...
   recursive subroutine handle_Done_collective_prefetch(this, message, rc)
      ...
      call this%containing_server%get_DataFromMem(multi_data_read, __RC)
   ...
   subroutine get_DataFromMem( this, multi_data_read, rc)
      ...
      do while (iter /= this%request_backlog%end())
         ...
         type is (CollectivePrefetchDataMessage)
           ...
           call mem_data_reference%fetch_data(offset_address,q%global_count,q%start-q%global_start+1, __RC)
           ...
           call this%request_backlog%erase(iter)
         class default
           __FAIL( "Message type should be CollectivePrefetchDataMessage ")
         end select
         iter = this%request_backlog%begin()
      enddo
