...
contains
   ...
   subroutine put_attributes(this, cf, varid, unusable, rc)
      ...
      do while (iter /= attributes%end())
         ...
         if (size(shp) > 0) then
           ...
           __ASSERT(associated(attr_values), "should have values")
           ...
           __ASSERT(associated(attr_value), "should have value")
   ...
   subroutine add_variable(this, cf, varname, unusable, rc)
      ...
      !$omp end critical
      __VERIFY(status)
      call this%def_variables(cf, varname=varname, __RC)
      !$omp critical
      status=nf90_enddef(this%ncid)
      !$omp end critical
      __VERIFY(status)
   ...
   subroutine def_variables(this, cf, unusable, varname, rc)
      ...
      do while (var_iter /= order%end())
         ...
         if (quantize_level /= 0) then
           ...
           __VERIFY(status)
#else
           __FAIL("netcdf was not built with quantize support")
#endif
         end if
         ...
         if (zstandard_level /= 0) then
           ...
           __VERIFY(status)
#else
           __FAIL("netcdf was not built with zstandard support")
#endif
         end if
   ...
   subroutine inq_variables(this, cf, unusable, rc)
      ...
      do varid = 1, nVariables
         ...
         if ( this%is_coordinate_dimension(trim(var_name))) then
            ...
            Fio_type = get_fio_type(xtype, rc=status); __VERIFY(status)
