...
contains
   ...
   subroutine receive_output_data(this, rc)
     ...
     if (allocated(ioserver_profiler)) call ioserver_profiler%start("receive_data")
     ...
     if (allocated(ioserver_profiler)) call ioserver_profiler%stop("receive_data")
     _RETURN(_SUCCESS)
   ...
   subroutine put_DataToFile(this, rc)
     ...
     if (allocated(ioserver_profiler)) call ioserver_profiler%start("write_data")
     ...
     do while (iter /= threadPtr%request_backlog%end())
        ...
        type is (CollectiveStageDataMessage)
           if (allocated(ioserver_profiler)) call ioserver_profiler%start("collection_"//i_to_string(q%collection_id))
           ...
           if (allocated(ioserver_profiler)) call ioserver_profiler%stop("collection_"//i_to_string(q%collection_id))
        end select
        call iter%next()
     ...
     !endif
     if (allocated(ioserver_profiler)) call ioserver_profiler%stop("write_data")
     _RETURN(_SUCCESS)
   ...
   subroutine create_remote_win(this, rc)
      ...
      if (allocated(ioserver_profiler)) call ioserver_profiler%start("create_shared_mem")
      ...
      if (allocated(ioserver_profiler)) call ioserver_profiler%stop("create_shared_mem")
      _RETURN(_SUCCESS)
