...
#define __UNUSED_DUMMY(x) if (.false.) then; associate (q____ => x); end associate; endif
...
contains
      ...
      subroutine fake_gmap(im)
      implicit none
      integer im
      PRINT*,'fake gmap ', im
      return
      end subroutine fake_gmap

!****6***0*********0*********0*********0*********0*********0**********72
      subroutine gmap4_logp(im, jm,          &
                            km, pe3d_m, q_m, &
                            kn, pe3d_n, q_n  )
!****6***0*********0*********0*********0*********0*********0**********72
...
!$omp  parallel do
!$omp& default (shared)
!$omp& private (i,j,k,pe1,pe2,dp1,dp2,q1)
!$omp& private (q2)
#endif
      do 2000 j=1,jm
...
! map u,v,q,oz
! ------------
      call mappm_ ( km, pe1, dp1, q1(1,1,1), kn, pe2, dp2, q2(1,1,1), im,  0, kord )
