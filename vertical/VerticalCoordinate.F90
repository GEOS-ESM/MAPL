...
contains
   ...
   function new_verticalCoordinate(metadata, var_name, rc) result(vertical_coord)
      ...
      var => metadata%get_variable(var_name, __RC)
      ...
      do while(iter /= dimensions%end())
         ...
         if (metadata%has_variable(dim_name)) then
            ...
            is_vertical_coord_var = detect_cf_vertical_coord_var(dim_var, __RC)
      ...
      if (lev_name /= '') then
         coord_var => metadata%get_coordinate_variable(lev_name, __RC)
         vertical_coord%levels = get_coords(coord_var,__RC) 
         ...
         has_pressure_units = safe_are_convertible(temp_units, 'hPa', __RC)
         ...
         has_height_units = safe_are_convertible(temp_units, 'm', __RC)
         if (has_height_units) then
            __ASSERT(allocated(vertical_coord%positive),"non pressure veritcal dimensional coordinates must have positive attribute")
            vertical_coord%level_units = temp_units
         ...
         if (coord_var%is_attribute_present("positive") .and. coord_var%is_attribute_present("formula_terms")) then
            ...
            if (standard_name == "atmosphere_hybrid_sigma_pressure_coordinate") then
               ...
               if (coord_var%is_attribute_present('bounds')) then
                  ...
                  var => metadata%get_variable(bounds_var, __RC)
                  formula_terms = var%get_attribute_string("formula_terms")
                  call parse_formula_terms(formula_terms, ps_name, ak_name, bk_name, __RC)
                  ...
                  call file_formatter%get_var(ak_name, temp_ak, __RC)
                  call file_formatter%get_var(bk_name, temp_bk, __RC)
               ...
               ! do we not have bounds, if so this is edge
                  ...
                  allocate(vertical_coord%bk(vertical_coord%num_levels+1))
                  call file_formatter%open(source_file, PFIO_READ, __RC)
                  call file_formatter%get_var(ak_name, vertical_coord%ak, __RC)
                  call file_formatter%get_var(bk_name, vertical_coord%bk, __RC)
               end if
            else
               __FAIL("unsupported hybrid vertical coordinate")
            end if
            __ASSERT(allocated(vertical_coord%surf_name), "PS was not found in the file")
    ...
    function detect_cf_vertical_coord_var(var, rc) result(is_vertical_coord_var)
       ...
       pressure_hpa = "Pa"
       has_positive = var%is_attribute_present("positive", __RC)
       has_units = var%is_attribute_present("units", __RC)
       has_pressure_units = .false.
       if (has_units) then
          units = var%get_attribute_string("units", __RC) 
          has_pressure_units = safe_are_convertible(units, pressure_hpa, __RC)
       end if
    ...
    function get_coords(coord_var, rc) result(coords)
       ...
       __ASSERT(associated(ptr),"coord variable coordinate data not found")
       select type (ptr)
       ...
       type is (integer(kind=INT32))
          coords=ptr
       class default
          __FAIL("unsupported coordinate variable type in ")
       end select
       __RETURN(__SUCCESS)
    ...
    function safe_are_convertible(from, to, rc) result(convertible)
       ...
       convertible = UDUNITS_are_convertible(unit1, unit2, __RC)
