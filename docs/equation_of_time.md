...
The earth rotates on its axis with a period $T_*$ called the *sidereal day* (after the Latin for
"star", since it is the rotation period of the earth with respect to distant stars). $T_*$ is
...
revolution, period $T_*$, an additional earth rotation of
$(T_{\mathcal{M}}-T_*) \, 2\pi/T_* = 2\pi/Y$ is required to "catch up with the moving sun", as
described earlier. Hence $T_{\mathcal{M}} - T_* = T_* / Y$ and so

$$
T_{\mathcal{M}} = T_* \, \frac{Y+1}{Y},
\tag{1}
$$
...
these units, the sidereal day $T_*$ is approximately 23h 56m 4s.
...
$$
M\!A(u) \equiv 2\pi\, (u - u_{\mathcal{P}}) / Y
\tag{6}
$$
...
need only to specify $M\!A(u_R)$.

To understand the mean anomaly $M\!A$, consider the standard two-body earth-sun problem in which the
earth ${\mathcal{E}}$ moves in an elliptical orbit about the sun ${\mathcal{S}}$ at one focus, all
in the *ecliptic plane*. The point on the ellipse closest to ${\mathcal{S}}$ is called the perihelion
${\mathcal{P}}$. Obviously, the center of the ellipse ${\mathcal{O}}$, the focus ${\mathcal{S}}$ and
the perihelion ${\mathcal{P}}$ are co-linear, the so-called major axis of the ellipse. Additionally,
let ${\mathcal{C}}$ be the circumscribing circle of the ellipse, with center ${\mathcal{O}}$ and
passing through ${\mathcal{P}}$ (and the corresponding aphelion ${\mathcal{A}}$). By Kepler's Second
Law, the sun-earth vector sweeps out equal areas in equal times, so the *fractional area* of the
elliptical sector ${\mathcal{P}}{\mathcal{S}}{\mathcal{E}}$ is a linear function of time, being zero
at perihelion and one a year later. Specifically, this fractional area is none other than the scaled
mean anomaly $M\!A(u) / (2\pi) = (u - u_{\mathcal{P}}) / Y$ from (6). Clearly $M\!A(u)$ can also
...
Note that while the angle $M\!A(u) = \angle{\mathcal{P}}{\mathcal{O}}{\mathcal{Q}}$ of this mean
earth at time $u$ is a linear function of time, the corresponding angle of the real earth, namely
$T\!A(u) \equiv \angle{\mathcal{P}}{\mathcal{S}}{\mathcal{E}}$, called the *true anomaly*, is a
non-linear function of time, since the real earth has a variable speed in its elliptical orbit, e.g.,
moving faster at perihelion, so that its *areal fraction* is linear in time. The relationship between
$M\!A(u)$ and $T\!A(u)$ is known from the orbital solution and will be discussed later. Finally, the
*ecliptic longitude of the earth*, $\lambda \equiv \angle\Upsilon{\mathcal{S}}{\mathcal{E}}$ is the
angle at the sun, measured in the same direction as the earth's motion, from the First Point of Aries
$\Upsilon$ to the earth. Then

$$
T\!A(u) \equiv \angle{\mathcal{P}}{\mathcal{S}}{\mathcal{E}}(u) =  \angle{\mathcal{P}}{\mathcal{S}}\Upsilon +  \angle\Upsilon{\mathcal{S}}{\mathcal{E}}(u) = \lambda(u) - \lambda_{\mathcal{P}},
$$
...
With this background, we can understand the quantity $M\!A(u_R)$ we are trying to specify. If we ***choose***

$$
M\!A(u_R) = - \lambda_{\mathcal{P}} = \angle{\mathcal{P}}{\mathcal{S}}\Upsilon \iff \angle{\mathcal{P}}{\mathcal{O}}{\mathcal{Q}}(u_R) = \angle{\mathcal{P}}{\mathcal{S}}\Upsilon,
\tag{7}
$$
...
$$
\langle EOT \rangle 
= \langle  M\!A(u) \rangle + \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle 
= M\!A(\langle u \rangle) + \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle,
$$

since $M\!A$ is a linear function of $u$. In particular, let

$$
{\langle f \rangle}_X \equiv Y^{-1} \int_{X-Y/2}^{X+Y/2} f(u) \,\text{d}u,
\tag{8}
$$
...
$$
\langle EOT \rangle_{u_X} = M\!A(u_X) + \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle_{u_X}.
\tag{9}
$$
...
\langle EOT \rangle_{u_\Upsilon} = M\!A(u_\Upsilon) + \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle_{u_\Upsilon}.
\tag{10}
$$
...
$$
\alpha_{\mathcal{S}} = \operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda),
$$
...
$$
\frac{\text{d}\lambda}{\text{d}u} = \frac{\text{d}\nu}{\text{d}u}
  = \frac{2\pi}{Y}\,(1-e^2)^{-3/2}\,(1 + e\cos\nu)^2,
$$

where $e$ is the eccentricity and $\nu \equiv \lambda(u) - \lambda_{\mathcal{P}}$ is shorthand for
the true anomaly of the earth, $T\!A(u)$. Then, without being precise on limits for now,

$$
\langle \alpha_{\mathcal{S}} \rangle
= Y^{-1} \int \frac{\alpha_{\mathcal{S}} \,\text{d}\lambda}{\text{d}\lambda / \text{d}u} 
= \int \frac{\operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda)}{(1-e^2)^{-3/2}\,[1+e\cos(\lambda-\lambda_{\mathcal{P}})]^2} \frac{\text{d}\lambda}{2\pi}.
$$
...
= & \frac{\lambda_{\mathcal{P}}}{2\pi} \int_{-\pi}^{+\pi} \frac{\text{d}\nu}{(1-e^2)^{-3/2}\,[1+e\cos\nu]^2},
\end{split}
$$
...
$$
-\sin\nu \frac{\text{d}\nu}{\text{d}E} = -\sin E \frac{1 - e^2}{(1 - e \cos E)^2}
\implies \frac{\text{d}\nu}{\text{d}E} = \frac{\sqrt{1 - e^2}}{1 - e \cos E}.
$$
...
$$
\langle \alpha_{\mathcal{S}} \rangle_{u_{\mathcal{P}}}
= \frac{\lambda_{\mathcal{P}}}{2\pi} \int_{-\pi}^{+\pi} (1 - e \cos E) \,\text{d}E  = \frac{\lambda_{\mathcal{P}}}{2\pi} \int_{-\pi}^{+\pi} \,\text{d}M
= \lambda_{\mathcal{P}}.
$$

where $M \equiv E - e \sin E$. Hence, as required, $\langle EOT \rangle_{u_{\mathcal{P}}} = 0$ by
(10). Note that $M(u)$ is none other than $M\!A(u)$, as per Kepler's Equation of the two-body
solution.
...
$$
\langle \alpha_{\mathcal{S}} \rangle = \int \operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda) \frac{\text{d}\lambda}{2\pi},
$$
...
= \int_{u_{\Upsilon'}}^{u_{\Upsilon'}+Y} \hspace{-1em} \alpha_{\mathcal{S}}(u) \frac{\text{d}u}{Y} 
= \int_{-\pi}^{+\pi} \!\! \operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda) \frac{\text{d}\lambda}{2\pi} 
= 0,
$$

since $\operatorname{atan2}$ is odd in $\lambda$. Then, by (9),

$$
\langle EOT \rangle_{u_{\Upsilon'}+Y/2} = M\!A(u_{\Upsilon'}+Y/2) + \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle_{u_{\Upsilon'}+Y/2} =  M\!A(u_\Upsilon) + \lambda_{\mathcal{P}},
$$

since $\Upsilon'$ and $\Upsilon$ *are* a half year apart for a circular ($e=0$) orbit. But also for a circular orbit, $M\!A(u) = T\!A(u) = \lambda(u) - \lambda_{\mathcal{P}}$, so
...
\begin{split}
\langle\alpha_{\mathcal{S}}\rangle_{u_{\Upsilon'}+Y/2} 
& = \int_{u_{\Upsilon'}}^{u_{\Upsilon'}+Y} \hspace{-1em} \alpha_{\mathcal{S}} \frac{\text{d}u}{Y} 
  = \int_{-\pi}^{+\pi} \!\! \frac{\operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda)}{(1-e^2)^{-3/2}\,[1+e\cos(\lambda-\lambda_{\mathcal{P}})]^2} \frac{\text{d}\lambda}{2\pi} \\
& = \int_0^{\pi} \frac{\operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda)}{(1-e^2)^{-3/2}}D(\lambda; \lambda_{\mathcal{P}}) \frac{\text{d}\lambda}{2\pi},
\end{split}
$$

since $\operatorname{atan2}$ is odd in $\lambda$, where
...
$$
\begin{split}
\langle& \alpha_{\mathcal{S}}\rangle_{u_{\Upsilon'}+Y/2} = \int_0^{\pi/2} \frac{\operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda)}{(1-e^2)^{-3/2}}D(\lambda; \lambda_{\mathcal{P}}) \\
& \hspace{3cm} + \frac{\operatorname{atan2}\,(\cos\lambda\,{\cos\varepsilon}, -\sin\lambda)}{(1-e^2)^{-3/2}}D(\lambda\!+\!\pi/2; \lambda_{\mathcal{P}}) \:\frac{\text{d}\lambda}{2\pi} \\
& = \int_0^{\pi/2} \hspace{-1em} \arctan(\tan\lambda\,{\cos\varepsilon}) \frac{D(\lambda; \lambda_{\mathcal{P}})}{(1-e^2)^{-3/2}} + [\pi-\arctan(\cot\lambda\,{\cos\varepsilon})] \frac{D(\lambda\!+\!\pi/2; \lambda_{\mathcal{P}})}{(1-e^2)^{-3/2}} \:\frac{\text{d}\lambda}{2\pi},
\end{split}
$$
...
\begin{split}
\langle& \alpha_{\mathcal{S}}\rangle_{u_{\Upsilon'}+Y/2} \\
& \approx -4 \, e \sin\lambda_{\mathcal{P}} \int_0^{\pi/2} \hspace{-1em} \arctan(\tan\lambda\,{\cos\varepsilon}) \sin\lambda + [\pi-\arctan(\cot\lambda\,{\cos\varepsilon})] \cos\lambda \:\frac{\text{d}\lambda}{2\pi} \\
& = \frac{-e \sin\lambda_{\mathcal{P}}}{\pi/2} \Big[ 
\cot\varepsilon \operatorname{artanh}(\sin\lambda\,{\sin\varepsilon}) - \cos\lambda \arctan(\tan\lambda\,{\cos\varepsilon}) \\
& \hspace{1.75cm} + \cot\varepsilon \operatorname{artanh}(\cos\lambda\,{\sin\varepsilon}) - \sin\lambda \arctan(\cot\lambda\,{\cos\varepsilon}) + \pi \sin\lambda \Big]_0^{\pi/2} \\
& = \frac{-e \sin\lambda_{\mathcal{P}}}{\pi/2} \Big[ \cot\varepsilon [\operatorname{artanh}({\sin\varepsilon}) - \operatorname{artanh}(0)] - \{ 0 \arctan(\infty) - \arctan(0) \} \\
& \hspace{2cm} + \cot\varepsilon [\operatorname{artanh}(0) - \operatorname{artanh}({\sin\varepsilon})] - \{ \arctan(0) - 0 \arctan(\infty) \} + \pi \Big] \\
...
& = M\!A(u_{\Upsilon'}+Y/2) + \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle_{u_{\Upsilon'}+Y/2} \\
& = M\!A(u_{\Upsilon'}) + \pi + \lambda_{\mathcal{P}} + 2e \sin\lambda_{\mathcal{P}} \\
& = E(u_{\Upsilon'})  - e \sin E(u_{\Upsilon'}) + \pi + \lambda_{\mathcal{P}} + 2e \sin\lambda_{\mathcal{P}} \\
& = 2\arctan\left[ \sqrt{\frac{1-e}{1+e}}\tan\left(\frac{\nu(u_{\Upsilon'})}{2}\right) \right]  - \frac{e\sqrt{1-e^2}\sin\nu(u_{\Upsilon'})}{1 + e\cos\nu(u_{\Upsilon'})} + \pi + \lambda_{\mathcal{P}} + 2e \sin\lambda_{\mathcal{P}} \\
& = -2\arctan\left[ \sqrt{\frac{1-e}{1+e}}\tan\left(\frac{\lambda_{\mathcal{P}} + \pi}{2}\right) \right] + \frac{e\sqrt{1-e^2}\sin(\lambda_{\mathcal{P}} + \pi)}{1 + e\cos(\lambda_{\mathcal{P}} + \pi)} + \pi + \lambda_{\mathcal{P}} + 2e \sin\lambda_{\mathcal{P}} \\
& = -2\arctan\left[ \sqrt{\frac{1-e}{1+e}}\tan\left(\frac{\lambda_{\mathcal{P}} + \pi}{2}\right) \right] - \frac{e\sqrt{1-e^2}\sin\lambda_{\mathcal{P}}}{1 - e\cos\lambda_{\mathcal{P}}} + \pi + \lambda_{\mathcal{P}} + 2e \sin\lambda_{\mathcal{P}},
\end{split}
$$

since $M\!A = E - e\sin E$ and since the eccentric anomaly $E$ obeys the following relations from the
two-body solution,
...
\begin{split}
\langle EOT & \rangle_{u_{\Upsilon'}+Y/2} 
   \approx -2\arctan\left[ (1-e)\tan\left(\frac{\lambda_{\mathcal{P}} + \pi}{2} \right) \right] - e\sin\lambda_{\mathcal{P}} + \pi + \lambda_{\mathcal{P}} + 2e \sin\lambda_{\mathcal{P}} \\
& \approx -2 \arctan\left[ \tan\left(\frac{\lambda_{\mathcal{P}} + \pi}{2} \right) \right] + \frac{2e\tan\left(\frac{\lambda_{\mathcal{P}} + \pi}{2} \right)}{1+\tan^2\left(\frac{\lambda_{\mathcal{P}} + \pi}{2} \right)} + \pi + \lambda_{\mathcal{P}} + e \sin\lambda_{\mathcal{P}} \\
& =  e\sin2\left(\frac{\lambda_{\mathcal{P}} + \pi}{2} \right) + e \sin\lambda_{\mathcal{P}} = e [ \sin(\lambda_{\mathcal{P}} + \pi) + \sin\lambda_{\mathcal{P}} ] = 0,
\end{split}
$$
...
\begin{split}
\langle\alpha_{\mathcal{S}}\rangle_{u_{\Upsilon'}+Y/2} 
& = \int_{M(u_{\Upsilon'})}^{M(u_{\Upsilon'})+2\pi} \hspace{-1em} \alpha_{\mathcal{S}} \frac{\text{d}M}{2\pi} 
 = \frac{1}{2\pi}\! \left( \left[\alpha_{\mathcal{S}} M \right]_{u_{\Upsilon'}}^{u_{\Upsilon'}+Y}  - \int_{-\pi}^{+\pi} M \,\text{d}\alpha_{\mathcal{S}} \right) \\
& = M(u_{\Upsilon'})+\pi -  \frac{1}{2\pi} \int_{-\pi}^{+\pi} M \,\frac{\text{d}\alpha_{\mathcal{S}}}{\text{d}\lambda} \,\text{d}\lambda \\
& = M(u_{\Upsilon'})+\pi -  \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{(E - e \sin E) \,{\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon},
\end{split}
$$

where (again) $M \equiv M\!A = E - e \sin E$, and since $\alpha_{\mathcal{S}}(u_{\Upsilon'}) = \pm \pi$ and

$$
\frac{\partial}{\partial x} \operatorname{atan2}(y,x) = \frac{-y}{x^2 + y^2} \quad\text{and}\quad
...
$$
\frac{\text{d} \alpha_{\mathcal{S}}}{\text{d}\lambda} 
= \frac{\text{d}}{\text{d}\lambda} \operatorname{atan2}\,(\sin\lambda\,{\cos\varepsilon}, \cos\lambda)
...
$$
\begin{split}
\langle EOT & \rangle_{u_{\Upsilon'}+Y/2} = M\!A(u_{\Upsilon'}) + \pi +  \lambda_{\mathcal{P}} - \langle \alpha_{\mathcal{S}} \rangle_{u_{\Upsilon'}+Y/2} \\
& = \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{(E - e \sin E) \,{\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& = \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} 
\frac{\left[2\arctan\left( \sqrt{\frac{1-e}{1+e}}\tan\left(\frac{\nu}{2}\right) \right) - \frac{e\sqrt{1-e^2}\sin\nu}{1 + e\cos\nu} \right] {\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon},
\end{split}
$$

with $\nu = \lambda - \lambda_{\mathcal{P}}$. Clearly this method avoids explicit calculation of $M\!A(u_{\Upsilon'})$.
...
\begin{split}
\langle EOT & \rangle_{u_{\Upsilon'}+Y/2} \\
& \approx \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\left[2\arctan\left( (1-e)\tan\left(\frac{\lambda - \lambda_{\mathcal{P}}}{2}\right) \right) - e\sin(\lambda - \lambda_{\mathcal{P}}) \right] {\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& \approx \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\left[\lambda - \lambda_{\mathcal{P}} - \frac{2e \tan\left(\frac{\lambda - \lambda_{\mathcal{P}}}{2}\right)}{1 +  \tan^2\left(\frac{\lambda - \lambda_{\mathcal{P}}}{2}\right)}
  - e\sin(\lambda - \lambda_{\mathcal{P}}) \right] {\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& = \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\left[\lambda - \lambda_{\mathcal{P}} - 2e\sin(\lambda - \lambda_{\mathcal{P}}) \right] {\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\ 
& = \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\left[\lambda - \lambda_{\mathcal{P}} - 2e(\sin\lambda\cos\lambda_{\mathcal{P}} - \cos\lambda\sin\lambda_{\mathcal{P}}) \right] {\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\ 
& = \lambda_{\mathcal{P}} - \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\left[\lambda_{\mathcal{P}} - 2e\sin\lambda_{\mathcal{P}}\,\cos\lambda \right] {\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon},
\end{split}
$$
...
\begin{split}
\langle EOT & \rangle_{u_{\Upsilon'}+Y/2}
= \lambda_{\mathcal{P}} - \frac{\lambda_{\mathcal{P}}}{2\pi} \int_{-\pi}^{+\pi} \text{d} \alpha_{\mathcal{S}} + \frac{2e\sin\lambda_{\mathcal{P}}}{2\pi} \int_{-\pi}^{+\pi} \frac{\cos\lambda \,{\cos\varepsilon}\; \text{d}\lambda} {1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& = \frac{2e\sin\lambda_{\mathcal{P}}}{2\pi} \int_0^0 \frac{{\cos\varepsilon}\; \text{d}Y'} {1 - Y'^2 \,\sin^2\varepsilon} = 0, \quad \text{where}\;Y' \equiv \sin\lambda,
\end{split}
$$
...
$$
\begin{split}
\langle & EOT \rangle_{u_{\Upsilon'}+Y/2} = \lambda_{\mathcal{P}} + \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{Q \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon} = \ldots \\
& = \frac{1}{2\pi} \sum_{n=2}^{\infty}  \frac{(D - e)^n}{n!} \int_{-\pi}^{+\pi} \frac{2\arctan^{(n)} \left( \tan\left(\frac{\lambda-\lambda_{\mathcal{P}}}{2}\right) \right) \tan^n \left(\frac{\lambda-\lambda_{\mathcal{P}}}{2}\right) \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& \quad\quad - \frac{e(1 + A)}{2\pi} \sum_{k=1}^{\infty} \binom{-1}{k} e^k \int_{-\pi}^{+\pi} \frac{ \cos^k(\lambda-\lambda_{\mathcal{P}}) \sin(\lambda-\lambda_{\mathcal{P}}) \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon}.
\end{split}
\tag{12}
$$
...
$$
\cos^k(\lambda-\lambda_{\mathcal{P}}) \sin(\lambda-\lambda_{\mathcal{P}}) = (C_1C_{1p} + S_1S_{1p})^k (S_1C_{1p} - C_1S_{1p}),
$$

where $C_n \equiv \cos(n\lambda)$, $S_n \equiv \sin(n\lambda)$ and the $p$ subscripts are for the
$\lambda_{\mathcal{P}}$ versions. For $k=1$,

$$
(C_1C_{1p} + S_1S_{1p})(S_1C_{1p} - C_1S_{1p}) = (S_2 C_{2p} - C_2 S_{2p}) / 2.
$$
...
$$
\begin{split}
\frac{e^2}{2\pi} & \int_{-\pi}^{+\pi} \frac{ \cos(\lambda-\lambda_{\mathcal{P}}) \sin(\lambda-\lambda_{\mathcal{P}}) \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& -\frac{e^2\sin(2\lambda_{\mathcal{P}})}{4\pi} \int_{-\pi}^{+\pi} \frac{ \cos(2\lambda) \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon}  \\
& \quad = \frac{e^2\sin(2\lambda_{\mathcal{P}})}{8\pi\sin^2\varepsilon} \Big[ (\cos(2\varepsilon) + 3) \arctan({\cos\varepsilon} \tan\lambda) - 4 \lambda\,{\cos\varepsilon} \Big]_{-\pi}^{+\pi} \\
...
$$
\begin{split}
 \frac{e^2}{2} & \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{2\arctan^{(2)} \left( \tan\left(\frac{\lambda-\lambda_{\mathcal{P}}}{2}\right) \right) \tan^2 \left(\frac{\lambda-\lambda_{\mathcal{P}}}{2}\right) {\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& = -\frac{e^2}{2} \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\sin^2(\lambda-\lambda_{\mathcal{P}}) \tan \left(\frac{\lambda-\lambda_{\mathcal{P}}}{2}\right) {\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& = -\frac{e^2}{2} \frac{1}{2\pi} \int_{-\pi}^{+\pi} \frac{\sin(\lambda-\lambda_{\mathcal{P}}) [1 - \cos(\lambda-\lambda_{\mathcal{P}})] \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon} \\
& = \frac{1}{2} \frac{e^2}{2\pi} \int_{-\pi}^{+\pi} \frac{\sin(\lambda-\lambda_{\mathcal{P}}) \cos(\lambda-\lambda_{\mathcal{P}}) \,{\cos\varepsilon}\; \text{d}\lambda}{1 - \sin^2\!\lambda \,\sin^2\varepsilon},
\end{split}
\tag{14}
$$
