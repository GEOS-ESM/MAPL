...
contains
   ...
   function make_geom_spec_from_hconfig(this, hconfig, rc) result(geom_spec)
      ...
      geom_spec = MeshGeomSpec(npoints)
   ...
   function make_geom_spec_from_metadata(this, file_metadata, rc) result(geom_spec)
      ...
      numElements = file_metadata%get_dimension('elementCount', _RC)
      numConns    = file_metadata%get_dimension('connectionCount', _RC)
      numNodes    = file_metadata%get_dimension('nodeCount', _RC)
      ...
      allocate(MeshGeomSpec :: geom_spec)
      geom_spec = MeshGeomSpec(numNodes, numElements, numConns)
   ...
   function make_geom(this, geom_spec, rc) result(geom)
      ...
      type is (MeshGeomSpec)
         ...
         allocate(tlats(local_count))
             ...
             _ASSERT(size(lons_deg) == local_count, 'MeshGeomSpec coordinate size mismatch')
             _ASSERT(size(lats_deg) == local_count, 'MeshGeomSpec coordinate size mismatch')

         ! Convert from degrees to radians for the Mesh
