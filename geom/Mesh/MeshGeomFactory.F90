...
contains
   ...
   function make_geom_spec_from_hconfig(this, hconfig, rc) result(geom_spec)
      ...
      !    Mesh coordinates are read via FileMetadata.
      ...
      if (has_file) then
         ...
         _ASSERT(.not.(has_lon .or. has_lat), 'Mesh hconfig may specify either lon/lat or file, but not both')
         ...
         ! MeshGeomSpec from the file's coordinates.
      ...
      _ASSERT(has_lon .and. has_lat, 'Mesh hconfig must provide lon/lat arrays or a file')
      ...
      _ASSERT(size(lon) == size(lat), "Mesh lon/lat arrays must have same length")
      npoints = size(lon)

      ! Create decomposition from current VM
      decomp = make_MeshDecomposition(npoints, _RC)

      allocate(MeshGeomSpec :: geom_spec)
      geom_spec = MeshGeomSpec(npoints, decomp)

      ! Store coordinate values (in degrees) so that the
      ! Mesh can be created with the correct content
      ! later in make_geom.
   ...
   function make_geom_spec_from_metadata(this, file_metadata, rc) result(geom_spec)
      ...
      ! For Mesh metadata we expect latitude and longitude
      ...
      _ASSERT(dim_name /= '', 'Mesh metadata missing longitude coordinates')

      lat_dim = get_dim_name(file_metadata, units='degrees_north', _RC)
      _ASSERT(lat_dim /= '', 'Mesh metadata missing latitude coordinates')
      _ASSERT(lat_dim == dim_name, 'Lat/Lon coordinates must share a single dimension for Mesh')
      ...
      _ASSERT(lon_var_name /= '', 'Mesh metadata missing longitude coordinate variable for dimension '//dim_name)

      lat_var_name = find_coord_var_name(file_metadata, dim_name, 'degrees_north', _RC)
      _ASSERT(lat_var_name /= '', 'Mesh metadata missing latitude coordinate variable for dimension '//dim_name)
      ...
      _ASSERT(size(lon) == npoints, 'Mesh metadata longitude size mismatch with dimension')
      _ASSERT(size(lat) == npoints, 'Mesh metadata latitude size mismatch with dimension')
   ...
   logical function supports_hconfig(this, hconfig, rc) result(supports)
      ...
      ! Minimal implementation for now: honor class: Mesh (case-insensitive)
      ...
      ! Mandatory entry: "class: Mesh" (case-insensitive)
      ...
      if (has_file) then
         ! File-based Mesh: explicit lon/lat arrays in the
         ...
         if (has_lon .or. has_lat) then
            supports = .false.
            _FAIL("Mesh hconfig may specify either lon/lat or file, but not both")
         else
            supports = .true.
         end if
      else
         ! Explicit-coordinate Mesh: both lon and lat must
         ! be present.
   ...
   logical function supports_metadata(this, file_metadata, rc) result(supports)
      ...
      ! Identify Mesh-style metadata: both latitude and
   ...
   function make_geom(this, geom_spec, rc) result(geom)
      ...
      type is (MeshGeomSpec)
         ...
         _ASSERT(associated(lons_deg) .and. associated(lats_deg), 'MeshGeomSpec missing coordinates')
         _ASSERT(i_1 <= size(lons_deg), 'MeshGeomSpec coordinate index out of bounds')
         _ASSERT(i_1 <= size(lats_deg), 'MeshGeomSpec coordinate index out of bounds')
         ...
         !Mesh = ESMF_MeshCreate(localCount=local_count, coordSys=ESMF_COORDSYS_SPH_RAD, _RC)
         !call ESMF_MeshAddKey(Mesh, keyName="ESMF:Lat", farray=tlats, datacopyflag=ESMF_DATACOPY_VALUE, &
         !     keyUnits="Radians", keyLongName="Latitude", _RC)
         !call ESMF_MeshAddKey(Mesh, keyName="ESMF:Lon", farray=tlons, datacopyflag=ESMF_DATACOPY_VALUE, &
   ...
   function make_file_metadata(this, geom_spec, unusable, chunksizes, rc) result(file_metadata)
      ...
      ! Mesh-specific file metadata generation can be added later.
