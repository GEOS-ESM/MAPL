...
!#   end interface                                               \
!#                                                               \
!#   interface CONCAT(matmul)                                    \
!#      procedure CONCAT(matmul_vec_,suffix)                     \
!#      procedure CONCAT(matmul_multi_vec_,suffix)               \
...
#define NEW_CSR_MATRIX(k,suffix)                                                 \
   function CONCAT(new_csr_matrix,suffix)(n_rows, n_columns, nnz) result(mat)    \
      type(T(k,suffix)) :: mat                                                   \
      allocate(mat%row_offsets(n_rows))                                          \
      allocate(mat%row_starts(n_rows))                                           \
      allocate(mat%row_lengths(n_rows))                                          \
      allocate(mat%v(nnz))                                                       \
      mat%row_offsets(1) = 0                                                     \
   end function
...
#define MATMUL_VEC(k,suffix)                                        \
   pure function CONCAT(matmul_vec,suffix)(A, x) result(y)          \
      ...
      do concurrent (i = 1:A%n_rows)                                \
                                                                    \
         y(i) = 0                                                   \
         associate (n => A%run_length(i))                           \
           if (n == 0) cycle                                        \
                                                                    \
           associate (j0 => A%run_start(i))                         \
             associate (j1 => j0 + n - 1)                           \
                                                                    \
               do j = j0, j1                                        \
...
#define MATMUL_MULTI_VEC(k, suffix)                                 \   
   function CONCAT(matmul_multivec,suffix)(A, x) result(b)          \
      type(T(k,suffix)), intent(inout) :: A(:)                      \
      real(k), intent(in) :: x(:,:)                                 \
