...
SHALL add a dependency edge, in the consuming component's default
dependency network, from the export's node to the matching import's node.
This is the real-wiring step and SHALL occur only once graph mutation is
...
- **WHEN** a destination component's import has the same short name as a
  source component's export, and an ordinary connection between the two
  components is declared
- **THEN** the destination's default dependency network contains a
  dependency edge from the export's node to the import's node
...
- **WHEN** a connection requires wildcard expansion, callback-style
  binding, or a Field/FieldBundle characteristic mismatch (e.g.
  incompatible units) between export and import
