...
`northward_wind` vs. `x_wind`/`y_wind` for a grid-relative basis) - see
`design.md` Non-Goals.
...
Export may legitimately have no YAML-declared metadata (see `design.md` for
the deferred extdata-yaml case).
...
from. This matches existing (pre-#5413) behavior, which already skips
dictionary lookup for a compound-encoded `standard_name`.
