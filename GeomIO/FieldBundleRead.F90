...
module mapl3g_FieldBundleRead
   ...
   use pFIO, only: FileMetaData, NetCDF4_FileFormatter, PFIO_READ, i_Clients
   use pFIO, only: StringVariableMap, StringVariableMapIterator, Variable
...
contains
   ...
   !---------------------------------------------------------------------------
   subroutine FieldBundlePopulate(bundle, file_geom, metadata_utils, only_vars, rc)
      ...
      block
         type(StringVectorIterator) :: it
         character(len=:), pointer :: dname
         it = gridded_dims%begin()
         do while (it /= gridded_dims%end())
            dname => it%of()
            exclude_list = exclude_list // trim(dname) // ","
            call it%next()
         end do
      end block
   ...
   subroutine FieldBundleRead(bundle, file_tmpl, time, only_vars, regrid_method, &
      ...
      if (field_count == 0) then
         call FieldBundlePopulate(bundle, file_geom, metadata_utils, &
      ...
      if (.not. same_grid) then
         ...
         spec = RegridderSpec( &
              generate_esmf_regrid_param(regrid_method_, typekind, _RC), &
