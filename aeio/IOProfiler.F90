   ...
   subroutine generate_io_summary(rank)
      ...
      type (ProfileReporter) :: reporter
        ...
        call io_prof%stop('global_io_prof')
         ...
         call reporter%add_column(FormattedTextColumn('Inclusive','(f9.4)', 15, InclusiveColumn('MEAN')))
         call reporter%add_column(FormattedTextColumn('% Incl','(f9.4)', 15, PercentageColumn(InclusiveColumn('MEAN'),'MAX')))
         call reporter%add_column(FormattedTextColumn('Exclusive','(f9.4)', 15, ExclusiveColumn('MEAN')))
         call reporter%add_column(FormattedTextColumn('% Excl','(f9.4)', 15, PercentageColumn(ExclusiveColumn('MEAN'))))
         call reporter%add_column(FormattedTextColumn(' Max Excl)','(f9.4)', 15, ExclusiveColumn('MAX')))
         call reporter%add_column(FormattedTextColumn(' Min Excl)','(f9.4)', 15, ExclusiveColumn('MIN')))
