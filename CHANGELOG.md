...
- Added `MAPL_FieldApplyUserRoutine`/`MAPL_FieldBundleApplyUserRoutine` 
- to apply a user routine to each slice of a field (or every field in 
- a bundle) with ungridded/vertical dimensions, plus `MAPL_FieldGetPointerToSlice` 
- (overloaded for R4 and R8) for typed per-slice access. Slices are 2D by default, 
- or 3D when the field has exactly three non-ungridded (grid + vertical) dimensions 
- (for example a 4D field whose fourth dimension is the ungridded dimension). 
- The slice-routine interface is unlimited-polymorphic and assumed-rank, 
- so a single user routine handles R4/R8 and 2D/3D slices via 
- `select rank`/`select type`
