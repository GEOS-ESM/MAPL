...
!!$  call MAPL_TimerOn(STATE,"generic", __RC__)
  ...
  isPresent = ESMF_InfoIsPresent(infoh,'GridType',__RC__)
  if (isPresent) then
     call ESMF_InfoGet(infoh,'GridType',gridTypeAttribute,__RC__)
...
!!$   call MAPL_TimerOff(STATE,"generic",__RC__)
   ...
   call ESMF_InfoSet(infoh,key='POSITIVE',value=trim(positive),__RC__)
...
!!!ALT      call MAPL_WireComponent(GCS(I), RC=status)
 ...
 recursive subroutine MAPL_GetChildLocstream(GC, result, name, rc)
  ...
  function MAPL_VerifyFriendlyInField(FIELD,FRIEND2COMP,RC) result(FRIENDLY)
    ...
    _VERIFY(STATUS)
    if(isPresent) then
      call ESMF_InfoGet(infoh,key="FriendlyTo"//trim(FRIEND2COMP),value=FRIENDLY, __RC__)
    else
       FRIENDLY = .false.
    end if
