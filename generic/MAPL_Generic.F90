...
!
   recursive subroutine MAPL_GenericSetServices ( GC, RC )
      ...
      call meta%t_profiler%start('generic',__RC)

      call register_generic_entry_points(gc, __RC)
      call MAPL_GetRootGC(GC, meta%rootGC, __RC)
      call setup_children(meta, __RC)

      call process_spec_dependence(meta, __RC)
      call meta%t_profiler%stop('generic',__RC)
   ...
   contains
     ...
     subroutine process_spec_dependence(meta, rc)
       ...
       call  MAPL_StateGetVarSpecs(meta, export=ex_specs, __RC)
       ! allow for possibility we do not have export specs
       __RETURN_IF(.not. associated(ex_specs))
       ...
       do K=1,size(EX_SPECS)
          call MAPL_VarSpecGet(EX_SPECS(K), SHORT_NAME=SHORT_NAME, &
               ...
               DEPENDS_ON=DEPENDS_ON, __RC)
          if (DEPENDS_ON_CHILDREN) then
!             mark SHORT_NAME in each child "alwaysAllocate"
             nc = meta%get_num_children()
             __ASSERT(nc > 0, 'DEPENDS_ON_CHILDREN requires at least 1 child')
             do I=1, nc
                childgridcomp => meta%get_child_gridcomp(i)
                call MAPL_InternalStateRetrieve(childgridcomp, cmeta, __RC)
                found = .false.
                call  MAPL_StateGetVarSpecs(cmeta, export=c_ex_specs, __RC)
                __ASSERT(associated(c_ex_specs), 'Component '//trim(cmeta%compname)//' must have a valid export spec')
                ! find the "correct" export spec (i.e. has the same SHORT_NAME)
                do j=1,size(c_ex_specs)
                   call MAPL_VarSpecGet(c_ex_specs(j), SHORT_NAME=NAME, __RC)
                   if (short_name == name) then
                      call MAPL_VarSpecSet(c_ex_specs(j), alwaysAllocate=.true., __RC)
                      found = .true.
                      exit
                   end if
                end do ! spec loop
                __ASSERT(found, 'All children must have '//trim(short_name))
             end do
...
!             mark SHORT_NAME in each variable "alwaysAllocate"
             ...
             __ASSERT(nvars > 0, 'DEPENDS_ON requires at least 1 var')
             do I=1, nvars
                ! find the "correct" export spec (i.e. has the same SHORT_NAME)
                do j=1,size(ex_specs)
                   call MAPL_VarSpecGet(ex_specs(j), SHORT_NAME=NAME, __RC)
                   if (name == depends_on(i)) then
                      call MAPL_VarSpecSet(ex_specs(j), alwaysAllocate=.true., __RC)
                      exit
                   end if
                end do ! spec loop
             end do
      ...
      subroutine register_generic_entry_points(gc, rc)
         ...
         if (.not. associated(meta%phase_init)) then
            call MAPL_GridCompSetEntrypoint(GC, ESMF_METHOD_INITIALIZE, MAPL_GenericInitialize,  __RC)
         endif

         if (.not. associated(meta%phase_run)) then
            call MAPL_GridCompSetEntrypoint(GC, ESMF_METHOD_RUN, MAPL_GenericRunChildren,  __RC)
         endif


         if (.not. associated(meta%phase_final)) then
            call MAPL_GridCompSetEntrypoint(GC, ESMF_METHOD_FINALIZE, MAPL_GenericFinalize,  __RC)
         endif

         !ALT check record!
         if (.not. associated(meta%phase_record)) then
            call MAPL_GridCompSetEntryPoint ( GC, ESMF_METHOD_WRITERESTART, MAPL_GenericRecord, __RC)
         end if
         __ASSERT(size(meta%phase_record)==1,'needs informative message')  !ALT: currently we support only 1 record
   ...
   enddo
      ...
      recursive subroutine setup_children(meta, rc)
         ...
         CHILDREN: if(nc > 0) then
            ...
            do I=1,NC
               call MAPL_GenericStateClockAdd(GC, name=trim(meta%GCNameList(I)), __RC)
            end do
            ...
            call MAPL_WireComponent(GC, __RC)
            ...
            if (NC > 0) then
               ...
               DO I = 1, NC
                  ...
                  call MAPL_GridCompGetVarSpecs(gridcomp, &
                       IMPORT=IM_SPECS, EXPORT=EX_SPECS, __RC)
                  ImSpecPtr(I)%Spec => IM_SPECS
                  ExSpecPtr(I)%Spec => EX_SPECS
               END DO

               call connect%checkReq(ImSpecPtr, ExSpecPtr, __RC)
            ...
            ROOT: if (.not. associated(meta%parentGC)) then
               ...
               call MAPL_GenericConnCheck(GC, __RC)

               ! Collect all IMPORT and EXPORT specs in the entire tree in one list
               !-------------------------------------------------------------------
               call MAPL_GenericSpecEnum(GC, SPECS, __RC)
               ...
               do I = 1, TS
                  ...
                  !-------------------------------------
                  call MAPL_VarSpecGet(SPECS%old_var_specs(I), LABEL=LBL, __RC)
                  __ASSERT(LBL > 0, "GenericSetServices :: Expected LBL > 0.")
               ...
               if (associated(meta%LINK)) then
                  do I = 1, size(meta%LINK)
                     fLBL = MAPL_LabelGet(meta%LINK(I)%ptr%FROM, __RC)
                     tLBL = MAPL_LabelGet(meta%LINK(I)%ptr%TO,   __RC)
                     LOWEST_(fLBL)
                     LOWEST_(tLBL)
               ...
               do I = 1, TS
                  ...
                  if (LBL == I) then
                     ...
                     call MAPL_VarSpecGet(SPECS%old_var_specs(LBL), FIELDPTR = FIELD, __RC)
                     call MAPL_VarSpecSet(SPECS%old_var_specs(I), FIELDPTR = FIELD, __RC)
                     call MAPL_VarSpecGet(SPECS%old_var_specs(LBL), BUNDLEPTR = BUNDLE, __RC  )
                     call MAPL_VarSpecSet(SPECS%old_var_specs(I), BUNDLEPTR = BUNDLE, __RC  )
                     call MAPL_VarSpecGet(SPECS%old_var_specs(LBL), STATEPTR = STATE, __RC  )
                     call MAPL_VarSpecSet(SPECS%old_var_specs(I), STATEPTR = STATE, __RC  )
                  end if

                  call MAPL_VarSpecSet(SPECS%old_var_specs(I), LABEL=LBL, __RC)
               end do
...
#undef LOWEST_
   ...
   recursive subroutine MAPL_GenericInitialize ( GC, import, EXPORT, CLOCK, RC )
      ...
      ! -------------------------------------------
      call MAPL_InternalStateGet ( GC, STATE, __RC)
      ...
      if (comp_name == comp_to_record .and. (is_test_framework .or. is_test_framework_driver)) then
         ...
         call ESMF_AttributeSet(import, name="MAPL_TestFramework", value=.true., __RC)
      end if

      ! Start my timer
      !---------------
!C$  call MAPL_TimerOn(STATE,"generic", __RC)
      ...
      call ESMF_VmGet(VM, localPet=MYGRID%MYID, petCount=ndes, __RC)
      call ESMF_VmGet(VM, mpicommunicator=comm, __RC)

      ! TODO: esmfgrid should be obtained separately
      isGridValid = grid_is_valid(gc, mygrid%esmfgrid, __RC)
      ...
      if (isGridValid) then
         ...
         do I=1, STATE%get_num_children()
            ...
            chldGridValid = grid_is_valid(gridcomp, ChlGrid, __RC)

            if (.not. chldGridValid) then
               ! This child does not have a valid grid
               call ESMF_GridCompSet( gridcomp, GRID = MYGRID%ESMFGRID, __RC )
            end if
         end do
         ...
         call ESMF_GridGet(MYGRID%ESMFGRID, DistGrid=distgrid, dimCount=dimCount, __RC)
         call ESMF_DistGridGet(distGRID, deLayout=MYGRID%LAYOUT, __RC)
         ...
         call MAPL_GridGet(MYGRID%ESMFGRID, localCellCountPerDim=COUNTS, __RC)
         ...
         call MAPL_GridGet(MYGRID%ESMFGRID, globalCellCountPerDim=COUNTS, __RC)
...
#if 0
         call ESMF_DELayoutGetDELocalInfo(delayout=MYGRID%LAYOUT, de=MYGRID%MYID, coord=DECOUNT, __RC)
...
#endif
         call set_checkpoint_restart_options(__RC)
         ...
         gridTypeAttribute = ''
         call ESMF_AttributeGet(MYGRID%ESMFGRID, name='GridType', isPresent=isPresent, __RC)
         if (isPresent) then
            call ESMF_AttributeGet(MYGRID%ESMFGRID, name='GridType', value=gridTypeAttribute, __RC)
            if (gridTypeAttribute == 'Doubly-Periodic') then
               ...
               call MAPL_GetResource( STATE, fixedLons, Label="FIXED_LONS:", __RC)
               call MAPL_GetResource( STATE, fixedLats, Label="FIXED_LATS:", __RC)
      ...
      call MAPL_GetResource( STATE, positive, Label="CHECKPOINT_POSITIVE:", &
           default='down', __RC)
      positive = ESMF_UtilStringLowerCase(positive,__RC)
      __ASSERT(trim(positive)=="up".or.trim(positive)=="down","positive must be up or down")
      ! Put the clock passed down in the generic state
      !-----------------------------------------------

      call handle_clock_and_main_alarm(clock, __RC)

      ! Create tiling for all gridded components with associated LocationStream
      ! -----------------------------------------------------------------------

      is_associated=MAPL_LocStreamIsAssociated(STATE%LOCSTREAM, __RC)
      if (is_associated) then
         NSUBTILES = MAPL_GetNumSubtiles(STATE, __RC)
         call MAPL_LocStreamAdjustNsubtiles(STATE%LocStream, NSUBTILES, __RC)
         call MAPL_LocStreamGet(STATE%LocStream, TILEGRID=TILEGRID, __RC)
      endif

      call handle_record(__RC)

!C$   call MAPL_TimerOff(STATE,"generic",__RC)

      call initialize_children_and_couplers(__RC)
      call MAPL_TimerOn(STATE,"generic",__RC)

      call create_import_and_initialize_state_variables(__RC)

      call ESMF_AttributeSet(import,'POSITIVE',trim(positive),__RC)

      call create_internal_and_initialize_state_variables(__RC)

      call create_export_state_variables(__RC)
      ...
      do i = 1, state%get_num_children()
         ...
         call ESMF_StateAdd(EXPORT, [child_export_state], __RC)
      end do

      if (.not. associated(STATE%parentGC)) then
         call MAPL_AdjustIsNeeded(GC, EXPORT, __RC)
      end if

      call handle_services(__RC)

      ! Write Memory Use Statistics.
      ! -------------------------------------------
      call MAPL_MemUtilsWrite(VM, Iam, __RC )

      call MAPL_TimerOff(STATE,"generic", __RC)
   ...
   contains
      ...
      logical function grid_is_valid(gc, grid, rc)
         ...
         call ESMF_GridCompGet(gc, gridIsPresent=is_present, __RC)

         if (is_present) then
            call ESMF_GridCompGet(gc, grid=grid, __RC)

            is_created = ESMF_GridIsCreated(grid, __RC)
            if (is_created) then
      ...
      subroutine set_checkpoint_restart_options(rc)
         ...
         call MAPL_GetResource( STATE, split_restart, Label="SPLIT_RESTART:", &
              default='NO', __RC)
         split_restart = ESMF_UtilStringUpperCase(split_restart,__RC)
         split_checkpoint = ESMF_UtilStringUpperCase(split_checkpoint,__RC)

         call MAPL_GetResource( STATE, write_restart_by_oserver, Label="WRITE_RESTART_BY_OSERVER:", &
              default='NO', __RC)
         write_restart_by_oserver = ESMF_UtilStringUpperCase(write_restart_by_oserver,__RC)
      ...
      recursive subroutine initialize_children_and_couplers(rc)
         ...
         if (STATE%ChildInit) then
            ...
            do I=1,NC
               gridcomp => STATE%GET_CHILD_GRIDCOMP(I)
               call MAPL_GetObjectFromGC(gridcomp, CHLDMAPL(I)%PTR, __RC)
            ...
            do I=1,NC
               ...
               if (PHASE .le. NUMPHASES) then
                  gridcomp => STATE%GET_CHILD_GRIDCOMP(I)
                  call ESMF_GridCompGet( gridcomp, NAME=CHILD_NAME, __RC )

                  call MAPL_TimerOn (STATE,trim(CHILD_NAME), __RC)
                  ...
                  call ESMF_GridCompInitialize (gridcomp, &
                       ...
                       userRC=userRC, __RC )
                  __VERIFY(userRC)
                  call MAPL_TimerOff(STATE,trim(CHILD_NAME), __RC)
               end if
            end do
            deallocate(CHLDMAPL)
            ...
            if (.not. isGridValid) then
               if (associated(STATE%COMPONENT_SPEC%import%OLD_VAR_SPECS)) then
                  call MAPL_StateCreateFromSpecNew(IMPORT,STATE%COMPONENT_SPEC%IMPORT,__RC)
               end if
            end if
               ...
               do J=1,NC
                  if(STATE%CCcreated(J,I)) then
                     ...
                     call ESMF_CplCompInitialize (STATE%CCS(J,I), &
                          ...
                          clock=CLOCK, userRC=userRC, __RC )
                     __VERIFY(userRC)
                  endif
               enddo
               ! ---------------------------------------------------
      ...
      subroutine handle_clock_and_main_alarm(clock, unusable, rc)
         ...
         STATE%CLOCK = CLOCK
         call ESMF_ClockGet(CLOCK, TIMESTEP = DELT, __RC)
         call ESMF_TimeIntervalGet(DELT, S=DELTSEC, __RC)
         __ASSERT(DELTSEC /= 0,'needs informative message')
         ...
         call ESMF_ClockGet(clock, calendar = cal, currTime=currTime, timestep=tstep, __RC)
         call ESMF_ConfigGetAttribute( state%CF, DEFDT, Label="RUN_DT:", __RC)
         ...
         ! component
         __ASSERT(MOD(DELTSEC,DTSECS)==0,'needs informative message')

         call MAPL_GetResource( STATE   , DT, Label="DT:", default=DEFDT, __RC)
         call MAPL_GetResource( STATE   , run_at_interval_start, Label="RUN_AT_INTERVAL_START:", default=.false., __RC)

         __ASSERT(DT /= 0.0,'needs informative message')

         DTSECS = nint(DT)
         ! Make sure this component's DT is multiple of CLOCK's timestep
         __ASSERT(MOD(DTSECS,DELTSEC)==0,'needs informative message')

         call ESMF_TimeIntervalSet(TIMEINT,  S=DTSECS , calendar=cal, __RC)
         ...
         STATE%ALARM(0) = ESMF_AlarmCreate(CLOCK = CLOCK, &
              ...
              RC           = status      )
         if(ringTime == currTime) then
            call ESMF_AlarmRingerOn(STATE%ALARM(0), __RC)
         end if
         __RETURN(ESMF_SUCCESS)
      ...
      subroutine handle_record(rc)
         ...
         if (associated(STATE%parentGC)) then
            call MAPL_GetObjectFromGC(STATE%parentGC, PMAPL, __RC)
            if (associated(PMAPL%RECORD)) then
               call MAPL_AddRecord(STATE, PMAPL%RECORD%ALARM, PMAPL%RECORD%FILETYPE, __RC)
            end if
         end if
         ! Add this component's own RECORD

         call ESMF_ConfigFindLabel( STATE%CF, LABEL="RECORD_FREQUENCY:", isPresent=isPresent, __RC)
         if (isPresent) then
            nra = ESMF_ConfigGetLen( STATE%CF, RC = status)
            __ASSERT( NRA > 0,'Empty list is not allowed')

            allocate(ref_date(NRA), ref_time(NRA), freq(NRA), freq_string(NRA), stat=status)

            call ESMF_ConfigFindLabel( STATE%CF, LABEL="RECORD_FREQUENCY:", __RC)
            call ESMF_ConfigGetAttribute( STATE%CF, valueList=freq_string, count=NRA, __RC)

            if (.not. all(freq_string == 'monthly')) then

               call ESMF_ConfigFindLabel( STATE%CF, LABEL="RECORD_REF_DATE:", __RC)
               !      __ASSERT(NRA == ESMF_ConfigGetLen(STATE%CF),'needs informative message')
               call ESMF_ConfigGetAttribute( STATE%CF, valueList=ref_date, count=NRA, __RC)

               call ESMF_ConfigFindLabel( STATE%CF, LABEL="RECORD_REF_TIME:", __RC)
               !      __ASSERT(NRA == ESMF_ConfigGetLen(STATE%CF),'needs informative message')
               call ESMF_ConfigGetAttribute( STATE%CF, valueList=ref_time, count=NRA, __RC)
            end if
               ...
               if (status/=ESMF_SUCCESS) then
                  monthly: if (freq_string(i) == 'monthly') then
                     ...
                     call ESMF_TimeSet( RefTime, YY = year, MM = month, &
                          DD = 1, H = 0, M = 0, S = 0, calendar=cal, __RC )
                     call ESMF_TimeIntervalSet( frequency, MM=1, __RC )
                     RingTime = RefTime
                     ...
                     call ESMF_TimeIntervalSet( frequency, S=sec, __RC )
                     RingTime = RefTime
                  ...
                  RecordAlarm = ESMF_AlarmCreate( name=trim(AlarmName), clock=clock, RingInterval=Frequency, &
                       RingTime=RingTime, sticky=.false.,__RC )

                  if(ringTime == currTime .and. .not.mnthly) then
                     call ESMF_AlarmRingerOn(RecordAlarm, __RC)
                  else
                     call ESMF_AlarmRingerOff(RecordAlarm, __RC)
                  end if
            ...
            END DO
            call MAPL_AddRecord(STATE, R_ALARM, R_FILETYPE, __RC)
         ...
         if (associated(STATE%RECORD)) then
            ...
            if(status==ESMF_SUCCESS) then
               ...
               call fill_grads_template(filename,trim(adjustl(FILEtpl)),experiment_id=trim(id_string), nymd=yyyymmdd,nhms=hhmmss,__RC)
      ...
      subroutine create_internal_and_initialize_state_variables(rc)
         ...
         internal_state = ESMF_StateCreate(name = trim(comp_name) // "_INTERNAL", __RC)
         ...
         if (associated(STATE%COMPONENT_SPEC%INTERNAL%OLD_VAR_SPECS)) then
            ...
            call ESMF_AttributeSet(internal_state,'POSITIVE',trim(positive),__RC)
            ...
            if(status==ESMF_SUCCESS) then
               ...
               call fill_grads_template(filename,trim(adjustl(FILEtpl)),experiment_id=trim(id_string), &
                    nymd=yyyymmdd,nhms=hhmmss,__RC)
               ...
               if (associated(STATE%phase_coldstart)) then
                  ...
                  call ESMF_GridCompReadRestart(GC, importState=import, &
                       exportState=export, clock=CLOCK, userRC=userRC, __RC)
                  GC%compp = GCCS%compp
      ...
      subroutine create_export_state_variables(rc)
         ...
         if (associated(STATE%COMPONENT_SPEC%EXPORT%OLD_VAR_SPECS)) then
            ...
            call MAPL_GetResource(STATE, restoreExport, label='RESTORE_EXPORT_STATE:', default=.false., __RC)
            if (restoreExport) then
               call MAPL_GetResource( STATE, FILENAME, LABEL='EXPORT_RESTART_FILE:', __RC)
               if(status==ESMF_SUCCESS) then
                  ...
                  if (status /= ESMF_SUCCESS) then
                     if (MAPL_AM_I_Root(VM)) then
                        call ESMF_StatePrint(EXPORT, __RC)
                     end if
                     __RETURN(ESMF_FAILURE)
                  end if
               endif
         ...
         call ESMF_AttributeSet(export,'POSITIVE',trim(positive),__RC)
      ...
      subroutine handle_services(rc)
         ...
         if (state%provided_services%size()>0) then
            call ProvidedServiceSet(state%provided_services, import, __RC)
         end if

         ! process any requesters
         if (state%requested_services%size()>0) then
            call FillRequestBundle(state%requested_services, state%get_internal_state(), __RC)
         end if

         ! process any service connections
         call MAPL_ProcessServiceConnections(state, __RC)
   ...
   recursive subroutine MAPL_GenericWrapper ( GC, IMPORT, EXPORT, CLOCK, RC)
      ...
      call t_p%start(trim(state%compname),__RC)
      ...
      if (method /= ESMF_METHOD_READRESTART .and. method /= ESMF_METHOD_WRITERESTART) then
         call state%t_profiler%start(__RC)
         call state%t_profiler%start(trim(sbrtn),__RC)
      end if

      if (associated(timers)) then
         do i = 1, size(timers)
            call MAPL_TimerOn (STATE,timers(i), __RC)
         end do
      end if
      ...
      call MAPL_GetResource(STATE, comp_to_record, label='COMPONENT_TO_RECORD:', default='', __RC)
      if (comp_name == comp_to_record) then
         call record_component('before', phase, method, GC, import, export, clock, __RC)
      end if

      if (use_threads .and. method == ESMF_METHOD_RUN)  then
         call omp_driver(GC, import, export, clock, __RC)  ! component threaded with OpenMP
      else
         call func_ptr (GC, &
              importState=IMPORT, &
              exportState=EXPORT, &
         ...
         __ASSERT(userRC==ESMF_SUCCESS .and. STATUS==ESMF_SUCCESS,'Error during '//stage_description//' for <'//trim(COMP_NAME)//'>')
      end if

      if (comp_name == comp_to_record) then
         call record_component('after', phase, method, GC, import, export, clock, __RC)
      end if
      ...
      if (associated(timers)) then
         do i = size(timers),1,-1
            call MAPL_TimerOff (STATE,timers(i),__RC)
         end do
      end if
      ...
      if (method /= ESMF_METHOD_FINALIZE) then
         if (method /= ESMF_METHOD_WRITERESTART .and. &
            ...
            call state%t_profiler%stop(trim(sbrtn),__RC)
            call state%t_profiler%stop(__RC)
         end if
      endif
      call t_p%stop(trim(state%compname),__RC)
   ...
   subroutine get_test_framework_resource(STATE, is_test_framework, is_test_framework_driver, &
     ...
     call MAPL_GetResource(STATE, is_test_framework, label='TEST_FRAMEWORK:', default=.false., __RC)
     call MAPL_GetResource(STATE, is_test_framework_driver, label='TEST_FRAMEWORK_DRIVER:', default=.false., __RC)
     call MAPL_GetResource(STATE, is_grid_capture, label='GRID_CAPTURE:', default=.false., __RC)
     call MAPL_GetResource(STATE, restore_export, label='RESTORE_EXPORT_STATE:', default=.false., __RC)
     __RETURN(__SUCCESS)
   ...
   subroutine record_component(POS, PHASE, METHOD, GC, IMPORT, EXPORT, CLOCK, RC)
     ...
     call get_test_framework_resource(STATE, is_test_framework, is_test_framework_driver, &
                                      is_grid_capture, restore_export, __RC)

     if (method == ESMF_METHOD_INITIALIZE) then
        call ESMF_AttributeSet(export, name="MAPL_RestoreExport", value=restore_export, __RC)
     else if (method == ESMF_METHOD_RUN) then
        call ESMF_AttributeSet(import, name="MAPL_GridCapture", value=is_grid_capture, __RC)
        if (is_test_framework) then
           call capture(POS, phase, GC, import, export, clock, __RC)
   ...
   subroutine capture(POS, PHASE, GC, IMPORT, EXPORT, CLOCK, RC)
     ...
     call MAPL_GetResource(state, clobber_file, LABEL="overwrite_checkpoint:", default = .false., __RC)
     call ESMF_ClockGet(clock, startTime=start_time, currTime=curr_time, __RC)

     call MAPL_GetResource(STATE, time_label, label='TARGET_TIME:', default='')
     if (time_label == '') then
        target_time = start_time
     else
        target_time = parse_time_string(time_label, __RC)
     end if
     ...
     if (curr_time == target_time) then
        ...
        call MAPL_ESMFStateWriteToFile(import, CLOCK, trim(FILENAME)//"import_"//trim(POS)//"_runPhase"//phase_, &
             ...
             write_with_oserver=state%grid%write_restart_by_oserver, __RC)
        call MAPL_ESMFStateWriteToFile(export, CLOCK, trim(FILENAME)//"export_"//trim(POS)//"_runPhase"//phase_, &
             FILETYPE, STATE, .false., clobber=clobber_file, &
             write_with_oserver=state%grid%write_restart_by_oserver, __RC)
        call MAPL_GetResource(STATE, hdr, default=0, LABEL="INTERNAL_HEADER:", __RC)
   ...
   subroutine omp_driver(GC, import, export, clock, RC)
      ...
      call ESMF_GridCompGet (GC, NAME=comp_name, currentPhase=phase, __RC)
      ...
      if(MAPL%is_threading_active()) then
         ...
         call MAPL%activate_threading(num_threads, __RC)
...
!@ call MAPL_TimerOff(STATE,"GenRunTot")
   ...
   recursive subroutine MAPL_GenericFinalize ( GC, IMPORT, EXPORT, CLOCK, RC )
      ...
      if (final_checkpoint) then
         ...
         !----------------------------------------
         call checkpoint_export_state(__RC)
      end if
      ...
      call state%t_profiler%stop('Finalize',__RC)
      call state%t_profiler%stop(__RC)
   ...
   recursive subroutine MAPL_GenericRecord ( GC, IMPORT, EXPORT, CLOCK, RC )
      ...
      t_p => get_global_time_profiler()
      call state%t_profiler%start(__RC)
      call state%t_profiler%start('Record',__RC)
      ...
      do I = 1, STATE%get_num_children()
         ...
         call ESMF_GridCompWriteRestart (gridcomp, &
              importState=child_import_state, &
              exportState=child_export_state, &
              clock=CLOCK, userRC=userRC, __RC ) ! number of phases is currently limited to 1
         __VERIFY(userRC)
         call MAPL_TimerOff(STATE,trim(CHILD_NAME), __RC)
      enddo
      ...
      call state%t_profiler%stop('Record',__RC)
      call state%t_profiler%stop(__RC)
   ...
   recursive subroutine MAPL_GenericRefresh ( GC, IMPORT, EXPORT, CLOCK, RC )
      ...
      t_p => get_global_time_profiler()
      call state%t_profiler%start(__RC)
      call state%t_profiler%start('Refresh',__RC)

      call MAPL_TimerOn(STATE,"GenRefreshTot", __RC)
      ...
      ! Do my "own" refresh
      ! ------------------
      call MAPL_TimerOn(STATE,"GenRefreshMine", __RC)
...
! I_Run
      ...
      call MAPL_TimerOff(STATE,"GenRefreshMine",__RC)
      call MAPL_TimerOff(STATE,"GenRefreshTot",__RC)

      call state%t_profiler%stop('Refresh',__RC)
      call state%t_profiler%stop(__RC)
...
!C$       root_composite = ConcreteComposite(MAPLOBJ)
   ...
   subroutine MAPL_GenericStateDestroy (STATE,RC)
      ...
      if(associated(STATE)) then
         ...
         __VERIFY(status)
         if (isCreated) then
            call ESMF_StateDestroy       (internal_state, __RC)
         end if
   ...
   subroutine MAPL_StateGetSpecAttrib(STATE, NAME,    &
      ...
      __ASSERT(associated(SPEC),'needs informative message')
   ...
   subroutine MAPL_StateAddImportSpec_(GC, SHORT_NAME, LONG_NAME,               &
      ...
      if (present(DIMS)) then
         __ASSERT(DIMS /= MAPL_DimsNone,'needs informative message')
      end if
   ...
   subroutine MAPL_StateAddImportSpecFrmChld ( STATE, SHORT_NAME, CHILD_ID, RC )
      ...
      if (status /= MAPL_DuplicateEntry) then
         ...
         __RETURN(ESMF_FAILURE) ! ALT this needs to be revisited
      endif
   ...
   subroutine MAPL_StateAddExportSpec_(GC, SHORT_NAME, LONG_NAME,            &
      ...
      if (present(UNGRIDDED_DIMS)) then
         if (present(UNGRIDDED_UNIT) .or. present(UNGRIDDED_NAME) .or. present(UNGRIDDED_COORDS)) then
            __ASSERT(size(UNGRIDDED_DIMS) == 1,'needs informative message')
            __ASSERT(UNGRIDDED_DIMS(1) == size(UNGRIDDED_COORDS),'needs informative message')
         end if
      end if
   ...
   subroutine MAPL_StateAddExportSpecFrmChld_All ( GC, CHILD_ID, RC)
      ...
      call MAPL_InternalStateRetrieve(gc, maplobj, __RC)

      gridcomp => maplobj%GET_CHILD_GRIDCOMP(child_id)
      call MAPL_GridCompGetVarSpecs(gridcomp, EXPORT=SPECS, __RC)
       __VERIFY(status)

      do I = 1, size(SPECS)
         call MAPL_VarSpecGet(SPECS(I), SHORT_NAME=short_name, __RC)
         call MAPL_StateAddExportSpecFrmChld(gc, short_name, child_id, __RC)
      enddo
   ...
   subroutine MAPL_StateAddExportSpecFrmChld ( GC, SHORT_NAME, CHILD_ID, RC, TO_NAME )
      ...
      call MAPL_InternalStateRetrieve(gc, maplobj, __RC)
      call MAPL_StateAddExportSpecFrmChldName(gc, short_name, maplobj%gcnamelist(child_id), TO_NAME=TO_NAME, __RC)
   ...
   subroutine MAPL_DoNotDeferExport(GC, NAMES, RC)
      ...
      if (associated(STATE%COMPONENT_SPEC%EXPORT%OLD_VAR_SPECS)) then
         ...
         DO I=1,K
            ...
            __ASSERT(J > 0, 'J is equal or less than 0')
            __ASSERT(J <= N, 'J is greater than N')
   ...
   subroutine MAPL_GridCompSetEntryPoint(GC, registeredMethod, usersRoutine, RC)
      ...
      if (registeredMethod == ESMF_METHOD_INITIALIZE) then
         ...
         call ESMF_GridCompSetEntryPoint(GC, ESMF_METHOD_INITIALIZE, &
              usersRoutine, phase=MAPL_CustomRefreshPhase, __RC)
         __RETURN(ESMF_SUCCESS)
      else
         __RETURN(ESMF_FAILURE)
      endif
...
!
   subroutine MAPL_GenericStateGet (STATE, IM, JM, LM, VERTDIM,                &
      ...
      if(present(ORBIT)) then
         ...
         if(.not.MAPL_SunOrbitCreated(STATE%ORBIT)) then
            ...
            call ESMF_GridGet(STATE%GRID%ESMFGRID,name=gname,__RC)
            FIX_SUN = (index(gname,"DP")>0)

            ! create the orbit object
            STATE%ORBIT = MAPL_SunOrbitCreateFromConfig (STATE%CF, STATE%CLOCK, FIX_SUN, __RC)
   ...
   subroutine MAPL_GenericStateSet (STATE, ORBIT, LM, RUNALARM, CHILDINIT, &
      ...
      if(present(ConfigFile)) then
         State%CF = ESMF_ConfigCreate(__RC)
         call ESMF_ConfigLoadFile(State%CF,ConfigFile,__RC)
      endif
   ...
   recursive integer function AddChildFromMeta(META, NAME, GRID, &
      ...
      call AddChild_preamble(meta, I, name, grid=grid, configfile=configfile, parentGC=parentgc, petList=petlist, child_meta=child_meta, __RC)

      stage_description = 'setServices() of the gridded component <'//trim(name)//'>'
      lgr => logging%get_logger('MAPL.GENERIC')

      t_p => get_global_time_profiler()
      call t_p%start(trim(NAME),__RC)
      call child_meta%t_profiler%start(__RC)
      call child_meta%t_profiler%start('SetService',__RC)

!C$     gridcomp => META%GET_CHILD_GRIDCOMP(I)
      call lgr%debug("Started %a", stage_description)
      call ESMF_GridCompSetServices ( child_meta%gridcomp, SS, userRC=userRC, __RC )
      __VERIFY(userRC)
      call lgr%debug("Finished %a", stage_description)

      call child_meta%t_profiler%stop('SetService',__RC)
      call child_meta%t_profiler%stop(__RC)
      call t_p%stop(trim(NAME),__RC)
   ...
   recursive subroutine AddChild_preamble(meta, I, name, grid, configfile, parentGC, petlist, child_meta, unusable, rc)
      ...
      call make_full_name(name, child_name, parentGC, __RC)
      call grow_children_names(meta%GCNamelist, child_name, __RC)

      allocate(tmp_meta, __STAT)
      tmp_framework => META%add_child(child_name, tmp_meta)
      deallocate(tmp_meta)
      __ASSERT(associated(tmp_framework),'add_child() failed')
      ...
      select type (tmp_framework)
         ...
         if (present(configfile)) then
            child_meta%cf = ESMF_ConfigCreate(__RC)
            call ESMF_ConfigLoadFile(child_meta%cf, configfile, __RC)
         ...
         child_export_state = ESMF_StateCreate (             &
              ...
              stateIntent = ESMF_STATEINTENT_EXPORT, __RC)

         ! create MAPL_Meta
         call MAPL_InternalStateCreate ( child_meta%gridcomp, child_meta, __RC)
   ...
   contains
      ...
      subroutine make_full_name(name, child_name, parentGC, unusable, rc)
         ...
         if (index(name,":") == 0) then
            if (present(parentGC)) then
               tmp_gc = parentGC
               call ESMF_GridCompGet(tmp_gc, name=pname, __RC)
   ...
   recursive integer function AddChildFromGC(GC, name, SS, petList, configFile, RC)
      ...
      AddChildFromGC = AddChildFromMeta(Meta, name, SS=SS, PARENTGC=GC, petList=petList, configFile=configFile, __RC)
   ...
   recursive integer function AddChildFromDSOMeta(meta, name, userRoutine, grid, sharedObj, petList, configFile, parentGC, RC)
      ...
      if (.not.allocated(meta%GCNameList)) then
         ! this is the first child to be added
         allocate(meta%GCNameList(0), __STAT)
      end if
      ...
      call ESMF_GridCompSetServices ( child_meta%gridcomp, userRoutine, &
           sharedObj=shared_object_library_to_load,userRC=userRC,__RC)
      __VERIFY(userRC)
   ...
   recursive integer function AddChildFromDSO(gc, name, userRoutine, grid, sharedObj, petList, configFile, RC)
      ...
      AddChildFromDSO = AddChildFromDSOMeta(meta, name, userRoutine, grid=grid, sharedObj=sharedObj, petList=petList, configFile=configFile, parentGC=gc, __RC)
   ...
   recursive integer function AddChildFromDSO_old(name, userRoutine, grid, ParentGC, SharedObj, petList, configFile, RC)
      ...
      __ASSERT(present(ParentGC),'must have a parent to use this interface')
      addchildfromdso_old = addChildFromDSO(parentGC, name, userRoutine, grid=grid, sharedObj=sharedObj, petList=petList, configFile=configFile, __RC)
   ...
   subroutine MAPL_AddConnectivityE2E ( GC, SHORT_NAME, &
      ...
      call MAPL_ConnectivityGet(gc, connectivityPtr=conn, __RC)

      call conn%connect%append(SHORT_NAME, TO_NAME=TO_NAME, &
           FROM_EXPORT=SRC_ID, TO_IMPORT=TO_EXPORT, __RC)
   ...
   subroutine MAPL_AddConnectivityRename ( GC, SRC_NAME, SRC_ID, &
      ...
      call CONN%CONNECT%append(SHORT_NAME=SRC_NAME, TO_NAME=DST_NAME,        &
           FROM_EXPORT=SRC_ID, TO_IMPORT=DST_ID, __RC)
   ...
   subroutine MAPL_DoNotConnect ( GC, SHORT_NAME, CHILD, RC )
      ...
      call CONN%DONOTCONN%append(SHORT_NAME, TO_IMPORT=CHILD, __RC)
   ...
   subroutine MAPL_TerminateImportAllBut ( GC, SHORT_NAMES, CHILD_IDS, RC )
      ...
      __ASSERT(size(SHORT_NAMES)==size(CHILD_IDS),'needs informative message')
   ...
   subroutine MAPL_RequestService(GC, SERVICE, VARS, RC)
      ...
      bundle = ESMF_FieldBundleCreate(NAME=SERVICE, __RC)
   ...
   subroutine MAPL_ServiceGetBundle(meta, service, bundle, provider, &
      ...
      __ASSERT(do_provider .neqv. do_requester, 'Only one of the arguments PROVIDER or REQUESTER must be provided')
      call MAPL_FindChild(META, name=cname, result=cmeta, rc=status)
      __VERIFY(status)
      __ASSERT(associated(cmeta), 'No child found')
   ...
   subroutine MAPL_GenericStateClockOn(STATE,NAME,RC)
      ...
      call state%t_profiler%start(get_actual_timer_name(name),__RC)
   ...
   subroutine MAPL_StateAlarmAdd(STATE,ALARM,RC)
      ...
      __ASSERT(STATE%ALARMLAST <= LAST_ALARM,'needs informative message')
   ...
   subroutine MAPL_GenericStateClockOff(STATE,NAME,RC)
      ...
      call state%t_profiler%stop(get_actual_timer_name(name),__RC)
   ...
   subroutine MAPL_ESMFStateWriteToFile(STATE,CLOCK,FILENAME,FILETYPE,MPL,HDR, write_with_oserver,clobber,RC)
      ...
      empty = MAPL_IsStateEmpty(state, __RC)
      if (empty) then
         call warn_empty('Checkpoint '//trim(filename), MPL, __RC)
         __RETURN(ESMF_SUCCESS)
      end if
      ...
      elseif(filetype=='pbinary') then
         ...
         TILE: if(IAND(ATTR, MAPL_AttrTile) /= 0) then
            __ASSERT(IAND(ATTR, MAPL_AttrGrid) == 0,'needs informative message') ! no hybrid allowed
            __ASSERT(MAPL_LocStreamIsAssociated(MPL%LOCSTREAM,RC=status),'needs informative message')
            ...
            call ArrDescrCreateWriterComm(arrdes,mpl%grid%comm,mpl%grid%num_writers,__RC)
            call ArrDescrSet(arrdes,                   &
...
!@       __VERIFY(status)
      ...
      elseif(filetype=='pnc4') then
         ...
         if (local_write_with_oserver) then
            call MAPL_VarWriteNCPar(filename,STATE,ArrDes,CLOCK, clobber=local_clobber, oClients=o_clients, __RC)
         else
            call MAPL_VarWriteNCPar(filename,STATE,ArrDes,CLOCK, clobber=local_clobber, __RC)
         end if
      ...
      call ArrDescrCommFree(arrdes, __RC)
      __RETURN(ESMF_SUCCESS)
   ...
   subroutine MAPL_ESMFStateReadFromFile(STATE,CLOCK,FILENAME,MPL,HDR,RC)
      ...
      empty = MAPL_IsStateEmpty(state, __RC)
      if (empty) then
         call warn_empty('Restart '//trim(filename), MPL, __RC)
         __RETURN(ESMF_SUCCESS)
      end if
      ...
      if(INDEX(FNAME,'*') == 0) then
         if (AmIRoot) then
             !if (mpl%grid%split_restart) then
                !hconfig = ESMF_HConfigCreate(filename = trim(filename), __RC)
                !__ASSERT(ESMF_HConfigIsDefined(hconfig,keyString="num_files"),"if input file is split must supply num_files")
                !num_files =  ESMF_HConfigAsI4(hconfig,keystring="num_files",__RC)
                !split_restart = .true.
             !end if
            block
         ...
         call MAPL_CommsBcast(vm,split_restart,n=1,ROOT=MAPL_Root,__RC)

         call MAPL_CommsBcast(vm, fileExists, n=1, ROOT=MAPL_Root, __RC)
         call MAPL_CommsBcast(vm, isNC4, n=1, ROOT=MAPL_Root, __RC)
         !if (split_restart) then
            !call MAPL_CommsBcast(vm, num_files,  n=1, ROOT=MAPL_Root, __RC)
            !call MAPL_CommsBcast(vm, split_restart, n=1, ROOT=MAPL_Root, __RC)
      ...
      elseif(filetype=='pbinary') then
         ...
         TILE: if (on_tiles) then
            ...
            call ArrDescrSet(arrdes,                   &
                 ...
                 im_world = COUNTS(1),                 &
                 jm_world = COUNTS(2)                  )
            call ArrDescrCreateReaderComm(arrdes,mpl%grid%comm,mpl%grid%num_readers,__RC)
         ...
         PNC4_TILE: if (on_tiles) then
            ...
            __ASSERT(grid_is_consistent(grid_type, fname), "grid in the file is different from app's grid")
   ...
   subroutine MAPL_StateCreateFromVarSpecNew(STATE,SPEC,GRID,TILEGRID,DEFER,range,RC)
      ...
      do L = range_(1), range_(2)
         ...
         if (IAND(STAT, MAPL_BundleItem) /= 0 .or. &
            ...
            ! choose the grid
            call MAPL_VarSpecGet(SPEC%var_specs%of(L), FIELD=SPEC_FIELD, __RC)
            is_created = ESMF_FieldIsCreated(SPEC_FIELD, __RC)
            if (is_created) then
               call ESMF_FieldGet(SPEC_FIELD, GRID=GRD, __RC)
            else
...
!
   recursive subroutine MAPL_WireComponent(GC, RC)
      ...
      call MAPL_InternalStateRetrieve ( GC, STATE, __RC)
      ...
      do I=1,NC       !  Cycle thru children's imports
         ...
         gridcomp => STATE%get_child_gridcomp(I)
         call MAPL_GridCompGetVarSpecs(gridcomp, EXPORT=EX_SPECS, __RC)
         ...
         do K=1,size(EX_SPECS)
            call MAPL_VarSpecGet(EX_SPECS(K), SHORT_NAME=SHORT_NAME, __RC)
            is_connected = connect%varIsConnected(IMPORT_NAME=SHORT_NAME, &
                 export=I, import=MAPL_Self, __RC)
            if (is_connected) then
               ...
               if (status /= MAPL_DuplicateEntry) then
                  ...
                  __FAIL('ERROR: duplicate entry for ' // trim(short_name))
               endif
            end if
         end do
            ...
            do J=1,NC
               ...
               if (connect%varIsConnected(IMPORT_NAME=SHORT_NAME, EXPORT_NAME=ENAME, &
                  ...
                  if (EX_SPECS(N) == IM_SPECS(K)) then
                     ...
                     __ASSERT(MAPL_VarSpecSamePrec(EX_SPECS(N), IM_SPECS(K)),'needs informative message')
                     call MAPL_VarSpecSet(EX_SPECS(N), SHORT_NAME=ENAME, RC=status)
                     __VERIFY(status)
                     ! coupler is needed
         ...
         do J=1,NC
            ...
            if(associated(DSTS(J,I)%SPEC)) then
               if(I/=J) then
                  ...
                  call ESMF_AttributeSet(CCS(J,I), name='ClockYetToAdvance', value=.true., __RC)
...
!ALT      call MAPL_WireComponent(GCS(I), RC=status)
   ...
   recursive subroutine MAPL_GenericSpecEnum(GC, SPECS, RC)
      ...
      if (associated(IMPORT_SPEC)) then
         do I = 1, size(IMPORT_SPEC)
            ...
            call MAPL_VarSpecSet(SPECS%old_var_specs(LBL), LABEL=LBL, __RC)
         end do
      end if
      ...
      if (associated(EXPORT_SPEC)) then
         do I = 1, size(EXPORT_SPEC)
            ...
            call MAPL_VarSpecGet(SPECS%old_var_specs(K), LABEL=LBL, __RC)
            if (LBL == 0) then
               call MAPL_VarSpecSet(SPECS%old_var_specs(K), LABEL=K, __RC)
            end if
         end do
      end if
      ...
      do I = 1, STATE%get_num_children()
         gridcomp => STATE%GET_CHILD_GRIDCOMP(I)
         call MAPL_GenericSpecEnum(gridcomp, SPECS, __RC)
      end do
   ...
   subroutine MAPL_FriendlyGet ( GC, NAME, FIELD, REQUESTER, RC )
      ...
      __ASSERT(iand(STAT, MAPL_FriendlyVariable) /= 0,'needs informative message')
      ...
      if (present(REQUESTER)) then
         ...
         __VERIFY(status)
         __ASSERT(FRIENDLY,'needs informative message')
      end if
...
!
   ...
   subroutine MAPL_ReadForcingX(MPL,NAME,DATAFILE,CURRTIME,  &
      ...
      if (itemType == ESMF_STATEITEM_FIELD) then
         ...
         if(ONED) then
            ...
            __ASSERT(L2==L1+1,'needs informative message')
      ...
      __ASSERT(FAC >= 0.0,'needs informative message')
      __ASSERT(FAC <= 1.0,'needs informative message')
   ...
   contains
      ...
      subroutine UPDATE_ENDPOINTS
         ...
         if ( DATE_PREV < 0 .or. CLIMATOLOGY .or. CurrentTime < MIDT1 .or. CurrentTime > MIDT2 ) then
            ...
            if(MIDT2<=CurrentTime .or. use_init_only) then ! The item we found is PREV
               ...
               call MAPL_Backspace(UNIT, LAYOUT, COUNT=4, RC=status); __VERIFY(status)
               ...
               call MAPL_Skip(UNIT, LAYOUT, COUNT=2, RC=status ); __VERIFY(status)
         ...
         elseif(.not.use_init_only) then ! Just need to update NEXT,  PREV is old NEXT
            ...
            __ASSERT(MIDT2 >= CurrentTime,'needs informative message')
      ...
      subroutine READIT(WHICH)
         ...
         if (PRF /= 0) then
            ...
            if (AmReader) then
               ...
               if (io_rank == 0) then
                  ...
                  __FAIL('NAG does not provide ftell. Convert to stream I/O')
#elif defined(__flang__)
                  __FAIL('flang does not provide ftell. Convert to stream I/O')
#else
                  ...
                  __ASSERT(offset >= 4,'needs informative message')
               end if
               !ALT: disclaimer
         ...
         if(TRANSFORM) then
            if (PRF /= 0) then
               __FAIL('needs informative message') ! for now
            else
   ...
   subroutine MAPL_GridCreate(GC, MAPLOBJ, ESMFGRID, srcGC, rc)
      ...
      if(present(srcGC)) then
         call ESMF_GridCompGet ( srcGC, grid=Grid, __RC)
         if(present(GC)) then
            call ESMF_GridCompSet(GC, GRID=GRID, __RC)
         end if
      ...
      if(present(GC)) then
         __ASSERT(.not. present(MAPLOBJ),'needs informative message')
         call MAPL_InternalStateGet(GC, STATE, __RC)
      ...
      if (trim(Prefix) /= '') then
         call MAPL_ConfigPrepend(state%cf,trim(comp_name),MAPL_CF_COMPONENT_SEPARATOR,'NX:', __RC)
         call MAPL_ConfigPrepend(state%cf,trim(comp_name),MAPL_CF_COMPONENT_SEPARATOR,'NY:', __RC)
      endif

      call ESMF_ConfigGetAttribute(state%cf,gridname,label=trim(Prefix)//'GRIDNAME:', __RC)
      nn = len_trim(gridname)
      dateline = gridname(nn-1:nn)
      if (dateline == 'CF') then
         ! convert global NY to a local NY for each face
         call ESMF_ConfigGetAttribute(state%CF,ny,label=trim(Prefix)//'NY:', __RC)
         call MAPL_ConfigSetAttribute(state%CF, value=ny/6, label=trim(Prefix)//'NY:', __RC)
      end if

      grid = grid_manager%make_grid(state%CF, prefix=trim(Prefix), __RC)

      call state%grid%set(grid, __RC)
   ...
   contains
      ...
      subroutine MAPL_ConfigPrepend(cf, comp_name,separator,label,rc)
         ...
         if (status /= ESMF_SUCCESS) then
            call ESMF_ConfigGetAttribute(CF,val,label=trim(label), __RC)
            call MAPL_ConfigSetAttribute(CF, val, label=trim(comp_name)//trim(separator)//trim(label), __RC)
         end if
   ...
   recursive subroutine MAPL_GetRootGC(GC, rootGC, RC)
      ...
      call MAPL_GetObjectFromGC(GC, meta, __RC)

      if (.not. associated(meta%parentGC)) then
         rootGC = GC
      else
         call MAPL_GetRootGC(meta%parentGC, rootGC, __RC)
      end if
   ...
   recursive subroutine MAPL_GenericStateSave( GC, IMPORT, EXPORT, CLOCK, RC )
      ...
      call MAPL_GetResource( STATE, FILENAME,         &
           ...
           RC=status)
      if(status==ESMF_SUCCESS) then
         __ASSERT(.not.allocated(state%initial_state%imp_fname),"can only save one state")
      ...
      call MAPL_GetResource( STATE   , filename,  &
           ...
           RC=status)
      if(status==ESMF_SUCCESS) then
         __ASSERT(.not.allocated(state%initial_state%int_fname),"can only save one state")
   ...
   subroutine MAPL_AddRecord(MAPLOBJ, ALARM, FILETYPE, RC)
      ...
      NRA = size(ALARM)
      __ASSERT(size(filetype) == NRA,'needs informative message')
   ...
   function  MAPL_GridGetSection(Grid, SectionMap, GridName, RC) result(SECTION)
      ...
      __ASSERT(dimcount == size(SectionMap),'needs informative message')
   ...
   subroutine MAPL_DoNotAllocateVar(SPEC, NAME, notFoundOK, RC)
      ...
      __ASSERT(I>0,'needs informative message') ! make sure NAME is in the SPEC
...
!C$      call meta_lgr%warning('obsolete interface MAPL_GetLogger()')
   ...
   subroutine warn_empty(string, MPL, rc)
      ...
      if (MAPL_Am_I_Root()) then
         call MAPL_GetLogger(mpl, lgr, __RC)
   ...
   recursive subroutine MAPL_AddAttributeToFields_I4(gc,field_name,att_name,att_val,rc)
      ...
      call MAPL_GetObjectFromGC(gc,state,__RC)
      call ESMF_StateGet(state%import_state,field_name,item_type,__RC)
      if (item_type == ESMF_STATEITEM_FIELD) then
         call ESMF_StateGet(state%import_state,field_name,field,__RC)
         call ESMF_AttributeGet(field,name=att_name,isPresent=is_Present,__RC)
         if (is_present) then
            call ESMF_AttributeGet(field,name=att_name,typekind=item_kind,itemCount=item_count,__RC)
            __ASSERT(item_kind == ESMF_TYPEKIND_I4,"attribute "//att_name//" in "//field_name//" is not I4")
            __ASSERT(item_count==1,"attribute "//att_name//" in "//field_name//" is not a scalar")
         end if
         call ESMF_AttributeSet(field,name=att_name,value=att_val,__RC)
      end if
      nc = state%get_num_children()
      do i=1,nc
         child_gc => state%get_child_gridcomp(i)
         call MAPL_AddAttributeToFields_I4(child_gc,field_name,att_name,att_val,__RC)
      enddo
   ...
   subroutine MAPL_MethodAdd(state, label, userRoutine, rc)
      ...
      call ESMF_MethodAdd(state, label=label, userRoutine=userRoutine, __RC)

      call get_callbacks(state, callbacks, __RC)
