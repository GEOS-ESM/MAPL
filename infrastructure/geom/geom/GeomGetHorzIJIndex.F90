...
contains
   ...
   subroutine get_horz_ij_index_r4(geom, lon, lat, ii, jj, local_coordinates, rc)
      ...
      logical, optional, intent(in) :: local_coordinates
      ...
      if (present(local_coordinates)) then
         if (local_coordinates) then
   ...
   subroutine get_horz_ij_index_r8(geom, lon, lat, ii, jj, local_coordinates, rc)
      type(ESMF_Geom), intent(in) :: geom
