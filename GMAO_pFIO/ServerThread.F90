...
contains
   ...
   subroutine run(this, rc)
      ...
      !if ( this%containing_server%serverthread_done_msgs(this%thread_rank)) then
      !   call dMessage%dispatch(this)
      !   return
      !endif
   ...
   recursive subroutine handle_Done(this, message, rc)
      ...
      type is (CollectivePrefetchDataMessage)
         ...
         if (data_status == UNALLOCATED ) then
            ...
            call this%containing_server%set_dataReference(dataRefPtr)
            nullify(dataRefPtr)
         endif
   ...
   subroutine create_remote_win(this, rc) 
      ...
      do collection_counter = 1, collection_total
         ...
         call this%containing_server%set_dataReference(remotePtr)
         remotePtr=>null()
      enddo
      _RETURN(_SUCCESS)
...
!      if (product(count) /= product(file_data_reference%shape)) stop "memory size not match"
   ...
   subroutine handle_StageData(this, message, rc)
      ...
      class(AbstractSocket),pointer :: connection
      type(LocalMemReference) :: mem_data_reference
      type(DummyMessage) :: hand_shake_msg

      connection=>this%get_connection()
      call connection%send(hand_shake_msg)
      call this%request_backlog%push_back(message)
...
!      if (product(count) /= product(file_data_reference%shape)) stop "memory size not match"
   ...
   subroutine receive_output_data(this, rc)
     ...
     do while (iter /= this%request_backlog%end())
        ...
        type is (CollectiveStageDataMessage)
            ...
            local_size = product(msg%count)*word_size(msg%type_kind)
            ...
            call MPI_type_create_subarray(ndims,words*msg%global_count,words*msg%count,words*(msg%start -1), &
            ...
            ! forward data to the mem_rank
            ! if it is nondistributed data, only the first thread forwards the data
            forward = ( .not. (all(msg%count == msg%global_count)) ) & ! collective
                      .or. ( this%thread_rank == 1 .and. this%containing_server%rank == rank) ! non distributed 
  ...
  subroutine clear_backlog(this)
    class (ServerThread),intent(inout) :: this
    call this%request_backlog%clear()
