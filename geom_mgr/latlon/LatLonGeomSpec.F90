...
contains
   ...
   function new_LatLonGeomSpec(lon_axis, lat_axis) result(spec)
   ...
   ! HConfig section
   function make_LatLonGeomSpec_from_hconfig(hconfig, rc) result(spec)
      type(LatLonGeomSpec) :: spec
   ...
   function make_LonAxis_from_hconfig(hconfig, regional, rc) result(axis)
   ...
   function make_LatAxis_from_hconfig(hconfig, regional, rc) result(axis)
      type(LatLonAxis) :: axis
      type(ESMF_HConfig), intent(in) :: hconfig
      logical, intent(in) :: regional
      integer, optional, intent(out) :: rc
   ...
   function get_distribution(hconfig, m_world, key_npes, key_distribution, rc) result(distribution)
      integer, allocatable :: distribution(:)
      type(ESMF_HConfig), intent(in) :: hconfig
   ...
   function get_lon_range(hconfig, im_world, regional, rc) result(ranges)
      type(AxisRanges) :: ranges
      type(ESMF_HConfig), intent(in) :: hconfig
   ...
   function get_lat_range(hconfig, jm_world, regional, rc) result(ranges)
      type(AxisRanges) :: ranges
      type(ESMF_HConfig), intent(in) :: hconfig
   ...
   ! dimension.
   function make_LatLonGeomSpec_from_metadata(file_metadata, rc) result(spec)
      type(LatLonGeomSpec) :: spec
   ...
   function make_distribution(im, nx) result(distribution)
      integer, allocatable :: distribution(:)
   ...
   function get_coordinates_try(file_metadata, try1, try2, rc) result(coordinates)
   ...
   function get_lon_corners(centers) result(corners)
   ...
   function get_lat_corners(centers) result(corners)
      real(kind=ESMF_KIND_R8), intent(in) :: centers(:)
      real(kind=ESMF_KIND_R8), allocatable :: corners(:)
   ...
   subroutine fix_bad_pole(centers)
   ...
   function get_dim_name(file_metadata, try1, try2, rc) result(dim_name)
   ...
   ! ------------------------------------------------------------------------------------
   ! This function attempts to find a layout with roughly square
   ! domains on each process.  Optimal value for
   !     nx = (im_world * petcount) / jm_world
   ! Except, it needs to be an integer
   ! --------------------------------------------------------------------
   function make_de_layout_petcount(aspect_ratio, petCount) result(nx_ny)
      integer :: nx_ny(2)
   ...
   function make_de_layout_vm(aspect_ratio, vm, rc) result(nx_ny)
      integer :: nx_ny(2)
