...
contains
   ...
   function make_geom_spec_from_hconfig(hconfig, supports, rc) result(geom_spec)
      ...
      geom_spec = LatLonGeomSpec(hconfig, supports=supports, _RC)
   ...
   function make_mapl_geom_from_metadata(metadata, rc) result(geom_spec)
      ...
      geom_spec = LatLonGeomSpec(metadata, _RC)
      
      _RETURN(_SUCCESS)
   end function make_mapl_geom_from_metadata


   logical function supports(this, geom_spec) result(supports)
      class(LatLonGeomFactory), intent(in) :: this
      class(GeomSpec), intent(in) :: geom_spec

      supports = same_type_as(geom_spec, LatLonGeomSpec)
         
   end function supports

   function make_geom(this, geom_spec, supports, rc) result(geom)
      class(LatLonGeomFactory), intent(in) :: this
      class(GeomSpec), intent(in) :: geom_spec
      ...
      class default
         geom = nullgeom
         _FAIL(_NOT_SUPPORTED, "geom_spec type not supported")
      end select
      
      _RETURN(_SUCCESS)
   end function make_geom

   function typesafe_make_geom(spec, rc)
...
!#      call this%add_horz_coordinates(grid, _RC)
   ...
   function create_basic_grid(spec, unusable, rc) result(grid)
      type (ESMF_Grid) :: grid
   ...
   function new_LatLonGeomSpec_from_metadata(file_metadata, supports, rc) result(spec)
      ...
      character(:), allocatable :: lon_name, lat_name, lev_name
      ...
      associate (im => spec%im_world, jm => spec%jm_world, lm => spec%lm)
        ...
        lev_name = find_dim_name(file_metadata, 'lev', 'levels', _RC)
