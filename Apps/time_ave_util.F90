...
!call timebeg ('main')
   ...
   __VERIFY(ierror)
   call ESMF_Initialize(logKindFlag=ESMF_LOGKIND_NONE,mpiCommunicator=MPI_COMM_WORLD, __RC)
   call MAPL_Initialize(__RC)
   t_prof = DistributedProfiler('time_ave_util',MpiTImerGauge(),MPI_COMM_WORLD)
   call t_prof%start(__RC)
   call io_server%initialize(MPI_COMM_WORLD,__RC)
   root = myid.eq.0
...
! **********************************************************************
   ...
   call file_handle%open(trim(name),PFIO_READ,__RC)
   basic_metadata = file_handle%read(__RC)
   call file_handle%close(__RC)

   allocate(factory, source=grid_manager%make_factory(trim(name)))
   input_grid = grid_manager%make_grid(factory)
   file_has_lev = has_level(input_grid,__RC)
   call MAPL_GridGet(input_grid,globalCellCountPerDim=global_dims,__RC)
   lm = global_dims(3)

   if (file_has_lev) then
      call get_file_levels(trim(name),vertical_data,__RC)
   end if

   if (allocated(output_grid_name)) then
      output_grid = create_output_grid(output_grid_name,lm,__RC)
   else
      output_grid = input_grid
   end if
   call ESMF_AttributeGet(output_grid,'GridType',grid_type,__RC)
   allow_zonal_means = trim(grid_type) == 'LatLon'
   if (trim(grid_type) == "Cubed-Sphere") then
      __ASSERT(mod(npes,6)==0,"If input files are Cubed-Sphere, must be run on multiple of 6 proccessors")
   end if
   call MAPL_GridGet(output_grid,localCellCountPerDim=local_dims,globalCellCountPerDim=global_dims,__RC)
   im = local_dims(1)
   jm = local_dims(2)
   lm = local_dims(3)
   ...
   call get_file_times(file_metadata,ntime,time_series,timinc,yymmdd,hhmmss,__RC)
   primary_bundle = ESMF_FieldBundleCreate(name="first_file",__RC)
   call ESMF_FieldBundleSet(primary_bundle,grid=output_grid,__RC)
   call MAPL_Read_Bundle(primary_bundle,trim(name),time=time_series(1),__RC)
   call ESMF_FieldBundleGet(primary_bundle,fieldCount=nvars,__RC)
   allocate(vname(nvars))
   call ESMF_FieldBundleGet(primary_bundle,fieldNameList=vname,__RC)
   kmvar = get_level_info(primary_bundle,__RC)
   vtitle = get_long_names(primary_bundle,__RC)
   vunits = get_units(primary_bundle,__RC)

   final_bundle = ESMF_FieldBundleCreate(name="first_file",__RC)
   call ESMF_FieldBundleSet(final_bundle,grid=output_grid,__RC)
   diurnal_bundle = ESMF_FieldBundleCreate(name="first_file",__RC)
   call ESMF_FieldBundleSet(diurnal_bundle,grid=output_grid,__RC)
   call copy_bundle_to_bundle(primary_bundle,final_bundle,__RC)

   if (size(time_series)>1) then
      time_interval = time_series(2) - time_series(1)
   else if (size(time_series)==1) then
      call ESMF_TimeIntervalSet(time_interval,h=6,__RC)
   end if
   clock = ESMF_ClockCreate(startTime=time_series(1),timeStep=time_interval,__RC)

   nvars2 = nvars

   if (file_has_lev) then
      lev_name = file_metadata%get_level_name(__RC)
      call file_metadata%get_coordinate_info(lev_name,coords=lev,coordUnits=lev_units,__RC)
   end if

   previous_undef = file_metadata%var_get_missing_value(trim(vname(1)),__RC)
   do i=2,size(vname)
      undef = file_metadata%var_get_missing_value(trim(vname(i)),__RC)
      __ASSERT(undef == previous_undef,"conflicting undefined values in your variables")
...
! -----------------------------
   ...
   do nv=1,nquad
      if( qloc(1,nv)*qloc(2,nv).ne.0 ) then
         ...
         if( nstar.ne.0 ) then
            __ASSERT(allow_zonal_means,"grid is not lat-lon so cannot compute zonal means")
            lzstar(nv) = .TRUE.
         ...
         call add_new_field_to_bundle(final_bundle,output_grid,kmvar(qloc(1,nv)),vname2(mv),vtitle2(mv),vunits2(mv),__RC)
...
! **********************************************************************
   ...
   do n=1,nfiles
      ...
      call file_handle%open(trim(fname(n)),PFIO_READ,__RC)
      basic_metadata = file_handle%read(__RC)
      call file_handle%close(__RC)
...
! -------------------------------
            if( strict .and. first ) then
               ...
               if( year.eq.yearm .and. month.eq.monthm ) then
                  ...
                  __FAIL("error processing dataset")
               endif
            endif
...
! --------------
            ...
            etime = local_esmf_timeset(nymd,nhms,__RC)
            call MAPL_Read_Bundle(primary_bundle,trim(fname(1)),time=etime,file_override=trim(fname(n)),__RC)
            do nv=1,nvars2
               call ESMF_FieldBundleGet(primary_bundle,trim(vname2(nv)),field=field,__RC)
            ...
            do nv=1,nquad
               if( qloc(1,nv)*qloc(2,nv).ne.0 ) then
                  ...
                  do L=1,max(1,kmvar2(qloc(1,nv)))
                     if( lzstar(nv) ) then
                        call latlon_zstar (dum(:,:,nloc(qloc(1,nv))+L-1),dumz1,undef,output_grid,__RC)
                        call latlon_zstar (dum(:,:,nloc(qloc(2,nv))+L-1),dumz2,undef,output_grid,__RC)
                        do j=1,jm
                           do i=1,im
...
! --------------
   if( root ) print *
   do n=1,nvars2
      call ESMF_FieldBundleGet(final_bundle,trim(vname2(n)),field=field,__RC)
...
! ----------
   ...
   do nv=1,nquad
      if( qloc(1,nv)*qloc(2,nv).ne.0 ) then
         ...
         call ESMF_FieldBundleGet(final_bundle,trim(vname2(mv)),field=field,__RC)
   ...
   etime = local_esmf_timeset(nymd0,nhms0,__RC)
   call ESMF_ClockSet(clock,currTime=etime, __RC)
   call standard_writer%create_from_bundle(final_bundle,clock,n_steps=1,time_interval=timeinc,vertical_data=vertical_data,__RC)
   call standard_writer%start_new_file(trim(hdfile),__RC)
   call standard_writer%write_to_file(__RC)
      ...
      do k=1,ntods
         ...
         if( k.eq.1 .or. mdiurnal ) then
            ...
            if( ldquad ) then
               ...
               if (k==1) then
                  call copy_bundle_to_bundle(final_bundle,diurnal_bundle,__RC)
               end if
            else
               ...
               if (k==1) then
                  do n=1,nvars
                     call ESMF_FieldBundleGet(final_bundle,trim(vname2(n)),field=field,__RC)
                     call MAPL_FieldBundleAdd(diurnal_bundle,field,__RC)
                  enddo
               endif
            endif
         endif
...
! --------------
         do n=1,nvars2
            call ESMF_FieldBundleGet(diurnal_bundle,trim(vname2(n)),field=field,__RC)
            if( kmvar2(n).eq.0 ) then
               kbeg = 0
               kend = 1
               call ESMF_FieldGet(field,0,farrayPtr=ptr2d,__RC)
...
! ----------
         if( ndvars.eq.mvars ) then
            ...
            do nv=1,nquad
               if( qloc(1,nv)*qloc(2,nv).ne.0 ) then
                  mv=mv+1
                  call ESMF_FieldBundleGet(diurnal_bundle,trim(vname2(mv)),field=field,__RC)
                  if( kmvar2(qloc(1,nv)).eq.0 ) then
                     kbeg = 0
                     kend = 1
                  else
         ...
         if (k==1 .or. mdiurnal) then
            ...
            call diurnal_writer%start_new_file(trim(hdfile),__RC)
         end if
         call diurnal_writer%write_to_file(__RC)
...
contains
   ...
   function create_output_grid(grid_name,lm,rc) result(new_grid)
      ...
      if (dateline=='CF') then
         ...
         __FAIL("Tripolar not yet implemented for outpout")
      else
         ...
         if (pole=='XY' .and. dateline=='XY') then
            __FAIL("regional lat-lon output not supported")
         end if
      end if

      new_grid = grid_manager%make_grid(cf,prefix=trim(grid_name)//".",__RC)
      if (present(rc)) then
         rc=__SUCCESS
      end if
   ...
   function has_level(grid,rc) result(grid_has_level)
      ...
      call MAPL_GridGet(grid,globalCellCountPerDim=global_dims,__RC)
   ...
   subroutine copy_bundle_to_bundle(input_bundle,output_bundle,rc)
      ...
      call ESMF_FieldBundleGet(input_bundle,fieldCount=num_fields,__RC)
      allocate(field_list(num_fields))
      call ESMF_FieldBundleGet(input_bundle,fieldNameList=field_list,__RC)
      do i=1,num_fields
         call ESMF_FieldBundleGet(input_bundle,field_list(i),field=field,__RC)
         call MAPL_FieldBundleAdd(output_bundle,field,__RC)
      enddo
      if (present(rc)) then
         RC=__SUCCESS
      end if
   ...
   subroutine add_new_field_to_bundle(bundle,grid,lm,field_name,long_name,units,rc)
      ...
      if (lm == 0) then
         field = ESMF_FieldCreate(grid,name=trim(field_name),typekind=ESMF_TYPEKIND_R4,__RC)
      else if (lm > 0) then
         field = ESMF_FieldCreate(grid,name=trim(field_name),typekind=ESMF_TYPEKIND_R4, &
               ungriddedLBound=[1],ungriddedUBound=[lm],__RC)
      end if
      call ESMF_AttributeSet(field,name='LONG_NAME',value=trim(long_name),__RC)
      call ESMF_AttributeSet(field,name='UNITS',value=trim(units),__RC)
      if (lm == 0) then
   ...
   subroutine get_file_times(file_metadata,num_times,time_series,time_interval,yymmdd,hhmmss,rc)
      ...
      num_times = file_metadata%get_dimension('time',__RC)
      call file_metadata%get_time_info(timeVector=time_series,__RC)
      if (num_times == 1) then
         time_interval = file_metadata%get_var_attr_int32('time','time_increment',__RC)
      else if (num_times > 1) then
         esmf_time_interval = time_series(2)-time_series(1)
         call ESMF_TimeIntervalGet(esmf_time_interval,h=hour,m=minute,s=second,__RC)
         time_interval = hour*10000+minute*100+second
      end if

      allocate(yymmdd(num_times),hhmmss(num_times))
      do i = 1,num_times
         call ESMF_TimeGet(time_series(i),yy=year,mm=month,dd=day,h=hour,m=minute,s=second,__RC)
   ...
   function get_level_info(bundle,rc) result(kmvar)
      ...
      call ESMF_FieldBundleGet(bundle,fieldCount=num_fields,__RC)
      allocate(field_list(num_fields))
      allocate(kmvar(num_fields))
      call ESMF_FieldBundleGet(bundle,fieldNameList=field_list,__RC)
      do i=1,num_fields
         call ESMF_FieldBundleGet(bundle,field_list(i),field=field,__RC)
         call ESMF_FieldGet(field,rank=rank,__RC)
         if (rank==2) then
            kmvar(i)=0
         else if (rank==3) then
            call ESMF_FieldGet(field,ungriddedLBound=lb,ungriddedUBound=ub,__RC)
            kmvar(i)=ub(1)-lb(1)+1
         else
   ...
   function get_long_names(bundle,rc) result(long_names)
      ...
      do i=1,num_fields
         call ESMF_FieldBundleGet(bundle,field_list(i),field=field,__RC)
         call ESMF_AttributeGet(field,name='LONG_NAME',value=long_names(i),__RC)
      enddo
      if (present(rc)) then
         RC=__SUCCESS
      end if
   ...
   function get_units(bundle,rc) result(units)
      ...
      do i=1,num_fields
         call ESMF_FieldBundleGet(bundle,field_list(i),field=field,__RC)
         call ESMF_AttributeGet(field,name='UNITS',value=units(i),__RC)
      enddo
      if (present(rc)) then
         RC=__SUCCESS
      end if
   ...
   function local_esmf_timeset(yymmdd,hhmmss,rc) result(etime)
      ...
      call ESMF_TimeSet(etime,yy=year,mm=month,dd=day,h=hour,m=minute,s=second,__RC)
      if (present(rc)) then
         rc=__SUCCESS
      endif
   ...
   subroutine latlon_zmean ( q,qz,undef,grid,rc)
      ...
      call MAPL_GridGet(grid,localCellCountPerDim=local_dims,globalCellCountPerDim=global_dims,__RC)
      im = local_dims(1)
      jm = local_dims(2)
      im_global = global_dims(1)
      jm_global = global_dims(2)
      call get_esmf_grid_layout(grid,nx,ny,ims,jms,__RC)
...
! send mean back to other ranks
   ...
   subroutine get_esmf_grid_layout(grid,nx,ny,ims_out,jms_out,rc)
      ...
      call ESMF_VMGetCurrent(vm,__RC)
      call ESMF_VMGet(vm,petCount=ndes,__RC)
      ...
      call MAPL_DistGridGet(dist_grid,minIndex=minindex,maxIndex=maxindex,__RC)
      call MAPL_GetImsJms(minindex(1,:),maxindex(1,:),minindex(2,:),maxindex(2,:),ims,jms,__RC)
      nx = size(ims)
      ny = size(jms)
