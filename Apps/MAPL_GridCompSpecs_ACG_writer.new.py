...
def add_to_tuple_dict(spec, dct, key):
...
def readline_with_number(file, line_number):
    return file.readline(), line_number + 1
...
#!        line, line_number = readline_with_number(code, line_number)
...
#!                        line, line_number = readline_with_number(code, line_number)
                ...
                if not continued(line):
                    ...
                    new_spec = f'{RANGE_OPEN}{start_line_number}{RANGE_CONNECTOR}{line_number}{DELIMITER}{joined}'
                    records = add_to_tuple_eict(new_spec, records, category)
...
#!            line, line_number = readline_with_number(code, line_number)
    ...
    # ENDWITH: with open(args.input, 'r') as code:
...
def parse_records(records):
    ...
    range_re = re.compile(f'^{RANGE_OPEN}(?P<first>[0-9]+){RANGE_CONNECTOR}(?P<last>[0-9]+)$', re.I)
    ...
    for category in records:
        ...
        for spec in reader:
            ...
            for col in spec:
                ...
                m = range_re.match(col)
                if m:
                    first_string = m.group('first')
                    last_string = m.group('last')
                    range_string = '# line ' + first_string + ' to line ' + last_string
                    fields[RANGE_TAG] = range_string
